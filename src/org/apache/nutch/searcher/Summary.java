...
public class Summary implements Writable {
	...
	public static class Fragment {
		...
		public boolean equals(Object o) {
			try {
				...
				return f.getText().equals(getText()) && f.isHighlight() == isHighlight() && f.isEllipsis() == isEllipsis();
			} catch (Exception e) {
				return false;
			}
		}
	...
	/**
	 * Returns a HTML representation of this Summary. HTML output for
	 * <b>Highlight</b> fragments is
	 * <code>&lt;span class="highlight"&gt;highlight's text&lt;/span&gt;</code>,
	 * for <b>Ellipsis</b> fragments is
	 * <code>&lt;span class="highlight"&gt; ... &lt;/span&gt;</code>, for
	 * generic <b>Fragment</b> is simply the fragment's text.<br/>
	 * 
	 * @param encode
	 *            specifies if the summary's entities should be encoded.
	 */
		...
		for (int i = 0; i < fragments.size(); i++) {
			...
			if (fragment.isHighlight()) {
				buf.append("<span class=\"highlight\">")
						.append(encode ? Entities.encode(fragment.getText()) : fragment.getText()).append("</span>");
			} else if (fragment.isEllipsis()) {
				buf.append("<span class=\"ellipsis\"> ... </span>");
			} else {
				buf.append(encode ? Entities.encode(fragment.getText()) : fragment.getText());
			}
		}
		return buf.toString();
	}
	...
	/**
	 * Helper method that return a String representation for each specified
	 * Summary.
	 */
