...
public class IndexSearcher implements Searcher, HitDetailer {
	...
	public IndexSearcher(Path index, Configuration conf) throws IOException {
		this.conf = conf;
		this.fs = FileSystem.get(conf);
	...
	public Hits search(Query query) throws IOException {
		...
		return translateHits(optimizer.optimize(luceneQuery, luceneSearcher, query.getParams().getNumHits(), query.getParams()
				.getSortField(), query.getParams().isReverse()), query.getParams().getDedupField(), query.getParams()
				.getSortField());
	}

	public String getExplanation(Query query, Hit hit) throws IOException {
		return luceneSearcher.explain(this.queryFilters.filter(query), Integer.valueOf(hit.getUniqueKey())).toHtml();
	}
