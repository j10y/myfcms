...
public class ArchRecTypeAction extends BaseAction {
	...
	public String combo() {
		...
		for (ArchRecType type : list) {
			sb.append("['").append(type.getRecTypeId()).append("','").append(type.getTypeName())
					.append("'],");
		}
		if (list.size() > 0) {
	...
	public String get() {
		...
		JSONSerializer serializer = new JSONSerializer();
		sb.append(serializer.exclude(new String[] { "class", "department.class" }).serialize(
				archRecType));
		sb.append("}");
		setJsonString(sb.toString());
 ...
 * Qualified Name: com.htsoft.oa.action.archive.ArchRecTypeAction JD-Core
 * Version: 0.5.4
 */