...
public class ArchRecUserAction extends BaseAction {
	...
	public String depList() {
		...
		for (int i = 0; i < list.size(); ++i) {
			...
			sb.append("{'depId':'" + dep.getDepId() + "','depName':'" + dep.getDepName()
					+ "','depLevel':" + dep.getDepLevel() + ",");
			if (ar != null)
				sb.append("'archRecId':'" + ar.getArchRecId() + "','userId':'" + ar.getUserId()
						+ "','fullname':'" + ar.getFullname() + "'}");
			else {
	...
	public String save() {
		...
		if (StringUtils.isNotEmpty(data)) {
			Gson gson = new Gson();
			ArchRecUser[] aru = (ArchRecUser[]) gson.fromJson(data, ArchRecUser[].class);
			for (ArchRecUser archRecUser : aru) {
				...
				if (archRecUser.getDepId() != null) {
					Department department = (Department) this.departmentService.get(archRecUser
							.getDepId());
	...
	public String select() {
		...
		if (StringUtils.isNotEmpty(strDepId)) {
			List<AppUser> list = this.appUserService.findByDepId(new Long(strDepId));
			for (AppUser appUser : list) {
				sb.append("['").append(appUser.getUserId()).append("','").append(
						appUser.getFullname()).append("'],");
			}
			if (list.size() > 0) {
