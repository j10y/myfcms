...
public class MutilMailAction extends BaseAction {
	...
	public String send() {
		...
		if (type.shortValue() == 0) {
			for (int i = 0; i < id.length; ++i) {
				Customer customer = (Customer) this.customerService.get(new Long(id[i]));
		...
		if (tos.length > 0) {
			...
			if ((StringUtils.isNotEmpty((String) configs.get("host")))
					&& (StringUtils.isNotEmpty((String) configs.get("username")))
					&& (StringUtils.isNotEmpty((String) configs.get("password")))
					&& (StringUtils.isNotEmpty((String) configs.get("from")))) {
				this.mailEngine.setFrom((String) configs.get("from"));
				this.mailEngine.sendMimeMessage(from, tos, cc, replyTo, subject, htmlMsgContent,
						attachedFileNames, attachedFiles, false);
	...
	public String loadVm() {
		...
		if (list.size() > 0) {
			...
			String pageHtml = VelocityEngineUtils.mergeTemplateIntoString(velocityEngine,
