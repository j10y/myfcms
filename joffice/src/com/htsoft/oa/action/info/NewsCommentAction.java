...
public class NewsCommentAction extends BaseAction {
	...
	public String list() {
		...
		for (NewsComment newsComment : list) {
			buff.append("{commentId:'").append(newsComment.getCommentId()).append("',subject:")
					.append(gson.toJson(newsComment.getNews().getSubject())).append(",content:")
					.append(gson.toJson(newsComment.getContent())).append(",createtime:'").append(
							simpleDate.format(newsComment.getCreatetime())).append("',fullname:'")
					.append(newsComment.getFullname()).append("',start:'").append(start).append(
							"'},");
		}
		if (list.size() > 0) {
	...
	public String save() {
		...
		this.newsComment
				.setAppUser((AppUser) this.appUserService.get(this.newsComment.getUserId()));
