 ...
 public class AppTipsAction extends BaseAction
 {
   ...
   public String multiDel()
   {
     ...
     if (getRequest().getParameter("ids").equals("all")) {
       ...
       filter.addFilter("Q_appUser.userId_L_EQ", ContextUtil.getCurrentUserId().toString());
       List list = this.appTipsService.getAll(filter);
       for (Iterator localIterator = list.iterator(); localIterator.hasNext(); ) { tips = (AppTips)localIterator.next();
       ...
       if (ids != null)
       {
         ...
         AppTips localAppTips1 = (arrayOfString1 = ids).length; for (tips = 0; tips < localAppTips1; ++tips) { String id = arrayOfString1[tips];
   ...
   public String save()
   {
     ...
     if (StringUtils.isNotEmpty(data)) {
       Gson gson = new Gson();
       AppTips[] tips = (AppTips[])gson.fromJson(data, [Lcom.htsoft.oa.model.info.AppTips.class);
