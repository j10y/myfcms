 ...
 public class DocPrivilegeAction extends BaseAction
 {
   ...
   public String list()
   {
     ...
     List list = this.docPrivilegeService.getAll(filter);
   ...
   public String change()
   {
     ...
     if (StringUtils.isNotEmpty(strPrivilegeId)) {
       this.docPrivilege = ((DocPrivilege)this.docPrivilegeService.get(Long.valueOf(Long.parseLong(strPrivilegeId))));
       ...
       if (StringUtils.isNotEmpty(strRoleIds)) {
         ...
         if (roles.length > 0) {
           for (int i = 0; i < roles.length; ++i) {
             ...
             docp.setFlag(Short.valueOf(3));
             Integer roleId = Integer.valueOf(Integer.parseInt(roles[i]));
             AppRole appRole = (AppRole)this.appRoleService.get(Long.valueOf(roleId.longValue()));
             docp.setUdrId(roleId);
             docp.setUdrName(appRole.getName());
             docp.setRights(rights);
             docp.setFdFlag(Short.valueOf(0));
       ...
       if (StringUtils.isNotEmpty(strUserIds)) {
         ...
         if (userIds.length > 0) {
           for (int i = 0; i < userIds.length; ++i) {
             DocPrivilege docp = new DocPrivilege();
             docp.setFolderId(folderId);
             docp.setFlag(Short.valueOf(1));
             Integer userId = Integer.valueOf(Integer.parseInt(userIds[i]));
             AppUser appUser = (AppUser)this.appUserService.get(Long.valueOf(userId.longValue()));
       ...
       if (StringUtils.isNotEmpty(strDepIds)) {
         ...
         if (depIds.length > 0) {
           for (int i = 0; i < depIds.length; ++i) {
             DocPrivilege docp = new DocPrivilege();
             docp.setFolderId(folderId);
             docp.setFlag(Short.valueOf(2));
             Integer depId = Integer.valueOf(Integer.parseInt(depIds[i]));
             Department department = (Department)this.departmentService.get(Long.valueOf(depId.longValue()));
             docp.setUdrId(depId);
