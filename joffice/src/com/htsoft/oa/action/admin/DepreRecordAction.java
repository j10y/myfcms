...
public class DepreRecordAction extends BaseAction {
	...
	public String depreciate()
   {
     ...
     if (StringUtils.isNotEmpty(strAssetsId)) {
       FixedAssets fixedAssets = (FixedAssets)this.fixedAssetsService.get(
         new Long(strAssetsId));
       ...
       if (method == 1) {
         ...
         BigDecimal monthRate = yearRate
           .divide(new BigDecimal(12), 2, 2);
         Date lastCalTime = this.depreRecordService.findMaxDate(
           new Long(strAssetsId));
         if (lastCalTime == null) {
         ...
         while (deprePeriod.intValue() >= 1) {
           ...
           if (curDate.after(new Date())) break label1938;
         ...
         if (method == 2) {
           ...
           if (StringUtils.isNotEmpty(cruCalDate)) {
             ...
             this.depreRecord.setDepreAmount(per.multiply(this.depreRecord
               .getWorkCapacity()));
             this.depreRecordService.save(this.depreRecord); break label1938:
           }
           setJsonString("{success:false}");
           return "success";
         }
         if (method == 3) {
           ...
           while (deprePeriod.intValue() > 0) {
             ...
             gc1.add(2, deprePeriod.intValue());
             if (gc1.getTime().after(new Date())) break label1938;
           ...
           while (deprePeriod.intValue() > 0) {
             ...
             Integer jian = Integer.valueOf(gc1.get(1) - 
               gcLast.get(1));
             if (gc1.getTime().after(new Date())) break;
             if (gc1.getTime().after(gcYears.getTime())) {
               break;
             }
             i = Integer.valueOf(i.intValue() + 1);
             Integer be = Integer.valueOf(gc1.get(1) - 
               gcStart.get(1));
             ...
             if (jian.intValue() == 0) {
               ...
               Integer beLast = Integer.valueOf(gcLast.get(1) - 
                 gcStart.get(1));
               ...
               depValue = stValue.multiply(rateLast).multiply(
                 new BigDecimal(months.intValue()).divide(
                 new BigDecimal(12), 2, 2));
       ...
       label1938: fixedAssets.setAssetCurValue(cruValue);
