...
public class DepreRecordAction extends BaseAction {
	...
	public String list() {
		...
		List list = this.depreRecordService.getAll(filter);
		StringBuffer buff = new StringBuffer("{success:true,'totalCounts':").append(
				filter.getPagingBean().getTotalItems()).append(",result:");
		JSONSerializer serializer = new JSONSerializer();
		serializer
				.transform(new DateTransformer("yyyy-MM-dd HH:mm:ss"), new String[] { "calTime" });
	...
	public String get() {
		DepreRecord depreRecord = (DepreRecord) this.depreRecordService.get(this.recordId);
		Gson gson = new Gson();
	...
	public String depreciate() {
		...
		if (StringUtils.isNotEmpty(strAssetsId)) {
			FixedAssets fixedAssets = (FixedAssets) this.fixedAssetsService.get(new Long(
					strAssetsId));
			...
			if (method == 1) {
				...
				BigDecimal value = assetValue.multiply(new BigDecimal(deprePeriod.toString()))
						.multiply(monthRate);
				...
				while (deprePeriod.intValue() >= 1) {
					...
					if (curDate.after(gcYears.getTime())) {
						fixedAssets.setAssetCurValue(cruValue);
					}
				...
				if (method == 2) {
					...
					if (StringUtils.isNotEmpty(cruCalDate)) {
						...
						BigDecimal total = new BigDecimal(1).subtract(
								fixedAssets.getRemainValRate().divide(new BigDecimal(100)))
								.multiply(fixedAssets.getAssetValue());
						BigDecimal per = total.divide(fixedAssets.getIntendWorkGross(), 2, 2);
						cruValue = cruValue.subtract(per.multiply(this.depreRecord
								.getWorkCapacity()));

						this.depreRecord.setCalTime(calDate);
						this.depreRecord.setFixedAssets(fixedAssets);
						this.depreRecord.setDepreAmount(per.multiply(this.depreRecord
								.getWorkCapacity()));
				...
				if (method == 3) {
					...
					GregorianCalendar gcYears = new GregorianCalendar();
					BigDecimal yearRate = new BigDecimal(2).divide(fixedAssets.getIntendTerm(), 2,
							3);
					...
					while (deprePeriod.intValue() > 0) {
						...
						if (gc1.getTime().after(new Date()))
							fixedAssets.setAssetCurValue(cruValue);
						if (gc1.getTime().after(gcYears.getTime())) {
							fixedAssets.setAssetCurValue(cruValue);
						}
						i = Integer.valueOf(i.intValue() + 1);
						if (!gc1.getTime().after(gcYear.getTime())) {
							...
							if (deprePeriod.intValue() - j.intValue() > 0) {
								if (flag.intValue() == 0) {
									twoYearValue = cruValue.subtract(cruValue.multiply(fixedAssets
											.getRemainValRate().divide(new BigDecimal(100), 2, 2)));
								}
								...
								if (fixedAssets.getIntendTerm().intValue() > 1) {
									bd = bd.add(twoYearValue.divide(new BigDecimal(24), 2, 3)
											.multiply(new BigDecimal(w.toString())));
									cruValue = cruValue.subtract(twoYearValue.divide(
											new BigDecimal(24), 2, 3).multiply(
											new BigDecimal(w.toString())));
								} else {
									bd = bd.add(twoYearValue.divide(new BigDecimal(12), 2, 3)
											.multiply(new BigDecimal(w.toString())));
									cruValue = cruValue.subtract(twoYearValue.divide(
					...
					BigDecimal total = intendTerm.multiply(intendTerm.add(new BigDecimal(1)))
					...
					BigDecimal stValue = fixedAssets.getAssetValue().multiply(
							new BigDecimal(1).subtract(fixedAssets.getRemainValRate().divide(
					...
					while (deprePeriod.intValue() > 0) {
						...
						BigDecimal rate = intendTerm.subtract(new BigDecimal(be.intValue()))
								.divide(total, 2, 2);
						if (jian.intValue() == 0) {
							depValue = stValue.multiply(rate).multiply(
									new BigDecimal(deprePeriod.intValue()).divide(
							...
							BigDecimal rateLast = intendTerm.subtract(
									new BigDecimal(beLast.intValue())).divide(total, 2, 2);
							Integer months = Integer.valueOf(YEARS - gcLast.get(2));
							depValue = stValue.multiply(rateLast).multiply(
									new BigDecimal(months.intValue()).divide(new BigDecimal(12), 2,
											2));

							Integer monthsNextYear = Integer.valueOf(gc1.get(2) + 1);
							depValue = depValue.add(stValue.multiply(rate).multiply(
									new BigDecimal(monthsNextYear.intValue()).divide(
											new BigDecimal(12), 2, 2)));
							cruValue = cruValue.subtract(depValue);
						}
						DepreRecord depreR = new DepreRecord();
	...
	public String work() {
		...
		if (StringUtils.isNotEmpty(id)) {
			...
			FixedAssets fixedAssets = (FixedAssets) this.fixedAssetsService.get(assetsId);
			...
			if (lastCalTime != null) {
				...
				if (cruCalTime.before(new Date())) {
					...
					setJsonString("{success:true,lastCalTime:'" + strLastDate + "',cruCalTime:'"
							+ strCruDate + "',workGrossUnit:'" + unit + "',defPerWorkGross:'"
