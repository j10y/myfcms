...
public class BookTypeAction extends BaseAction {
	...
	public String list() {
		...
		List list = this.bookTypeService.getAll(filter);
	...
	public String tree() {
		...
		for (BookType bookType : list) {
			sb.append("{id:'" + bookType.getTypeId() + "',text:'" + bookType.getTypeName()
					+ "',leaf:true},");
		}
		if (list.size() > 0) {
	...
	public String remove() {
		...
		setBookType((BookType) this.bookTypeService.get(typeId));
	...
	public String get() {
		BookType bookType = (BookType) this.bookTypeService.get(this.typeId);
		Gson gson = new Gson();
