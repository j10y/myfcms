...
public class DutyAction extends BaseAction {
	...
	public String save() {
			...
			for (int i = 0; i < uIds.length; ++i) {
				...
				try {
					...
					if (uDuty.getDutyId() == null) {
						isExist = this.dutyService.isExistDutyForUser(user.getUserId(), uDuty
								.getStartTime(), uDuty.getEndTime());
					}
					if (isExist) {
						isExcept = true;
		...
		if (isExcept) {
			...
			ssb.append("在该时间(").append(sdf.format(this.duty.getStartTime())).append("至").append(
					sdf.format(this.duty.getEndTime())).append(")内已经存在班制!");
		}
