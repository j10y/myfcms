...
public class DutyRegisterAction extends BaseAction {
	...
	public String today() {
		...
		Duty duty = this.dutyService.getCurUserDuty(ContextUtil.getCurrentUserId());
		...
		if (dutySystem == null) {
			...
			for (int i = 0; i < curDaySections.length; ++i) {
				...
				DutySection dutySection = (DutySection) this.dutySectionService.get(new Long(
						curDaySections[i]));

				DutyRegister signInReg = this.dutyRegisterService.getTodayUserRegister(curUser
						.getUserId(), DutyRegister.SIGN_IN, dutySection.getSectionId());
				DutyRegister signOffReg = this.dutyRegisterService.getTodayUserRegister(curUser
						.getUserId(), DutyRegister.SIGN_OFF, dutySection.getSectionId());
				if (i > 0)
					sb.append(",");
				sb.append("{sectionId:'").append(dutySection.getSectionId()).append(
						"',systemName:'").append(dutySection.getSectionName()).append(
						"',startSignin:'").append(dutySection.getStartSignin1()).append(
						"',dutyStartTime:'").append(dutySection.getDutyStartTime1()).append(
						"',endSignin:'").append(dutySection.getEndSignin1()).append(
						"',earlyOffTime:'").append(dutySection.getEarlyOffTime1()).append(
						"',dutyEndTime:'").append(dutySection.getDutyEndTime1()).append(
						"',signOutTime:'").append(dutySection.getSignOutTime1()).append("'");

				if (signInReg != null) {
					sb.append(",signInTime:'").append(signInReg.getRegisterTime()).append(
							"',signInFlag:'").append(signInReg.getRegFlag()).append(
							"',allowSignIn:'0'");
				} else {
				...
				if (signOffReg != null) {
					sb.append(",signOffTime:'").append(signOffReg.getRegisterTime()).append(
							"',signOffFlag:'").append(signOffReg.getRegFlag()).append(
							"',allowSignOff:'0'");
				} else {
	...
	public String signIn() {
		...
		this.dutyRegisterService.signInOff(new Long(sectionId), DutyRegister.SIGN_IN, ContextUtil
	...
	public String signOff() {
		String sectionId = getRequest().getParameter("sectionId");
		this.dutyRegisterService.signInOff(new Long(sectionId), DutyRegister.SIGN_OFF, ContextUtil
				.getCurrentUser(), new Date());
		this.jsonString = "{success:true}";
		return "success";
	}
	...
	public String get() {
		DutyRegister dutyRegister = (DutyRegister) this.dutyRegisterService.get(this.registerId);
	...
	public String save() {
		...
		for (int i = 0; i < uIds.length; ++i) {
			AppUser appUser = (AppUser) this.appUserService.get(new Long(uIds[i]));
			this.dutyRegisterService.signInOff(sectionId, inOffFlag, appUser, registerDate);
		}
