 ...
 public class SysConfigAction extends BaseAction
 {
   ...
   public String load() {
     ...
     while (it.hasNext()) {
       ...
       List list = (List)type.getValue();
       if (typeName.equals("验证码配置")) {
         for (SysConfig con : list)
           buff.append("{xtype:'container',style:'padding-bottom:3px;',layout:'column',items:[{xtype:'label',style:'font-weight:bold;',text:'")
             .append(con.getConfigName())
             .append(":',width:100},{xtype:'combo',mode:'local',editable:false,triggerAction:'all',store:[['1','开启验证码'],['2','屏蔽验证码']],width:300,allowBlank:false,hiddenName:'")
             .append(con.getConfigKey())
             .append("',value:'")
             .append(con.getDataValue())
             .append("'},{xtype:'label',width:200,text:'")
             .append(con.getConfigDesc())
             .append("'}]},");
       }
       else {
         for (SysConfig con : list) {
           buff.append("{xtype:'container',style:'padding-bottom:3px;',layout:'column',items:[{xtype:'label',style:'font-weight:bold;',text:'")
             .append(con.getConfigName())
             .append(":',width:100},{xtype:'textfield',width:300,allowBlank:false,id:'")
             .append(con.getConfigKey())
             .append("',name:'")
             .append(con.getConfigKey())
             .append("',value:'")
