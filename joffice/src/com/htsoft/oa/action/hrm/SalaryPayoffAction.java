...
public class SalaryPayoffAction extends BaseAction {
	...
	public String list() {
		...
		List list = this.salaryPayoffService.getAll(filter);
	...
	public String get() {
		SalaryPayoff salaryPayoff = (SalaryPayoff) this.salaryPayoffService.get(this.recordId);
	...
	public String save() {
		...
		BigDecimal acutalAmount = this.salaryPayoff.getStandAmount().add(
				this.salaryPayoff.getEncourageAmount()).subtract(
				this.salaryPayoff.getDeductAmount());
	...
	public String check() {
		SalaryPayoff checkSalaryPayoff = (SalaryPayoff) this.salaryPayoffService.get(new Long(
	...
	public String personal() {
		...
		for (SalaryPayoff salaryDetail : list) {
			buff.append("{recordId:'").append(salaryDetail.getRecordId()).append("',fullname:'")
					.append(salaryDetail.getFullname()).append("',profileNo:'").append(
							salaryDetail.getProfileNo()).append("',idNo:'").append(
							salaryDetail.getIdNo()).append("',standAmount:'").append(
							salaryDetail.getStandAmount()).append("',acutalAmount:'").append(
							salaryDetail.getAcutalAmount()).append("',startTime:'").append(
							salaryDetail.getStartTime()).append("',endTime:'").append(
							salaryDetail.getEndTime()).append("',checkStatus:'").append(
							salaryDetail.getCheckStatus());
			...
			if ((salaryDetail.getEncourageAmount() != new BigDecimal(0))
				...
				content.append("<th>").append("奖励金额</th><td>").append(
						salaryDetail.getEncourageAmount()).append("</td>");
			}

			if ((salaryDetail.getEncourageAmount() != new BigDecimal(0))
					&& (salaryDetail.getEncourageAmount() != null)) {
				content.append("<th>").append("扣除金额</th><td>").append(
						salaryDetail.getDeductAmount()).append("</td>");
			}

			if ((salaryDetail.getEncourageAmount() != new BigDecimal(0))
					&& (salaryDetail.getEncourageAmount() != null)) {
				content.append("<th>").append("效绩金额</th><td>").append(
						salaryDetail.getAchieveAmount()).append("</td>");
			}
			content
