...
public class MailAction extends BaseAction {
	...
	public String list() {
		...
		List<MailBox> list = this.mailBoxService.getAll(filter);
		...
		for (MailBox mailBoxTemp : list) {
			...
			buff.append("{boxId:'").append(mailBoxTemp.getBoxId()).append("',sendTime:'").append(
					mailBoxTemp.getSendTime()).append("',delFlag:'").append(
					mailBoxTemp.getDelFlag()).append("',readFlag:'").append(
					mailBoxTemp.getReadFlag()).append("',replyFlag:'").append(
					mailBoxTemp.getReplyFlag()).append("',mailId:'").append(mailTemp.getMailId())
					.append("',importantFlag:'").append(mailTemp.getImportantFlag()).append(
							"',mailStatus:'").append(mailTemp.getMailStatus())
					.append("',fileIds:'").append(mailTemp.getFileIds()).append("',subject:'")
					.append(gson.toJson(mailTemp.getSubject()).replace("\"", "")).append(
							"',recipientNames:'").append(mailTemp.getRecipientNames()).append(
							"',sender:'").append(mailTemp.getSender()).append("',content:'");
	...
	public String multiDel() {
		MailFolder deleteFolder = (MailFolder) this.mailFolderService.get(Long
	...
	public String get() {
		...
		if ((opt == null) || ("".equals(opt))) {
			...
			if (opt.equals("_next")) {
				...
				if (filter.getPagingBean().getStart().intValue() + 1 == filter.getPagingBean()
						.getTotalItems())
		...
		if (this.mail.getMailStatus().shortValue() != 1) {
			...
			sb.append(serializer.exclude(
					new String[] { "class", "mail.appUser", "appUser.department",
							"mailFolder.appUser" }).serialize(this.mail));
			sb.append("]}");
	...
	public String save() {
		if (mail.getMailStatus().shortValue() == IS_MAIL) {
			...
			MailFolder sendFolder = (MailFolder) mailFolderService
			...
			if (mail.getMailId() == null) {
				...
				try {
					...
					for (int l = 0; l < i1; l++) {
						String id = as1[l];
						if (!id.equals(""))
							mailAttachs.add((FileAttach) fileAttachService.get(new Long(id)));
					}
			...
			if (sendMessage != null && sendMessage.equals("on")) {
				...
				msgContent.append(mail.getSender()).append("</font>").append(
						"在<font color=\"red\">").append(sdf.format(mail.getSendTime())).append(
						"</font>").append("给您发了一封邮件，请注意查收。");
				shortMessageService.save(AppUser.SYSTEM_USER, mail.getRecipientIDs(), msgContent
						.toString(), ShortMessage.MSG_TYPE_SYS);
			}
			String as[];
			...
			for (int i = 0; i < k; i++) {
				...
				if (!"".equals(id)) {
					...
					mailBoxSend.setAppUser((AppUser) appUserService.get(new Long(id)));
			...
			for (int j = 0; j < k; j++) {
				...
				if (!"".equals(id)) {
					...
					mailBoxCopy.setAppUser((AppUser) appUserService.get(new Long(id)));
	...
	public String opt() {
		...
		newSubject.append("<br><strong>收件人</strong>:" + this.mail.getRecipientNames());
		...
		newSubject.append("<br><strong>内容</strong>:<br><br>" + this.mail.getContent());
		...
		if (opt.equals("回复")) {
			...
			reply.setRecipientIDs((new StringBuilder()).append(mail.getAppSender().getUserId())
					.toString());
		...
		sb.append(serializer.exclude(new String[] { "class", "appUser" }).serialize(reply));
		sb.append("]}");
		setJsonString(sb.toString());
		return "success";
	}
	...
	public String move() {
		MailFolder moveToFolder = (MailFolder) this.mailFolderService.get(new Long(this.folderId
				.longValue()));
	...
	public String search() {
		...
		List<MailBox> list = this.mailBoxService.findBySearch(searchContent, pb);
		Gson gson = new Gson();
		...
		for (MailBox mailBoxTemp : list) {
			...
			buff.append("{boxId:'").append(mailBoxTemp.getBoxId()).append("',sendTime:'").append(
					...
					.append("',importantFlag:'").append(mailTemp.getImportantFlag()).append(
							"',mailStatus:'").append(mailTemp.getMailStatus())
					.append("',fileIds:'").append(mailTemp.getFileIds()).append("',subject:'")
					.append(gson.toJson(mailTemp.getSubject())).append("',recipientNames:'")
					.append(mailTemp.getRecipientNames()).append("',sender:'").append(
							mailTemp.getSender()).append("',content:'");
			String content = StringUtil.html2Text(mailTemp.getContent());
