   ...
   implements CustomerDao
 {
   ...
   public boolean checkCustomerNo(String customerNo)
   {
     ...
     Long count = (Long)getHibernateTemplate().execute(new HibernateCallback(customerNo)
     {
       public Object doInHibernate(Session session)
         throws HibernateException, SQLException
       {
         Query query = session.createQuery("select count(*) from Customer c where c.customerNo = ?");
         query.setString(0, this.val$customerNo);
