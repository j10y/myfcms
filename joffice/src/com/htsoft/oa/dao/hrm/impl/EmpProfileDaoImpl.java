   ...
   implements EmpProfileDao
 {
   ...
   public boolean checkProfileNo(String profileNo)
   {
     ...
     Long count = (Long)getHibernateTemplate().execute(new HibernateCallback(profileNo)
     {
       public Object doInHibernate(Session session) throws HibernateException, SQLException
       {
         Query query = session.createQuery("select count(*) from EmpProfile ep where ep.profileNo = ?");
         query.setString(0, this.val$profileNo);
