...
public class DutyRegisterServiceImpl extends BaseServiceImpl<DutyRegister> implements
		DutyRegisterService {
	...
	public void signInOff(Long sectionId, Short signInOff, AppUser curUser, Date registerDate) {
		...
		DutyRegister dutyRegister = this.dao.getTodayUserRegister(curUser.getUserId(), signInOff,
				sectionId);
		...
		if (DutyRegister.SIGN_IN.equals(signInOff)) {
			if (regiserCal.compareTo(startCalendar) > 0) {
				...
				Long minis = Long.valueOf((regiserCal.getTimeInMillis() - startCalendar
			...
			Long minis = Long.valueOf((startCalendar.getTimeInMillis() - regiserCal
					.getTimeInMillis()) / 60000L);
			register.setRegMins(Integer.valueOf(minis.intValue()));
		} else {
			register.setRegFlag(DutyRegister.REG_FLAG_NORMAL);
