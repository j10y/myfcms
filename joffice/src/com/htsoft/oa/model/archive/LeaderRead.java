...
public class LeaderRead extends BaseModel {
	...
	public boolean equals(Object object) {
		...
		return new EqualsBuilder().append(this.readId, rhs.readId).append(this.leaderName,
				rhs.leaderName).append(this.userId, rhs.userId).append(this.leaderOpinion,
				rhs.leaderOpinion).append(this.createtime, rhs.createtime).append(this.depLevel,
				rhs.depLevel).append(this.depName, rhs.depName).append(this.isPass, rhs.isPass)
				.append(this.checkName, rhs.checkName).isEquals();
	}

	public int hashCode() {
		return new HashCodeBuilder(-82280557, -700257973).append(this.readId).append(
				this.leaderName).append(this.userId).append(this.leaderOpinion).append(
				this.createtime).append(this.depLevel).append(this.depName).append(this.isPass)
				.append(this.checkName).toHashCode();
	}

	public String toString() {
		return new ToStringBuilder(this).append("readId", this.readId).append("leaderName",
				this.leaderName).append("userId", this.userId).append("leaderOpinion",
				this.leaderOpinion).append("createtime", this.createtime).append("depLevel",
				this.depLevel).append("depName", this.depName).append("isPass", this.isPass)
				.append("checkName", this.checkName).toString();
	}
}
 ...
 * Qualified Name: com.htsoft.oa.model.archive.LeaderRead JD-Core Version: 0.5.4
 */