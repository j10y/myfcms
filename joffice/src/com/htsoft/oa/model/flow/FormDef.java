...
public class FormDef extends BaseModel {
	...
	public boolean equals(Object object) {
		...
		return new EqualsBuilder().append(this.formDefId, rhs.formDefId).append(this.formName,
				rhs.formName).append(this.columns, rhs.columns).append(this.isEnabled,
				rhs.isEnabled).append(this.activityName, rhs.activityName).append(this.deployId,
				rhs.deployId).isEquals();
	}

	public int hashCode() {
		return new HashCodeBuilder(-82280557, -700257973).append(this.formDefId).append(
				this.formName).append(this.columns).append(this.isEnabled)
				.append(this.activityName).append(this.deployId).toHashCode();
	}

	public String toString() {
		return new ToStringBuilder(this).append("formDefId", this.formDefId).append("formName",
				this.formName).append("columns", this.columns).append("isEnabled", this.isEnabled)
				.append("activityName", this.activityName).append("deployId", this.deployId)
				.toString();
	}
}
