...
public class DutyRegister extends BaseModel {
	...
	public boolean equals(Object object) {
		...
		return new EqualsBuilder().append(this.registerId, rhs.registerId).append(
				this.registerDate, rhs.registerDate).append(this.regFlag, rhs.regFlag).append(
				this.regMins, rhs.regMins).append(this.reason, rhs.reason).append(this.dayOfWeek,
				rhs.dayOfWeek).append(this.inOffFlag, rhs.inOffFlag).isEquals();
	}

	public int hashCode() {
		return new HashCodeBuilder(-82280557, -700257973).append(this.registerId).append(
				this.registerDate).append(this.regFlag).append(this.regMins).append(this.reason)
				.append(this.dayOfWeek).append(this.inOffFlag).toHashCode();
	}

	public String toString() {
		return new ToStringBuilder(this).append("registerId", this.registerId).append(
				"registerDate", this.registerDate).append("regFlag", this.regFlag).append(
				"regMins", this.regMins).append("reason", this.reason).append("dayOfWeek",
				this.dayOfWeek).append("inOffFlag", this.inOffFlag).toString();
	}
