...
public class DutySection extends BaseModel {
	...
	public void setStartSignin1(String inVal) {
		...
		try {
			this.startSignin = DateUtils
					.parseDate(finalVal, new String[] { "yyyy-MM-dd HH:mm:ss" });
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	}
	...
	public void setDutyStartTime1(String inVal) {
		...
		String finalVal = "1900-01-01 " + inVal + ":00";
		try {
			this.dutyStartTime = DateUtils.parseDate(finalVal,
	...
	public void setEarlyOffTime1(String inVal) {
		...
		String finalVal = "1900-01-01 " + inVal + ":00";
		try {
			this.earlyOffTime = DateUtils.parseDate(finalVal,
					new String[] { "yyyy-MM-dd HH:mm:ss" });
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public boolean equals(Object object) {
		...
		return new EqualsBuilder().append(this.sectionId, rhs.sectionId).append(this.startSignin,
				rhs.startSignin).append(this.dutyStartTime, rhs.dutyStartTime).append(
				this.endSignin, rhs.endSignin).append(this.earlyOffTime, rhs.earlyOffTime).append(
				this.dutyEndTime, rhs.dutyEndTime).append(this.signOutTime, rhs.signOutTime)
				.isEquals();
	}

	public int hashCode() {
		return new HashCodeBuilder(-82280557, -700257973).append(this.sectionId).append(
				this.startSignin).append(this.dutyStartTime).append(this.endSignin).append(
				this.earlyOffTime).append(this.dutyEndTime).append(this.signOutTime).toHashCode();
	}

	public String toString() {
		return new ToStringBuilder(this).append("sectionId", this.sectionId).append("startSignin",
				this.startSignin).append("dutyStartTime", this.dutyStartTime).append("endSignin",
				this.endSignin).append("earlyOffTime", this.earlyOffTime).append("dutyEndTime",
				this.dutyEndTime).append("signOutTime", this.signOutTime).toString();
	}
}
