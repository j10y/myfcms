			...
			<tr>
				...
				<table class="queryMiddleTable" width="95%" border="0"
					...
					<tr class="queryMiddleRow">
						<td width="47%" class="queryMiddleCell">
						<input type="button" class="button_addChildNode" name="Submit2" onclick="addChild();">
						<input type="button" class="button_addBrotherNode" name="Submit3" onclick="addBrother();"> 
						<input type="button" class="button_deleteNode" name="Submit4" onclick="delNode();">
						<input type="button" class="button_editNode" name="Submit4" onclick="updateNode();"></td>
					</tr>
				</table>
				<table width="97%" border="0" cellspacing="0" cellpadding="0">
					<tr>
...
tree.action = "JavaScript:putId('', '', '0', '权限', '');";
var treeItems = new Array;
<c:forEach items="${privileges}" var="privilege" varStatus="status">
   treeItems[<c:out value="${status.count - 1}"/>] = new appDictTreeItem("<c:out value="${privilege.privName}"/>","","<c:out value="${privilege.id}"/>","<c:out value="${privilege.parent.id}"/>","","","");
...
function addChild(){
	...
	if(node!=null){		
		if(node==tree){
			...
			window.location.href='<c:url value="/treeEdit.do?parentId='+node.uParentId+'&itemId='+node.uItemId+'&isBrother=0&referenceNodeId='+node.uItemId+'&category='+node.uCategory+'&editFlag=add&returnUrl=${requestUrl}"/>';
	    }	       
	}else{
	...
	if(node==null){
		...
		if(node==tree){
			...
			if(node.uItemId==""){
			    ...
			    window.location.href='<c:url value="/treeEdit.do?parentId='+node.uParentId+'&itemId='+node.uItemId+'&isBrother=1&referenceNodeId='+node.uItemId+'&category='+node.uCategory+'&editFlag=add&returnUrl=${requestUrl}"/>';
			}
		}
	}     
}
...
function updateNode(){
	...
	if(node==null){
		...
		if(node==tree){
			...
			window.location.href='<c:url value="/treeEdit.do?parentId='+node.uParentId+'&itemId='+node.uItemId+'&referenceNodeId='+node.uItemId+'&editFlag=modify&returnUrl=${requestUrl}"/>';
		}
	}
}
function updateOrg(){
	...
	if(node==null){
		...
		if(node==tree){
			...
			window.location.href='<c:url value="/treeOrg.do?treeId='+node.uItemId+'&editFlag=modify&returnUrl=${requestUrl}"/>';
		}
	}
}
</script></td>
