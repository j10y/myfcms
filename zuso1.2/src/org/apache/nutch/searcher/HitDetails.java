...
public final class HitDetails implements Writable {
	...
	public String[] getValues(String field) {
		...
		return (vals.size() > 0) ? vals.toArray(new String[vals.size()]) : null;
	}
	...
	public String getColValue(String type, Query query) {
		String title = getValue(type);
		
		for (int i = 0; i < query.getClauses().length; i++) {
			String clauses = query.getClauses()[i].toString();
			String[] clause = clauses.replace("\"", "").split(" ");
			for (int j = 0; j < clause.length; j++) {
				title = title.replace(clause[j].toUpperCase(), "<Strong><font color='#ff0000'>" + clause[j].toUpperCase() + "</font></Strong>");
			}
		}
		return title;
