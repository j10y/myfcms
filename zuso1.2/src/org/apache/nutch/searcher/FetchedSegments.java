...
/**
 * Implements {@link HitSummarizer} and {@link HitContent} for a set of fetched
 * segments.
 */
public class FetchedSegments implements RPCSegmentBean {
	...
	private class SegmentUpdater extends Thread {
		...
		public void run() {
			...
			while (!stopRequested && !Thread.currentThread().isInterrupted()) {
				try {
					final FileStatus[] fstats = fs.listStatus(segmentsDir, HadoopFSUtil.getPassDirectoriesFilter(fs));
					...
					while (i.hasNext()) {
						...
						if (!fs.exists(seg.segmentDir)) {
							try {
								...
								/*
								 * A segment may fail to close since it may
								 * already be deleted from file system. So we
								 * just ignore the exception and remove the
								 * mapping from 'segments'.
								 */
							} finally {
								i.remove();
							}
					...
					if (segmentDirs != null) {
						for (final Path segmentDir : segmentDirs) {
							segments.putIfAbsent(segmentDir.getName(), new Segment(fs, segmentDir, conf));
						}
					}
	...
	public long getFetchDate(HitDetails details) throws IOException {
		return getSegment(details).getCrawlDatum(getUrl(details)).getFetchTime();
	}
