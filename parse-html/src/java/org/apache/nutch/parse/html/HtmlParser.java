...
public class HtmlParser implements Parser {
	...
	private static Pattern metaPattern = Pattern
			.compile("<meta\\s+([^>]*http-equiv=(\"|')?content-type(\"|')?[^>]*)>",
	 ...
	 * from the first <code>CHUNK_SIZE</code> bytes. If there's no meta tag
	 * for Content-Type or no charset is specified, <code>null</code> is
	 * returned. <br />
	...
	public ParseResult getParse(Content content) {
		...
		ParseData parseData = new ParseData(status, title, outlinks, content.getMetadata(),
				metadata);
		ParseResult parseResult = ParseResult.createParseResult(content.getUrl(), new ParseImpl(
				text, parseData));

		// run filters on parse
		ParseResult filteredParse = this.htmlParseFilters.filter(content, parseResult, metaTags,
				root);
	...
	private DocumentFragment parseNeko(InputSource input) throws Exception {
		...
		try {
			parser.setFeature(
					"http://cyberneko.org/html/features/scanner/allow-selfclosing-iframe", true);
			...
			parser.setFeature("http://cyberneko.org/html/features/balance-tags/document-fragment",
					true);
			parser.setFeature("http://cyberneko.org/html/features/report-errors", LOG
					.isTraceEnabled());
		} catch (SAXException e) {
		}
