...
<script type="text/javascript">
	...
	function applyDel(ID) {
		var dc = new DataCollection();
			dc.add("ID",ID);
			dc.add("SiteID",$V("SiteID"));
	   		Server.sendRequest("com.zving.bbs.admin.PostAudit.applyDel",dc,function(response){
	   			Dialog.alert(response.Message);
				if(response.Status==1 || response.Status==2){
					DataList.loadData("list1");
				}
	...
	function editPost(ID){
		...
		window.location = "./EditPanelPost.jsp?ID="+ID;
	}
...
<input type="hidden" value="${SiteID}" id="SiteID">
  <tr valign="top">
    ...
    <h4>我发表的文章</h4>
    <ul class="tabs">
	<li><a href="MyArticles.jsp?SiteID=${SiteID}">我的主题</a></li>
    ...
    <form method="post" name="moderate">
      <table width="100%" cellpadding="0" cellspacing="0" bordercolor="#eeeeee" class="forumTable">
        ...
        <z:datalist id="list1" size="10" method="com.zving.bbs.MyPost.getMyPost">
        <tbody>
          <tr>
            <td>${Name}</td>
            <td><a href="./Post.jsp?ThemeID=${ThemeID}&ForumID=${ForumID}&SiteID=${SiteID}&User=reply" >${TSubject}</a></td>
            <td ><span id="thread_6417">${Subject}</span></td>
            <td><em>${AddTime}</em></td>
            <td>${AuditStatus}</td>
            <td>${Operation}</td>
          </tr>
