...
public class PreParser {
	...
	private boolean parseAD() {
		...
		while (m.find(lastEndIndex)) {
			...
			int NameCount = new QueryBuilder(
					"select count(*) from zcadposition where PositionName = ? and siteid=?", name,
					this.siteID).executeInt();
			if (NameCount > 0) {
				continue;
			}
			ad.setID(NoUtil.getMaxID("AdPositionID"));
			ad.setCode(String.valueOf(ad.getID()));
	...
	private boolean parseImagePlayer() {
		...
		while (m.find(lastEndIndex)) {
			...
			int NameCount = new QueryBuilder(
					"select count(*) from ZCImagePlayer where name = ? and siteid=?", name,
					this.siteID).executeInt();
			int CodeCount = new QueryBuilder(
					"select count(*) from ZCImagePlayer where code = ? and siteid=?", code,
					this.siteID).executeInt();
			if (NameCount > 0)
				continue;
			if (CodeCount > 0) {
 ...
 * com.zving.cms.template.PreParser JD-Core Version: 0.5.3
 */