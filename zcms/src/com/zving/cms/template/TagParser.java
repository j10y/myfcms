 ...
 public class TagParser
 {
   ...
   final Pattern resourcePatternCss2 = Pattern.compile("(url)\\s*?\\(\\s*?(\\\"|\\')(.*?)\\2(\\))", 
     34);
   ...
   private String processText(String srcType, String resourcePath, String levelStr)
   {
     ...
     if (("#".equals(resourcePath)) || ("#;".equals(resourcePath))) {
       ...
       (resourcePath.toLowerCase().indexOf("<%") != -1) || (resourcePath.toLowerCase().indexOf("+") != -1) || 
     ...
     while (m.find(lastEndIndex)) {
       ...
       else
       {
         ...
         if (("article".equalsIgnoreCase(item)) || ("image".equalsIgnoreCase(item)) || 
           ...
           ("attachment".equalsIgnoreCase(item)) || ("goods".equalsIgnoreCase(item))) {
           ...
           jspContent = "<!--循环：" + item + " count：" + count + "-->\n" + "<% DataTable dt_" + varIndex + 
             " = TemplateData.getDocList(\"" + item + "\"," + catalogStr + ",\"" + this.displayLevel + 
             "\",\"" + attribute + "\",\"" + type + "\",\"" + count + "\"," + conditionStr + ");\n" + 
             "if(dt_" + varIndex + "==null){write(\"没有找到栏目。parent：" + map.get("parent") + "  catalog：" + 
           ...
           if (!(this.isPageBlock)) {
             ...
             else sb.append(getListBlock(item, count, type, catalog, parent, jspContent));
           }
           else
             sb.append(jspContent);
         }
   ...
   private String getListBlock(String item, int count, String type, String catalog, String parent, String jspContent) {
     ...
     if (catalogID != null) {
       ...
       String code = catalogID + "_" + item + "_" + type + "_" + count + "_" + hex + "_" + this.dirLevel;
       String cacheFileName = "/cache/template/block_" + code + ".html";
       String targetFileName = "cache/page/block_" + code + ".html";
   ...
   private void parseTree() {
     ...
     while (m.find(lastEndIndex)) {
       ...
       String jspContent = "<%write(TemplateData.getTree(\"" + id + "\",\"" + method + "\",\"" + tagBody + 
         "\", \"" + style + "\",\"" + levelStr + "\"));%>";
       sb.append(jspContent);
     }
     sb.append(this.content.substring(lastEndIndex));
   ...
   private void parseVote() {
     ...
     while (m.find(lastEndIndex)) {
       ...
       if (voteBodyContent.indexOf("cms:list") > 0) {
         ...
         String jspContent = "<%DataRow " + voteVar + " = TemplateData.getVoteData(\"" + name + "\",\"" + id + "\");\n" + 
           "if(" + voteVar + "==null){\n write(\"没有找到对应的投票主题:" + name + "\");\n}else{\n%>";
         sb.append(jspContent);
 
         voteBodyContent = "<div id='vote_${vote.id}' class='votecontainer' style='text-align:left'><form id='voteForm_${vote.id}' name='voteForm_${vote.id}' action='" + Config.getValue("ServicesContext") + "/VoteResult.jsp' method='post' target='_blank'><input type='hidden' id='ID' name='ID' value='${vote.id}'><input type='hidden' id='VoteFlag' name='VoteFlag' value='Y'>" + 
           voteBodyContent + "<input type='submit' value='提交' onclick='return checkVote(${vote.id});'>&nbsp;&nbsp<input type='button' value='查看' onclick='javascript:window.open(\"" + Config.getValue("ServicesContext") + "/VoteResult.jsp?ID=${vote.id}\",\"VoteResult\")'></form></div>";
   ...
   private void parseInclude() {
     ...
     while (m.find(lastEndIndex)) {
       ...
       if (StringUtil.isEmpty(fileName)) {
         ...
         String fileLevelName = fileName.substring(0, fileName.lastIndexOf(46)) + "_" + this.dirLevel + 
           fileName.substring(fileName.lastIndexOf(46));
   ...
   private void parseLinkURL() {
     ...
     while (m.find(lastEndIndex)) {
       ...
       String jspContent = "<%write(TemplateData.getLinkURL(\"" + type + "\",\"" + code + "\",\"" + name + 
         "\",\"" + spliter + "\"));%>";
       sb.append(jspContent);
     }
     sb.append(this.content.substring(lastEndIndex));
   ...
   private void parsePage()
   {
     ...
     while (m.find(lastEndIndex)) {
       ...
       String jspContent = "<%write(TemplateData.getPage(\"" + type + "\",\"" + value + "\",\"" + name + 
         "\",\"" + target + "\"));%>";
       sb.append(jspContent);
     }
     sb.append(this.content.substring(lastEndIndex));
     ...
     while (m.find(lastEndIndex)) {
       ...
       if (StringUtil.isEmpty(name)) {
         ...
         if (("article".equalsIgnoreCase(type)) || ("image".equalsIgnoreCase(type)) || 
           ...
           jspContent = "<% DataRow " + newVar + "=TemplateData.getDocument(\"" + type + "\",\"" + name + 
             "\"); if(" + newVar + "==null){write(\"没有找到对象" + name + "+\");}else{%>";
         else if ("catalog".equalsIgnoreCase(type)) {
           jspContent = "<% DataRow " + newVar + "=TemplateData.getCatalog(\"" + type + "\",\"" + name + 
             "\"); if(" + newVar + "==null){write(\"没有找到对象" + name + "+\");}else{%>";
         }
   ...
   private void dealBlock() {
     ...
     TagParser tmp50_49 = this; tmp50_49.templateFileName = tmp50_49.templateFileName + "_" + this.dirLevel;
     this.trans.add(
       new QueryBuilder("delete from zcpageblock where exists (select blockid from  ZCTemplateBlockRela where filename='" + 
       this.templateFileName + "' and blockid=zcpageblock.id)"));
     this.trans.add(new QueryBuilder("delete  from  ZCTemplateBlockRela where filename='" + this.templateFileName + "'"));
 ...
 * Qualified Name:     com.zving.cms.template.TagParser
