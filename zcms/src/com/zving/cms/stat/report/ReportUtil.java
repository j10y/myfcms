...
public class ReportUtil {
	...
	public static int getTotalStatDays(long siteID) {
		int c = new Double(Math.ceil((System.currentTimeMillis() - getStartStatDate(siteID)
				.getTime()) * 1.0D / 86400000.0D)).intValue();
		if (c == 0) {
			c = 1;
		}
		return c;
	...
	public static Date getStartStatDate(long siteID) {
		...
		if (date == null) {
			...
			for (int i = 5; i < dt.getColCount(); ++i) {
				if (dt.getInt(0, i) != 0) {
					i -= 4;
					date = DateUtil.parse(dt.getString(0, "Period")
							+ ((i > 9) ? i : new StringBuffer("0").append(i).toString()),
							"yyyyMMdd");
	...
	public static DataTable toDateTable(DataTable src, Date start, Date end) {
		...
		for (int i = 0; i < src.getRowCount(); ++i) {
			...
			if (period.equals(period1)) {
				...
				for (int j = colStart; j >= 4 + day1; --j) {
					int d = j - 4;
					String key = period
							+ ((d > 9) ? d : new StringBuffer("0").append(d).toString());
		...
		for (int i = 0; i < ks.length; ++i) {
			...
			dt.set(i, "Date", date.substring(0, 4) + "-" + date.substring(4, 6) + "-"
					+ date.substring(6));
 ...
 * com.zving.cms.stat.report.ReportUtil JD-Core Version: 0.5.3
 */