 ...
 {
   public static void vote(HttpServletRequest request, HttpServletResponse response)
   {
     ...
     if ("Y".equals(voteFlag)) {
       ...
       if (now.before(vote.getStartTime())) {
         try {
           response.getWriter().write("<script language='javascript' >alert('对不起，此调查还没有开始！开始时间为：" + vote.getStartTime() + "，请您到时候再来投票');</script>");
         } catch (IOException e1) {
           e1.printStackTrace();
         }
         flag = false;
       ...
       if ((flag) && ("Y".equals(vote.getIPLimit()))) {
         int count = new QueryBuilder("select count(*) from zcvotelog where IP = ? and voteID = ?", IP, ID).executeInt();
         if (count > 0) {
           try {
             response.getWriter().write("<script language='javascript' >alert('您已经投过票了，所以本次投票无效！');</script>");
           } catch (IOException e1) {
             e1.printStackTrace();
           }
           flag = false;
       ...
       if (flag) {
         ...
         trans.add(new QueryBuilder("update zcvoteitem set score=score+1 where id in (" + itemIDsb.deleteCharAt(itemIDsb.length() - 1) + ")"));
         ...
         if (trans.commit())
           try {
             response.getWriter().write("<script language='javascript' >alert('投票成功，谢谢您的关注！');</script>");
           } catch (IOException e1) {
             e1.printStackTrace();
           }
         else
 ...
 * Qualified Name:     com.zving.cms.stat.report.VoteResult
