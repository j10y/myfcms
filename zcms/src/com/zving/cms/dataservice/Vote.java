 ...
 public class Vote extends Page
 {
   ...
   public static Mapx JSCodeDialog(Mapx params) {
     ...
     JSCode = JSCode + "<script language='javascript' src='" + new StringBuffer(String.valueOf(Config.getContextPath())).append(Config.getValue("Statical.TargetDir")).append("/").append(Application.getCurrentSiteAlias()).append("/js/vote_").append(vote.getID()).toString().replaceAll("//", "/") + ".js'></script>";
     JSCode = JSCode + "\n</div>";
   ...
   public static void dg1DataBind(DataGridAction dga) {
     ...
     if (dga.getTotal() == 0) {
       dga.setTotal(new QueryBuilder("select count(*) from ZCVote where SiteID = ?", Application.getCurrentSiteID()));
     }
   ...
   public static void dg2DataBind(DataGridAction dga)
   {
     ...
     if (dga.getTotal() == 0) {
       dga.setTotal(new QueryBuilder("select count(*) from ZCVotelog where VoteID = ?", dga.getParam("ID")));
     }
   ...
   public static boolean generateJS(String ID) {
     ...
     vote.fill();
     StringBuffer sb = new StringBuffer();
     sb.append("document.write(\"<div id='vote_" + ID + "' class='votecontainer' style='text-align:left' >\");\n");
     String serviceUrl = Config.getValue("ServicesContext");
     sb.append("document.write(\" <form id='voteForm_" + ID + "' name='voteForm_" + ID + "' action='" + serviceUrl + Config.getValue("Vote.ActionURL") + "' method='post' target='_blank'>\");\n");
     ...
     for (int i = 0; i < subjectSet.size(); ++i) {
       ...
       sb.append("document.write(\"  <dt id='" + subject.getID() + "'>" + (i + 1) + "." + subject.getSubject() + "</dt>\");\n");
       ...
       for (int j = 0; j < itemSet.size(); ++j) {
         ...
         if ("0".equals(item.getItemType()))
           sb.append("document.write(\"\t\t\t\t<dd><label><input name='Subject_" + subject.getID() + "' type='" + type + "' value='" + item.getID() + "' />" + item.getItem() + "</label></dd>\");\n");
         else {
           sb.append("document.write(\"\t\t\t\t<dd><label><input name='Subject_" + subject.getID() + "' type='" + type + "' value='" + item.getID() + "' id='Subject_" + subject.getID() + "_Item_" + item.getID() + "_Button'/>" + item.getItem() + "</label><input id='Subject_" + subject.getID() + "_Item_" + item.getID() + "' name='Subject_" + subject.getID() + "_Item_" + item.getID() + "' type='text' value='' onClick=\\\"clickInput('Subject_" + subject.getID() + "_Item_" + item.getID() + "');\\\"/></dd>\");\n");
         }
       }
     }
     if ("Y".equals(vote.getVerifyFlag())) {
       sb.append("document.write(\" </dl>\");\n");
       sb.append("document.write(\" <dl>\");\n");
       sb.append("document.write(\" <dd><img src='" + Config.getContextPath() + "AuthCode.jsp' alt='点击刷新验证码' height='16' align='absmiddle' style='cursor:pointer;' onClick=\\\"this.src='" + Config.getContextPath() + "AuthCode.jsp'\\\" />&nbsp; <input\tname='VerifyCode' type='text' style='width:60px' id='VerifyCode' class='inputText' onfocus='this.select();'/></dd>\");\n");
       sb.append("document.write(\" </dl>\");\n");
     }
     sb.append("document.write(\" <dl>\");\n");
     sb.append("document.write(\" <dd><input type='submit' value='提交' onclick='return checkVote(" + ID + ");'>&nbsp;&nbsp<input type='button' value='查看' onclick='javascript:window.open(\\\"" + serviceUrl + Config.getValue("Vote.ActionURL") + "?ID=" + ID + "\\\",\\\"VoteResult\\\")'></dd>\");\n");
     sb.append("document.write(\" </dl>\");\n");
     sb.append("document.write(\" </form>\");\n");
     sb.append("document.write(\"</div>\");\n");
     String file = new StringBuffer(String.valueOf(Config.getContextRealPath())).append(Config.getValue("Statical.TargetDir")).append("/").append(SiteUtil.getAlias(vote.getSiteID())).append("/js/").toString().replaceAll("//", "/") + "vote_" + ID + ".js";
   ...
   public void del() {
     ...
     ZCVoteSubjectSchema subject = new ZCVoteSubjectSchema();
     ZCVoteSubjectSet subjectset = subject.query(new QueryBuilder("where EXISTS (select * from zcvote b where zcvotesubject.voteid=b.id and b.id in (" + ids + "))"));
     ZCVoteItemSchema voteItem = new ZCVoteItemSchema();
     ZCVoteItemSet itemset = voteItem.query(new QueryBuilder("where EXISTS (select * from zcvote b where zcvoteitem.voteid=b.id and b.id in (" + ids + "))"));
   ...
   public static DataTable getVoteItems(Mapx params, DataRow parentDR) {
     String voteID = params.getString("ID");
     DataTable dt = new QueryBuilder("select * from ZCVoteItem where voteID =? and SubjectID = ? order by ID", voteID, parentDR.get("ID")).executeDataTable();
     ...
     for (int i = 0; i < dt.getRowCount(); ++i) {
       ...
       if ("0".equals(dt.getString(i, "ItemType")))
         html = "<label><input name='Subject_" + dt.getString(i, "SubjectID") + "' type='" + inputType + "' value='" + 
           dt.getString(i, "id") + "' />" + dt.getString(i, "item") + "</label>\n";
       else {
         html = "<label><input name='Subject_" + dt.getString(i, "SubjectID") + "' type='" + inputType + "' value='" + 
           dt.getString(i, "id") + "' id='Subject_" + dt.getString(i, "SubjectID") + "_Item_" + dt.getString(i, "id") + 
           "_Button'/>" + dt.getString(i, "item") + "</label><input id='Subject_" + dt.getString(i, "SubjectID") + "_Item_" + 
           dt.getString(i, "id") + "' name='Subject_" + dt.getString(i, "SubjectID") + "_Item_" + dt.getString(i, "id") + 
           "' type='text' value='' onClick=\"clickInput('Subject_" + dt.getString(i, "SubjectID") + "_Item_" + 
           dt.getString(i, "id") + "');\"/>\n";
       }
       dt.set(i, "html", html);
     }
