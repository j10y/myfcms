 ...
 public class Shop extends Page
 {
   ...
   public void save() {
     ...
     if (!("1".equals(extend)))
     {
       ...
       {
         trans.add(
           new QueryBuilder("update zccatalog set workflow =? where innercode like ?", catalog.getWorkflow(), catalog.getInnerCode() + 
           "%"));
   ...
   public void move()
   {
     ...
     if (catalog.getChildCount() > 0L) {
       ...
       for (i = 0; i < childCatalogSet.size(); ++i) {
   ...
   public Mapx moveChild(ZCCatalogSchema catalog, long TreeLevel, String parentInnerCode)
   {
     ...
     map.put("update zccatalog set TreeLevel=" + TreeLevel + ",innercode='" + innerCode + "',modifyuser='" + 
       User.getUserName() + "',modifytime='" + DateUtil.getCurrentDate() + "' where id=" + catalogID, new Integer(4));
 
     map.put("update zcarticle set CatalogInnerCode='" + innerCode + "' where catalogid=" + catalogID, new Integer(4));
     map.put("update bzcarticle set CatalogInnerCode='" + innerCode + "' where catalogid=" + catalogID, new Integer(4));
   ...
   public static void deleteCatalog(Transaction trans, long ID) {
     ...
     trans.add(catalog.query(new QueryBuilder(" where InnerCode like ?", catalog.getInnerCode() + "%")), 5);
     trans.add(new ZCArticleSchema().query(new QueryBuilder(" where CatalogInnerCode like ?", catalog.getInnerCode() + "%")), 5);
   }
 ...
 * Qualified Name:     com.zving.cms.site.Shop
