...
public class Keyword extends Page {
	...
	private static void updateCache() {
		...
		for (int i = 0; i < dt.getRowCount(); ++i) {
			...
			QueryBuilder qb = new QueryBuilder("where siteID=? order by length(KeyWord) desc", dt
					.getString(i, 0));
	...
	public static void dg1DataBind(DataGridAction dga) {
		...
		String sql = "select ID,Keyword,LinkURL,LinkAlt,LinkTarget,addTime from ZCKeyword where 1=1 "
				+ wherePart;
		...
		for (int i = 0; i < keyWords.length; ++i) {
			if ((!(keyWords[i].equals("\r"))) && (!(StringUtil.isEmpty(keyWords[i])))) {
				...
				if ((word.length == 0) || (word.length > 4))
					continue;
				if (StringUtil.isEmpty(word[0])) {
					continue;
				}
				boolean flag = false;

				if (new QueryBuilder(
						"select count(*) from ZCKeyWord where KeyWord = ? and SiteID=? ", word[0],
						SiteID).executeInt() > 0)
					flag = true;
				else {
					flag = false;
				...
				if (flag) {
					String WordID = new QueryBuilder(
							"select ID from ZCKeyWord where KeyWord = ? and SiteID=? ", word[0],
							SiteID).executeOneValue().toString();
	...
	public void del() {
		...
		long SiteID = Application.getCurrentSiteID();
		ZCKeywordSchema keyword = new ZCKeywordSchema();
		ZCKeywordSet set = keyword.query(new QueryBuilder("where SiteID=? and id in (" + ids + ")",
				SiteID));
		trans.add(set, 5);
 ...
 * com.zving.cms.site.Keyword JD-Core Version: 0.5.3
 */