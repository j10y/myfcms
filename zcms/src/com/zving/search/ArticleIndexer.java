...
public class ArticleIndexer extends Indexer {
	...
	public void create() {
		try {
			...
			DataTable dt;
			if (rela.indexOf("-1") >= 0) {
				QueryBuilder qb = new QueryBuilder("select ID from ZCCatalog where SiteID=?", ft
						.getSiteID());
			...
			try {
				...
				for (int n = 0; n < dt.getColCount(); ++n) {
					...
					if (columnName.equalsIgnoreCase("URL"))
						...
						doc.add(new Field("TITLE", dr.getString(n), Field.Store.YES,
								Field.Index.ANALYZED));
					else if (columnName.equalsIgnoreCase("Content"))
						doc
								.add(new Field("CONTENT", content, Field.Store.YES,
										Field.Index.ANALYZED));
					else {
						doc.add(new Field(columnName.toUpperCase(), dr.getString(n),
		...
		try {
			...
			if (rela.indexOf("-1") >= 0) {
				DataTable catalogs = (new QueryBuilder(
						"select distinct CatalogID from zcarticle where siteid=" + ft.getSiteID()
								+ " and (addtime>=?" + " or modifytime>=?) and status=" + 30,
						lastDate, lastDate)).executeDataTable();
				for (int j = 0; j < catalogs.getRowCount(); j++) {
					QueryBuilder qb = new QueryBuilder(
							"select (select Name from zccatalog where id=zcarticle.catalogid) as catalogname,zcarticle.* from zcarticle where siteid="
									+ ft.getSiteID()
									+ " and CatalogID=? and (addtime>=? or modifytime>=?) and status="
									+ 30, catalogs.getString(j, "CatalogID"), lastDate);
					qb.add(lastDate);
				...
				DataTable dt = (new QueryBuilder(
						"select id from bzcarticle where status=30 and siteid=? and catalogid in ("
								+ rela + ") and backuptime>?", ft.getSiteID(), lastDate))
						.executeDataTable();
				for (int i = 0; i < dt.getRowCount(); i++)
					writer.deleteDocuments(new Term("ID", dt.getString(i, 0)));
	...
	public static SearchResult search(SearchParameters sps) {
		try {
			...
			Highlighter highlighter = new Highlighter(formatter, new QueryScorer(sps
					.getBooleanQuery()));
			...
			for (int i = start; (i < start + sps.getPageSize()) && (i < docs.scoreDocs.length); ++i) {
				...
				TokenStream tokenStream = new IKAnalyzer().tokenStream("TITLE", new StringReader(
						title));
 ...
 * com.zving.search.ArticleIndexer JD-Core Version: 0.5.3
 */