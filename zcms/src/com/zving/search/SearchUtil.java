...
public class SearchUtil {
	...
	static {
		...
		isPagePattern3 = Pattern.compile("[<\\{\\[\\|\\\\\\/]?\\d{1,4}[>\\}\\]\\|\\\\\\/]?", 34);
	...
	public static boolean isRightUrl(String url) {
		if ((url.equals("")) || (url.indexOf(32) > 0) || (url.toLowerCase().startsWith("res:"))
				...
				|| (url.toLowerCase().startsWith("mailto:")) || (url.startsWith("#"))
	...
	private static String encodeUrlEx(String url, String charset) {
		...
		for (int i = 0; i < url.length(); ++i) {
			...
			if ((c.equals("%")) && (i < url.length() - 2)
					&& (urlEncodedPattern.matcher(url.substring(i, i + 3)).find())) {
	...
	public static String[] getPageURL(WebDocument doc) {
		...
		while (m.find(lastEndIndex)) {
			...
			text = m.group(2);
			if ((isRightUrl2(url))
					|| ((url.startsWith("javascript")) && (url.indexOf("__doPostBack(") > 0))) {
	...
	private static boolean isPageURL(String url, String refurl, String text) {
		...
		if ((text.indexOf("下一页") >= 0) || (text.indexOf("后一页") >= 0) || (text.indexOf("下页") >= 0)
				|| (text.indexOf("后页") >= 0) || (text.toLowerCase().indexOf("next") >= 0)
 ...
 * com.zving.search.SearchUtil JD-Core Version: 0.5.3
 */