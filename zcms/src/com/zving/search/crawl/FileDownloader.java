 ...
 public class FileDownloader
 {
   ...
   public static final Pattern charsetPattern = Pattern.compile("<meta\\s*?http\\-equiv.*?content-type.*?content\\s*?=.*?charset\\s*?=\\s*?([^\\\"\\'\\s]+?)(\\\"|\\'|\\s|>|(/>))", 34);
   ...
   public static int downloadOne(HttpClient httpClient, WebDocument doc, boolean second, int threadIndex) {
     ...
     try {
       ...
       gm.setRequestHeader("User-Agent", "Mozilla/5.0 (Windows; U; Windows NT 5.2; zh-CN; rv:1.8.1.11) Gecko/20071127 Firefox/3.0.1 QQDownload");
*;q=0.8");
       ...
       if (h != null) {
         ...
         if (lastModified == null)
           LogUtil.getLogger().info("Error on Get lastModified：" + h.getValue() + "\t" + url + "\t RefURL：" + 
             doc.getRefUrl());
       }
       else {
       ...
       if ((contentType.indexOf("text") < 0) && (contentType.indexOf("html") < 0)) {
         doc.setTextContent(false); break label1094:
       }
       ...
       LogUtil.getLogger().info("Error Charset,URL not been Downloaded：" + charset + "\t" + url + "\t RefURL：" + 
         doc.getRefUrl());
     } catch (IllegalArgumentException e) {
       if (second) {
         e.printStackTrace();
         doc.setErrorMessage("URL语法不正确");
         LogUtil.getLogger().info("URL Syntax Error,URL not been Downloaded：\t" + url + "\t RefURL：" + doc.getRefUrl());
       } else {
         statusCode = downloadOne(httpClient, doc, true, threadIndex);
       }
     } catch (CircularRedirectException e) {
       doc.setErrorMessage("URL循环重定向");
       LogUtil.getLogger().info(gm.getStatusCode() + ":Circular Redirect,URL not been Downloaded：\t" + url + "\t RefURL：" + 
         doc.getRefUrl());
     } catch (HttpException e) {
       if (second) {
         e.printStackTrace();
         doc.setErrorMessage("HttpException,第二次重试失败.");
         LogUtil.getLogger().info("HttpException,Failed With 2 Times Retry：\t" + url + "\t RefURL：" + doc.getRefUrl());
       } else {
         statusCode = downloadOne(httpClient, doc, true, threadIndex);
       }
     } catch (IOException e) {
       if (second) {
         ...
         LogUtil.getLogger().info("IOException,Failed With 2 Times Retry：\t" + url + "\t RefURL：" + doc.getRefUrl());
       } else {
         statusCode = downloadOne(httpClient, doc, true, threadIndex);
       }
     } finally {
 ...
 * Qualified Name:     com.zving.search.crawl.FileDownloader
