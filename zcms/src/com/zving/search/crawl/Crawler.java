   ...
   public void writeArticle()
   {
     if (this.config.getType() == 1) {
       ...
       while ((doc = this.list.next()) != null) {
         ...
         label794: label847: this.task.setCurrentInfo("正在转换文档, <font color='green'>" + cSuccess + "</font> 个成功, <font color='red'>" + 
           cFailure + "</font> 个失败, <font color='green'>" + cLost + "</font> 个未匹配");
       }
     }
   }
     ...
     while (m.find(lastEndIndex)) {
       ...
       if ((SearchUtil.isRightUrl(url)) && (StringUtil.isNotEmpty(ext)) && (".gif.jpg.jpeg.bmp.png".indexOf(ext) >= 0)) {
         ...
         if ((tdoc != null) && (!(tdoc.isTextContent()))) {
           ...
           imageFilePath = saveImage(data, sitePath, imagePath, imageCatalogID, ext, fullUrl);
