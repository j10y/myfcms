...
public class UrlExtracter extends Thread {
	...
	public boolean isMatchedUrl(String url2, String refUrl) {
		...
		if ((StringUtil.isNotEmpty(ext))
				&& (((StringUtil.isEmpty(this.fileDownloader.getAllowExtension())) || (this.fileDownloader
		...
		for (int i = 0; i < this.rpArr.size(); ++i) {
			...
			if (url.indexOf(47, 8) > 0) {
				...
				if ((prefix.indexOf(36) < 0) && (!(url2.startsWith(prefix)))) {
					continue;
				}
 ...
 * com.zving.search.crawl.UrlExtracter JD-Core Version: 0.5.3
 */