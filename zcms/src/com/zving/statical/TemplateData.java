 ...
 public class TemplateData
 {
   ...
   public String getPageBar(int id) {
     ...
     sb.append("共" + this.Total + "条记录，每页" + this.PageSize + "条，当前第<span class='fc_ch1'>" + (this.PageIndex + 1) + 
       "</span>/<span class='fc_ch1'>" + this.PageCount + "</span>页&nbsp;&nbsp;&nbsp;&nbsp;");
     if (this.PageIndex > 0) {
     ...
     sb.append("&nbsp;&nbsp;<input type='button' onclick=\"if(/[^\\d]/.test(document.getElementById('_PageBar_Index_" + 
       ...
       id + "').value>" + this.PageCount + "){alert('错误的页码');document.getElementById('_PageBar_Index_" + 
       id + "').focus();}else{var PageIndex = (document.getElementById('_PageBar_Index_" + 
       id + "').value)>0?document.getElementById('_PageBar_Index_" + 
       id + "').value:1;if(PageIndex==1){window.location='index.shtml'}else{window.location='index_'+PageIndex+'.shtml';}}\" style='' value='跳转'></td>");
     sb.append("</tr></table>");
     return sb.toString();
   }
   ...
   public DataRow getPageRow() {
     ...
     String[] cols = { "Total", "PageCount", "PageSize", "FirstPage", "PrevPage", "NextPage", "LastPage", "PageIndex", 
       "PageList" };
     String[] values = { this.Total, this.PageCount, this.PageSize, getFirstPage(), getPreviousPage(), getNextPage(), 
       getLastPage(), this.PageIndex + 1, pageList.toString() };
 ...
 * Qualified Name:     com.zving.statical.TemplateData
