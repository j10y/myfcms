...
public class TemplateParser {
	...
	private static final Pattern pConfig = Pattern.compile(
			"<z:config *?(.*?) *?((/>)|(.*?</z:config>))", 34);

	private static final Pattern pDefine = Pattern.compile(
			"<z:define *?(.*?) *?((/>)|(.*?</z:define>))", 34);
	...
	public void parse() throws EvalException {
		...
		if (this.language.equalsIgnoreCase("java")) {
			...
			sb
					.append("void writeln(String str){if(str==null)str=\"\"; _Result.append(str+\"\\n\");}\n");
		...
		for (int i = 0; i < this.classArr.size(); ++i) {
	...
	private void parseHead() {
		...
		if (m.find()) {
			...
			this.template = this.template.substring(0, m.start())
					+ this.template.substring(m.end());
		}
		...
		while (m.find(lastEndIndex)) {
			...
			if (StringUtil.isNotEmpty(prop)) {
				...
				if (StringUtil.isNotEmpty((String) map.get("charwidth"))) {
					...
					if (c != 0) {
						sb.append("if(" + table + ".getString(\"" + field + "\").length()>" + c
								+ "){\n");
						sb.append("\twrite(StringUtil.subStringEx(" + table + ".getString(\""
								+ field + "\")," + (c - 1) + ")+\"...\");\n");
						sb.append("}else{\n");
				...
				if (map.get("format") != null) {
					...
					sb.append("\twrite(StringUtil.getTextFromHtml(" + table + ".getString(\""
							+ field + "\")));\n");
					sb.append("}else{\n");
					sb.append("\twrite(" + table + ".getString(\"" + field + "\"));\n");
					sb.append("}\n");
 ...
 * com.zving.statical.TemplateParser JD-Core Version: 0.5.3
 */