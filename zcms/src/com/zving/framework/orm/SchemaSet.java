   ...
   implements Serializable, Cloneable
 {
       ...
       for (int k = 0; k < this.elementCount; ++k) {
         ...
         if (this.bOperateFlag)
           for (i = 0; i < this.operateColumnOrders.length; ++i) {
             v = schema.getV(this.operateColumnOrders[i]);
             SchemaUtil.setParam(this.Columns[this.operateColumnOrders[i]], pstmt, conn, this.operateColumnOrders[i], v);
           }
         else {
         ...
         int i = 0; for (int j = 0; i < this.Columns.length; ++i) {
           SchemaColumn sc = this.Columns[i];
           if (sc.isPrimaryKey()) {
             Object v = schema.getV(sc.getColumnOrder());
             if (v == null) {
               LogUtil.warn("不满足Update的条件，" + this.TableCode + "Schema的" + sc.getColumnName() + "为空");
               return false;
             }
             if (this.bOperateFlag)
               SchemaUtil.setParam(this.Columns[i], pstmt, conn, j + this.operateColumnOrders.length, v);
             else {
               SchemaUtil.setParam(this.Columns[i], pstmt, conn, j + this.Columns.length, v);
             }
       ...
       for (int k = 0; k < this.elementCount; ++k) {
         ...
         int i = 0; for (int j = 0; i < this.Columns.length; ++i) {
           ...
           if (sc.isPrimaryKey()) {
             Object v = schema.getV(sc.getColumnOrder());
             if (v == null) {
               LogUtil.warn("不满足delete的条件，" + this.TableCode + "Schema的" + sc.getColumnName() + "为空");
               return false;
             }
   ...
   public void setOperateColumns(String[] colNames) {
     ...
     int i = 0; for (int k = 0; i < colNames.length; ++i) {
       boolean flag = false;
   ...
   public DataTable toDataTable()
   {
     ...
     int j;
     if (this.bOperateFlag) {
       dcs = new DataColumn[this.operateColumnOrders.length];
       values = new Object[this.elementCount][this.Columns.length];
       for (i = 0; i < this.operateColumnOrders.length; ++i) {
         dc = new DataColumn();
         dc.setColumnName(this.Columns[this.operateColumnOrders[i]].getColumnName());
         dc.setColumnType(this.Columns[this.operateColumnOrders[i]].getColumnType());
         dcs[i] = dc;
       }
       for (i = 0; i < this.elementCount; ++i) {
         for (j = 0; j < this.operateColumnOrders.length; ++j) {
           values[i][j] = this.elementData[i].getV(this.operateColumnOrders[j]);
         }
       }
     ...
     for (DataTable dt = 0; dt < this.Columns.length; ++dt) {
       j = new DataColumn();
       j.setColumnName(this.Columns[dt].getColumnName());
       j.setColumnType(this.Columns[dt].getColumnType());
       dcs[dt] = j;
     }
     for (dt = 0; dt < this.elementCount; ++dt) {
       for (j = 0; j < this.Columns.length; ++j) {
         values[dt][j] = this.elementData[dt].getV(j);
       }
     }
     dt = new DataTable(dcs, values);
     return dt;
   ...
   public void sort(String columnName, String order, boolean isNumber) {
     ...
     String od = order;
     sort(new Comparator(cn, od, isNumber) { private final String val$cn;
       private final String val$od;
       private final boolean val$isNumber;
 
       public int compare(Object obj1, Object obj2) { DataRow dr1 = ((Schema)obj1).toDataRow();
         DataRow dr2 = ((Schema)obj2).toDataRow();
         Object v1 = dr1.get(this.val$cn);
         Object v2 = dr2.get(this.val$cn);
         double d1;
         double d2;
         if ((v1 instanceof Number) && (v2 instanceof Number)) {
           d1 = ((Number)v1).doubleValue();
           d2 = ((Number)v2).doubleValue();
           if (d1 == d2)
             return 0;
           if (d1 > d2) {
             return (("asc".equalsIgnoreCase(this.val$od)) ? 1 : -1);
           }
           return (("asc".equalsIgnoreCase(this.val$od)) ? -1 : 1);
         }
         if ((v1 instanceof Date) && (v2 instanceof Date)) {
           Date d1 = (Date)v1;
           Date d2 = (Date)v1;
           if ("asc".equalsIgnoreCase(this.val$od)) {
         ...
         int c = dr1.getString(this.val$cn).compareTo(dr2.getString(this.val$cn));
         if ("asc".equalsIgnoreCase(this.val$od)) {
           return c;
         }
         return (-c);
 ...
 * Qualified Name:     com.zving.framework.orm.SchemaSet
