...
public class VideoUtil {
	...
	public static boolean captureImage(String src, String destImage, int ss, int width, int height) {
		return exec("\"" + Config.getContextRealPath() + "Tools/" + "ffmpeg\" -i " + "\"" + src
				+ "\"" + " -y -f image2 -ss " + ss + " -t 0.001 -s " + width + "*" + height + " \""
				+ destImage + "\"");
	}
	...
	public static int getDuration(String src) {
		String command = "\"" + Config.getContextRealPath() + "Tools/" + "mplayer\" -identify "
				+ "\"" + src + "\"" + " -nosound -vc dummy -vo null";
		int duration = 0;
		try {
 ...
 * com.zving.framework.utility.VideoUtil JD-Core Version: 0.5.3
 */