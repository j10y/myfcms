...
public class SSIServletExternalResolver implements SSIExternalResolver {
	protected final String[] VARIABLE_NAMES = { "AUTH_TYPE", "CONTENT_LENGTH", "CONTENT_TYPE",
			"DOCUMENT_NAME", "DOCUMENT_URI", "GATEWAY_INTERFACE", "HTTP_ACCEPT",
			...
			"QUERY_STRING_UNESCAPED", "REMOTE_ADDR", "REMOTE_HOST", "REMOTE_PORT", "REMOTE_USER",
			"REQUEST_METHOD", "REQUEST_URI", "SCRIPT_FILENAME", "SCRIPT_NAME", "SERVER_ADDR",
			"SERVER_NAME", "SERVER_PORT", "SERVER_PROTOCOL", "SERVER_SOFTWARE", "UNIQUE_ID" };
	...
	private Mapx timeMap = new Mapx();
	private Mapx fileMap = new Mapx();

	public SSIServletExternalResolver(ServletContext context, HttpServletRequest req,
			HttpServletResponse res, boolean isVirtualWebappRelative, int debug,
	...
	protected boolean isNameReserved(String name) {
		return ((name.startsWith("java.")) || (name.startsWith("javax.")) || (name
				.startsWith("sun.")));
	}
	...
	protected String getCGIVariable(String name) {
		...
		if (nameParts.length == 1) {
			...
			} else if ((nameParts[1].equals("USER")) && (nameParts.length == 3)
			...
			if (nameParts[1].equals("STRING")) {
				...
				if (nameParts.length == 2) {
					...
					if (queryString != null) {
						...
						else if ((useBodyEncodingForURI) && (requestEncoding != null)) {
	...
	protected ServletContextAndPath getServletContextAndPath(String originalPath, boolean virtual)
		...
		if (this.debug > 0) {
			log("SSIServletExternalResolver.getServletContextAndPath( " + originalPath + ", "
					+ virtual + ")", null);
		}
		if (virtual)
	...
	public String getFileText(String originalPath, boolean virtual) throws IOException {
		try {
			...
			if ((".html".equals(ext)) || (".shtml".equals(ext)) || (".htm".equals(ext))) {
				...
				ResponseIncludeWrapper responseIncludeWrapper = new ResponseIncludeWrapper(context,
						this.req, this.res, basos);
 ...
 * com.zving.framework.ssi.SSIServletExternalResolver JD-Core Version: 0.5.3
 */