 ...
 public class Transaction
 {
   ...
   {
     if (this.bConnFlag == 0) {
       this.mDataAccess = new DataAccess(); } boolean NoErrFlag = true;
     Object[] ks;
     Object[] vs;
     int i;
     try { if ((this.bConnFlag == 0) || (setAutoCommitStatus)) {
       ...
       if (obj instanceof QueryBuilder) {
         this.mDataAccess.executeNoQuery((QueryBuilder)obj); } else {
         if (obj instanceof Schema) {
           Schema s = (Schema)obj;
           s.setDataAccess(this.mDataAccess);
           if (type == 1) {
             if (s.insert()) break label926;
             NoErrFlag = false;
           }
           while (true)
           {
             ...
             if (type == 2) {
               if (s.update()) break label926;
               NoErrFlag = false;
             }
 
             if (type == 3) {
               if (s.delete()) break label926;
               NoErrFlag = false;
             }
 
             if (type == 4) {
               if (s.backup(this.BackupOperator, this.BackupMemo)) break label926;
               NoErrFlag = false;
             }
 
             if (type == 5) {
               if (s.deleteAndBackup(this.BackupOperator, this.BackupMemo)) break label926;
               NoErrFlag = false;
             }
 
             if ((type != 6) || 
               (s.deleteAndInsert())) break label926;
             NoErrFlag = false;
           }
         }
 ...
 * Qualified Name:     com.zving.framework.data.Transaction
