   ...
   implements Serializable, Cloneable
 {
   ...
   public void insertRow(Object[] rowValue, int index) {
     ...
     if (rowValue != null) {
       ...
       if (rowValue.length != this.columns.length) {
         throw new RuntimeException("新增行的列数为" + rowValue.length + "，要求的列数为" + this.columns.length + "！");
       }
       for (int i = 0; i < this.columns.length; ++i) {
         if ((this.columns[i].ColumnType != 0) || 
           (rowValue[i] == null) || (Date.class.isInstance(rowValue[i]))) continue;
   ...
   public void sort(String columnName, String order, boolean isNumber) {
     ...
     sort(new Comparator(cn, od, isNumber) { private final String val$cn;
       ...
       public int compare(Object obj1, Object obj2) { DataRow dr1 = (DataRow)obj1;
 ...
 * Qualified Name:     com.zving.framework.data.DataTable
