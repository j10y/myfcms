...
public class DataTableUtil {
	public static DataTable txtToDataTable(String fileName) {
		return txtToDataTable(fileName, "", ",");
	}
	...
	public static DataTable txtToDataTable(String fileName, String encoding, String filedspliter,
			String rowspliter) {
	...
	public static DataTable txtToDataTable(String fileName, String[] columnNames, String encoding,
			String filedspliter, String rowspliter) {
	...
	public static void dataTableToExcel(DataTable dt, String fileName, String[] columnNames,
			String[] widths) {
		try {
			FileOutputStream fos = new FileOutputStream(fileName);
	...
	public static void dataTableToExcel(DataTable dt, OutputStream os, String[] columnNames,
			String[] widths) {
			...
			for (int i = 0; i < dt.getColCount(); ++i) {
				...
				if ((widths != null) && (widths.length > i)) {
					sheet.setColumnWidth((short) i,
							(short) (int) (Short.parseShort(widths[i]) * 37.5D));
				}
			}
	...
	public static void dataTableToExcel(DataTable dt, OutputStream os, String[] columnNames,
			String[] widths, String[] columnIndexes) {
		HSSFWorkbook wb = new HSSFWorkbook();
		HSSFSheet sheet = wb.createSheet("First");
		try {
			HSSFFont fontBold = wb.createFont();
			...
			for (int i = 0; i < columnNames.length; ++i) {
				...
				if ((widths != null) && (widths.length > i)) {
					sheet.setColumnWidth((short) i,
							(short) (int) (Integer.parseInt(widths[i]) * 37.5D));
				}
			}
		...
		for (int i = 0; i < dt.getRowCount(); ++i) {
			...
			if (i == 0) {
				for (int j = 0; j < backdt.getColCount(); ++j) {
					backdt.getDataColumn(j).setColumnName(
							"Backup" + backdt.getDataColumn(j).getColumnName());
					dt.insertColumn(backdt.getDataColumn(j).getColumnName());
				}
			}
			if (backdt.getRowCount() == 1) {
				System.arraycopy(backdt.getDataRow(0).getDataValues(), 0, dt.getDataRow(i)
						.getDataValues(), dt.getColCount() - backdt.getColCount(), backdt
						.getColCount());
			}
		}
		return null;
 ...
 * com.zving.framework.data.DataTableUtil JD-Core Version: 0.5.3
 */