...
public class DBConnPoolImpl {
	...
	public void init() {
		...
		if (StringUtil.isNotEmpty(this.dcc.JNDIName)) {
			...
			this.dcc.DBServerAddress = Config.getValue("Database." + this.dcc.PoolName
					+ "ServerAddress");
			...
			for (int i = 0; i < this.dcc.ConnCount; ++i) {
				...
				if (conn.isUsing) {
					if (!(conn.LongTimeFlag)) {
						if (now - conn.LastSuccessExecuteTime > this.dcc.MaxConnUsingTime) {
							LogUtil.getLogger().error(
									this.dcc.PoolName + ":检测到连接使用超时，程序可能存在连接池泄漏，将自动关闭连接。以下是调用堆栈:");
						...
						LogUtil.getLogger().warn(
								this.dcc.PoolName + ":检测到连接长时间使用，共使用了"
										+ (now - conn.LastSuccessExecuteTime) + "毫秒。以下是调用堆栈:");
					...
					if (System.currentTimeMillis() - conn.getLastSuccessExecuteTime() > this.dcc.RefershPeriod) {
						...
						try {
							dAccess.executeOneValue(new QueryBuilder("select 1 from "
	...
	public static DBConn createConnection(DBConnConfig dbcc, boolean bLongTimeFlag)
			throws Exception {
		Connection conn = null;
		if (dbcc.isJNDIPool) {
 ...
 * com.zving.framework.data.DBConnPoolImpl JD-Core Version: 0.5.3
 */