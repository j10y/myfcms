       ...
       for (int j = 0; j < dt.getColCount(); ++j) {
         if (dt.getDataColumn(j).getColumnName().indexOf("_") > 0) {
           trans.add(p.query(
             ...
             RoleCode + "' and PrivType = '" + dt.getString(i, "PrivType") + "' and ID = '" + 
             dt.getString(i, "ID") + "' and Code = '" + dt.getDataColumn(j).getColumnName() + "' ")), 5);
         }
       }
     }
 ...
 * Qualified Name:     com.zving.platform.RoleTabSite
