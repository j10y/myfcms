 ...
 public class Install extends Ajax
 {
   public void execute()
   {
     ...
     try {
       ...
       if (importData) {
         ...
         ltt = new LongTimeTask(conn2, autoCreate, r, dcc) { private final DBConn val$conn2;
           ...
           public void execute() { try { DBImport di = new DBImport();
               ...
               if (di.importDB(Config.getContextRealPath() + "WEB-INF/data/backup/install.dat", this.val$conn2, 
                 this.val$autoCreate)) {
                 if (this.val$autoCreate) {
                   setCurrentInfo("正在建立索引");
                   Install.createIndexes(this.val$conn2, null, this.val$conn2.getDBType());
                 }
                 ...
                 Config.loadConfig();
                 CronManager.getInstance().init(); break label160:
               }
         ...
         this.Response.setStatus(1); return;
       }
       init(conn2, r);
   ...
   public static void generateSQL(HttpServletRequest request, HttpServletResponse response) {
     ...
     String sql = new DBImport().getSQL(Config.getContextRealPath() + "WEB-INF/data/backup/install.dat", dbtype);
   ...
   public static void createIndexes(DBConn conn, StringBuffer sbAll, String dbtype) {
     String[] arr = { "zcarticle(CatalogID)", "zcarticle(CatalogInnercode)", "zcarticle(SiteID)", 
       "zcarticle(keyword)", "zcimage(CatalogID)", "zcimage(CatalogInnercode)", "zcimage(SiteID)", 
       "zcvideo(CatalogID)", "zcvideo(CatalogInnercode)", "zcvideo(SiteID)", "zcaudio(CatalogID)", 
       "zcaudio(CatalogInnercode)", "zcaudio(SiteID)", "zcattachment(CatalogID)", 
       "zcattachment(CatalogInnercode)", "zcattachment(SiteID)", "zcarticlelog(articleID)", 
       "zcarticlevisitlog(articleID)", "zccomment(relaid)", "zccomment(catalogid)", "zccatalog(SiteID)", 
       "zccatalog(InnerCode)", "zcpageblock(SiteID)", "zcpageblock(CatalogID)", "zdprivilege(owner)", 
       "zdform(siteid)", "zdcolumnrela(relaid)", "zdcolumnvalue(relaid)", "zwworkflowentry(workflowdefid)", 
       "zwcurrentstep(entryid)", "zwcurrentstep(stepid)", "zwcurrentstep(actionid)", "zwcurrentstep(owner)" };
     DataAccess da = null;
     if (conn == null)
       da = new DataAccess();
     else {
       ...
       if (conn != null) {
         try {
           if (dbtype.equals("MYSQL")) {
             da.executeNoQuery(new QueryBuilder("alter table " + table + " drop index " + name)); break label678: }
           if (dbtype.equals("MSSQL")) {
             da.executeNoQuery(new QueryBuilder("drop index " + name + " on " + table)); break label678:
           }
