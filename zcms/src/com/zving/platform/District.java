...
public class District extends Page {
	...
	public static void dg1BindDistrict(DataGridAction dga) {
		...
		if (dga.getTotal() == 0) {
			dga.setTotal(new QueryBuilder(
					"select count(*) from ZDDistrict where TreeLevel = 1 or TreeLevel = 2"));
		}
	...
	public static void dg1BindDistrictList(DataGridAction dga) {
		...
		dga.setTotal(new QueryBuilder(
				"select count(*) from ZDDistrict where TreeLevel = '3' and code like ?", parentCode
						+ "%"));
		dga
	...
	public static Mapx init(Mapx params) {
		...
		map.put("Code", code);
		String Name = new QueryBuilder("select name from ZDDistrict where code = ?", code)
				.executeString();
		map.put("Name", Name);
		return map;
	}
	...
	public static void generateDistrictJS() {
		...
		String path = Config.getContextRealPath()
				+ "WEB-INF/classes/com/zving/Platform/district.template";
		String func = FileUtil.readText(path);
		String JS = Config.getContextRealPath() + "Framework/District.js";
		FileUtil.writeText(JS, provinceMap.toString() + cityMap.toString() + districtMap.toString()
				+ func);
	}
 ...
 * com.zving.platform.District JD-Core Version: 0.5.3
 */