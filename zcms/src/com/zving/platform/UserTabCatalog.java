 ...
 public class UserTabCatalog extends Page
 {
   ...
   public static void dg1DataBind(DataGridAction dga) {
     ...
     String sql = "select ID,Name,0 as TreeLevel ,'site' as PrivType" + sb.toString().replaceAll("''", "''") + " from ZCSite a where a.ID = ?";
     ...
     sql = "select InnerCode as ID,Name,TreeLevel ,'" + PrivType + "' as PrivType" + sb.toString() + " from ZCCatalog a where Type =" + catalogType + " and a.SiteID = " + siteID + " order by orderflag,innercode ";
     ...
     for (int i = 0; i < siteDT.getRowCount(); ++i) {
       ...
       for (j = 0; j < dr.getColumnCount(); ++j) {
         ...
         if (columnName.indexOf("_") > 0) {
           dr.set(j, (Priv.getPriv(userName, "site", dr.getString("ID"), columnName)) ? "√" : "");
         }
       }
     }
 
     for (i = 0; i < catalogDT.getRowCount(); ++i) {
     ...
     for (int i = 0; i < resultDT.getRowCount(); ++i) {
       for (int j = 0; j < resultDT.getColCount(); ++j) {
         if (resultDT.getDataColumn(j).getColumnName().indexOf("_") > 0) {
           ...
           trans.add(p.query(new QueryBuilder("where OwnerType='U' and Owner = '" + UserName + "' and PrivType = '" + resultDT.getString(i, "PrivType") + "' and ID = '" + resultDT.getString(i, "ID") + "' and Code = '" + resultDT.getDataColumn(j).getColumnName() + "' ")), 5);
         }
       }
     }
     ...
     for (int i = 0; (i < rolePrivDT.getRowCount()) && (i < 1); ++i) {
       for (j = 0; j < rolePrivDT.getColCount(); ++j) {
         if (rolePrivDT.getDataColumn(j).getColumnName().indexOf("_") > 0) {
           rolePrivDT.set(i, j, (RolePriv.getRolePriv(RoleCodes, "site", rolePrivDT.getString(i, "ID"), rolePrivDT.getDataColumn(j).getColumnName())) ? "1" : 
             "0");
         }
       }
     }
     for (i = 1; i < rolePrivDT.getRowCount(); ++i) {
       for (j = 0; j < rolePrivDT.getColCount(); ++j) {
         if (rolePrivDT.getDataColumn(j).getColumnName().indexOf("_") > 0) {
           rolePrivDT.set(i, j, (RolePriv.getRolePriv(RoleCodes, PrivType, rolePrivDT.getString(i, "ID"), rolePrivDT.getDataColumn(j).getColumnName())) ? "1" : 
             "0");
         }
       }
