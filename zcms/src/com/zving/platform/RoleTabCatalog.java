	...
	public static void dg1DataBind(DataGridAction dga) {
		...
		if (StringUtil.isEmpty(siteID)) {
			...
			if (StringUtil.isNotEmpty(siteID)) {
				if (!(RolePriv
						.getRolePriv(new String[] { roleCode }, "site", siteID, "site_browse"))) {
					siteID = "";
				}
			}
		}
			...
			if (mapRow != null) {
				for (int j = 0; j < dr.getColumnCount(); ++j) {
					if (dr.getDataColumn(j).getColumnName().toLowerCase().indexOf("_") > 0) {
						dr.set(j,
								("1".equals(mapRow.get(dr.getDataColumn(j).getColumnName()))) ? "√"
										: "");
					}
				}
			}
		...
		QueryBuilder qb = new QueryBuilder("select InnerCode as ID,Name,TreeLevel ,'" + PrivType
				+ "' as PrivType" + sb.toString() + " from ZCCatalog a where Type = " + catalogType
		...
		for (int i = 0; i < catalogDT.getRowCount(); ++i) {
			...
			if (mapRow == null) {
				...
				for (j = 0; j < dr.getColumnCount(); ++j) {
					if (dr.getDataColumn(j).getColumnName().toLowerCase().indexOf("_") > 0) {
						dr.set(j,
								("0".equals(mapRow.get(dr.getDataColumn(j).getColumnName()))) ? ""
										: "√");
					}
				}
			}
 ...
 * com.zving.platform.RoleTabCatalog JD-Core Version: 0.5.3
 */