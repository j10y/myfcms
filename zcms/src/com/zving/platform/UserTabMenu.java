		...
		for (int i = 0; i < rolePrivDT.getRowCount(); ++i) {
			for (int j = 0; j < rolePrivDT.getColCount(); ++j) {
				if (rolePrivDT.getDataColumn(j).getColumnName().indexOf("_") > 0) {
					rolePrivDT.set(i, j, (RolePriv.getRolePriv(RoleCodes, "menu", SiteID + "-"
							+ rolePrivDT.getString(i, "ID"), rolePrivDT.getDataColumn(j)
							.getColumnName())) ? "1" : "0");
				}
			}
 ...
 * com.zving.platform.UserTabMenu JD-Core Version: 0.5.3
 */