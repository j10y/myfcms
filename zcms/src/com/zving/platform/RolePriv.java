...
public class RolePriv {
	...
	public static void updateAllPriv(String RoleCode) {
		Object obj = new QueryBuilder("select RoleCode from ZDRole where RoleCode = ?", RoleCode)
				.executeOneValue();
		if (obj == null) {
	...
	public static boolean getRolePriv(String RoleCodes[], String PrivType, String ID, String Code) {
		...
		if ("menu".equals(PrivType)) {
			for (int i = 0; i < RoleCodes.length; i++) {
				Map map = getPrivTypeMap(RoleCodes[i], PrivType);
				map = (Map) map.get(ID);
				if (map != null) {
 ...
 * com.zving.platform.RolePriv JD-Core Version: 0.5.3
 */