			...
			for (int j = 0; j < dt.getColCount(); ++j) {
				if (dt.getDataColumn(j).getColumnName().indexOf("_") > 0) {
					trans.add(p.query(new QueryBuilder("where OwnerType='U' and Owner = '"
							+ UserName + "' and PrivType = '" + dt.getString(i, "PrivType")
		...
		for (int i = 0; i < dt.getRowCount(); ++i) {
			...
			while (j < dr.getColumnCount()) {
				if (dr.getDataColumn(j).getColumnName().indexOf("_") > 0) {
					...
					boolean roleValue = RolePriv.getRolePriv(RoleCodes, PrivType, dr
							.getString("ID"), dr.getDataColumn(j).getColumnName());
					if (value != roleValue) {
						same = false;
						break;
					}
			...
			for (j = 0; j < dr.getColumnCount(); ++j) {
				if (dr.getDataColumn(j).getColumnName().indexOf("_") > 0) {
					value = "√".equals(dr.getString(j));
					roleValue = RolePriv.getRolePriv(RoleCodes, PrivType, dr.getString("ID"), dr
 ...
 * com.zving.platform.UserTabSite JD-Core Version: 0.5.3
 */