...
var cssdropdown = {
	disappeardelay : 250, // set delay in miliseconds before menu disappears
							// onmouseout
	disablemenuclick : true, // when user clicks on a menu item with a drop
	...
	swipeeffect : function() {
		if (this.bottomclip < parseInt(this.dropmenuobj.offsetHeight)) {
			this.bottomclip += 10 + (this.bottomclip / 10) // unclip drop down
															// menu visibility
															// gradually
			this.dropmenuobj.style.clip = "rect(0 auto " + this.bottomclip
					+ "px 0)"
		} else
			return
	...
	clearbrowseredge : function(obj, whichedge) {
		...
		if (whichedge == "rightedge") {
			var windowedge = this.ie && !window.opera
					? this.iecompattest().scrollLeft
							+ this.iecompattest().clientWidth - 15
			...
			var topedge = this.ie && !window.opera
					? this.iecompattest().scrollTop
					: window.pageYOffset
			var windowedge = this.ie && !window.opera
					? this.iecompattest().scrollTop
							+ this.iecompattest().clientHeight - 15
			...
			for (var i = 0; i < menuitems.length; i++) {
				if (menuitems[i].getAttribute("rel")) {
					...
					menuitems[i].onmouseover = function(e) {
						...
						cssdropdown.dropit(this, event, this
										.getAttribute("rel"))
					}
				}
			}
