 ...
 * 描述：
 */
public class SupplierQuery extends ListWindow {
	...
	public void onFind() {
		...
		if (StringUtils.hasText(search.getValue())) {
			LogicalExpression l1 = Restrictions.or(Restrictions.like("companyName", search
					.getValue(), MatchMode.ANYWHERE), Restrictions.like("address", search
					.getValue(), MatchMode.ANYWHERE));

			LogicalExpression l2 = Restrictions.or(l1, Restrictions.like("legalPerson", search
					.getValue(), MatchMode.ANYWHERE));

			LogicalExpression l3 = Restrictions.or(l2, Restrictions.like("reference", search
					.getValue(), MatchMode.ANYWHERE));

			LogicalExpression l4 = Restrictions.or(l3, Restrictions.like("contacts", search
					.getValue(), MatchMode.ANYWHERE));

			LogicalExpression l5 = Restrictions.or(l4, Restrictions.like("remarks", search
					.getValue(), MatchMode.ANYWHERE));
		...
		Pagination pagination = supplierService.findPageByCriteria(detachedCriteria, pg
				.getPageSize(), pg.getActivePage() + 1);
	...
	public void onAdd() {
		try {
			((Window) Executions.createComponents("/supplier/supplierAdd.zul", this, null))
					.doModal();
	...
	public void onEdit() {
		...
		try {
			((Window) Executions.createComponents("/supplier/supplierEdit.zul", this, map))
					.doModal();
		} catch (SuspendNotAllowedException e) {
			e.printStackTrace();
		} catch (InterruptedException e) {
	...
	public void onDelete() {
		...
		Map map = new HashMap();
		map.put("suppliers", set);
		try {
			((Window) Executions.createComponents("/supplier/supplierDelete.zul", this, map))
					.doModal();
		} catch (SuspendNotAllowedException e) {
			e.printStackTrace();
		} catch (InterruptedException e) {
		...
		if (media.getFormat().equals("xls")) {
			try {
				...
				for (int i = 1; i < rs.getRows(); i++) {
					...
					supplier.setCompanyName(cells[1].getContents());
					supplier.setAddress(cells[2].getContents());
					supplier.setLegalPerson(cells[3].getContents());
					supplier.setReference(cells[4].getContents());
					supplier.setContacts(cells[5].getContents());
					supplier.setRemarks(cells[6].getContents());
					Dict dict = new Dict();
					// dict.setId(24L);
					supplier.setCategory(dict);
	...
	private boolean checkExcel(jxl.Sheet rs) {
		...
		if (!cells[0].getContents().equals("类别") || !cells[1].getContents().equals("单位名称")
				|| !cells[2].getContents().equals("地址") || !cells[3].getContents().equals("法人代表")
				|| !cells[4].getContents().equals("联系人") || !cells[5].getContents().equals("联系电话")) {
	...
	public void onDetail() {
		...
		try {
			((Window) Executions.createComponents("/supplier/supplierDetail.zul", this, map))
					.doModal();
		} catch (SuspendNotAllowedException e) {
			e.printStackTrace();
		} catch (InterruptedException e) {
