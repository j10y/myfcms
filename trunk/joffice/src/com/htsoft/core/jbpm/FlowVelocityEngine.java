...
public class FlowVelocityEngine implements FactoryBean {
	...
	public VelocityEngine createVelocityEngine() throws IOException, VelocityException {
		...
		for (Iterator it = this.velocityProperties.entrySet().iterator(); it.hasNext();) {
		...
		velocityEngine.setProperty("file.resource.loader.path", AppUtil.getAppAbsolutePath()
				+ this.templatePath);
		try {
			velocityEngine.init();
			...
			this.logger.error(
					"Why does VelocityEngine throw a generic checked exception, after all?", ex);
