...
public class JbpmServiceImpl implements JbpmService {
	...
	public ProDefinition saveOrUpdateDeploy(ProDefinition proDefinition) {
		if (proDefinition.getDeployId() == null) {
			...
			String deployId = this.repositoryService.createDeployment().addResourceFromString(
					"process.jpdl.xml", proDefinition.getDefXml()).deploy();
			...
			ProDefinition proDef = (ProDefinition) this.proDefinitionService.get(proDefinition
					.getDefId());
	...
	public ProcessDefinition getProcessDefinitionByKey(String processKey) {
		List list = this.repositoryService.createProcessDefinitionQuery().processDefinitionKey(
				processKey).orderDesc("versionProperty.longValue").list();
	...
	public ProDefinition getProDefinitionByKey(String processKey) {
		...
		if (processDefinition != null) {
			ProDefinition proDef = this.proDefinitionService.getByDeployId(processDefinition
					.getDeploymentId());
			return proDef;
		}
		return null;
	...
	public String getDefinitionXmlByPiId(String piId) {
		ProcessInstance pi = this.executionService.createProcessInstanceQuery().processInstanceId(
				piId).uniqueResult();
	...
	public String startProcess(String deployId, Map variables) {
		ProcessDefinition pd = this.repositoryService.createProcessDefinitionQuery().deploymentId(
	...
	public void assignTask(ProcessInstance pi, ProcessDefinition pd, String assignId,
			String taskName) {
		if (pd == null) {
			pd = this.repositoryService.createProcessDefinitionQuery().processDefinitionId(
		...
		if (StringUtils.isNotEmpty(taskName)) {
			taskList = this.taskService.createTaskQuery().processInstanceId(pi.getId())
					.activityName(taskName).list();
		}
		...
		for (Task task : taskList) {
			if (StringUtils.isNotEmpty(assignId)) {
				...
				ProUserAssign assign = this.proUserAssignService.getByDeployIdActivityName(pd
						.getDeploymentId(), task.getActivityName());

				if (assign != null) {
					if ("__start".equals(assign.getUserId())) {
						AppUser flowStartUser = (AppUser) this.executionService.getVariable(pi
								.getId(), "flowStartUser");
						if (flowStartUser != null)
							this.taskService.assignTask(task.getId(), flowStartUser.getUserId()
									.toString());
					} else {
						StringBuffer upIds;
						Object localObject;
						...
						if ("__super".equals(assign.getUserId())) {
							...
							if (flowStartUser != null) {
								List superUserIds = this.userSubService.upUser(flowStartUser
										.getUserId());
								upIds = new StringBuffer();
								for (localObject = superUserIds.iterator(); ((Iterator) localObject)
										.hasNext();) {
								...
								this.taskService.addTaskParticipatingUser(task.getId(), upIds
										.toString(), "candidate");
							}
							...
							if ((userIds != null) && (userIds.length > 1)) {
								...
								for (upIds = 0; upIds < userId; ++upIds) {
									String uId = localObject[upIds];
									this.taskService.addTaskParticipatingUser(task.getId(), uId,
											"candidate");
								}
							} else {
					...
					if (StringUtils.isNotEmpty(assign.getRoleId()))
						this.taskService.addTaskParticipatingGroup(task.getId(),
								assign.getRoleId(), "candidate");
				} else {
					AppUser flowStartUser = (AppUser) this.executionService.getVariable(pi.getId(),
							"flowStartUser");
					if (flowStartUser != null)
						this.taskService.assignTask(task.getId(), flowStartUser.getUserId()
								.toString());
	...
	protected boolean isProcessInstanceEnd(String executionId) {
		HistoryProcessInstance hpi = this.historyService.createHistoryProcessInstanceQuery()
 ...
 * Qualified Name: com.htsoft.oa.service.flow.impl.JbpmServiceImpl JD-Core
 * Version: 0.5.4
 */