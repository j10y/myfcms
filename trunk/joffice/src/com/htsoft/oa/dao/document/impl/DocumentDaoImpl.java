...
public class DocumentDaoImpl extends BaseDaoImpl<Document> implements DocumentDao {
	...
	public List<Document> findByIsShared(Document document, Date from, Date to, Long userId,
			ArrayList<Long> roleIds, Long depId, PagingBean pb) {
	...
	public List<Document> findByPublic(String path, Document document, Date from, Date to,
			AppUser appUser, PagingBean pb) {
		StringBuffer sb = new StringBuffer();
		List list = new ArrayList();
		if (!appUser.getRights().contains("__ALL")) {
			sb
					.append("select doc from Document doc,DocFolder docF,DocPrivilege pr where doc.docFolder=docF and pr.docFolder=docF");
			...
			Integer userId = Integer.valueOf(Integer.parseInt(appUser.getUserId().toString()));
			list.add(userId);
			if (appUser.getDepartment() != null) {
				Integer depId = Integer.valueOf(Integer.parseInt(appUser.getDepartment().getDepId()
						.toString()));
			...
			sb
					.append("select doc from Document doc,DocFolder docF where doc.docFolder=docF and docF.isShared=1");
		}
		if (path != null) {
		...
		sb.append(" group by doc order by doc desc");
		List docList = findByHql(sb.toString(), list.toArray(), pb);
		return docList;
	}

	public List<Document> findByPersonal(Long userId, Document document, Date from, Date to,
			String path, PagingBean pb) {
		StringBuffer sb = new StringBuffer();
		ArrayList list = new ArrayList();
		sb
				.append("select doc from Document doc,DocFolder docFolder where doc.docFolder=docFolder and docFolder.appUser.userId is not Null");
		if (path != null) {
	...
	public List<Document> searchDocument(AppUser appUser, String content, boolean isHaveData,
			PagingBean pb) {
		StringBuffer buff = new StringBuffer("select doc from Document doc,DocFolder docF ");
		if (isHaveData) {
		...
		buff
				.append(") or (doc.isShared=0 and doc.docFolder=docF and docF.appUser.userId is not Null and doc.appUser.userId=? )");
		list.add(appUser.getUserId());
		...
		if ((isHaveData) && (!appUser.getRights().contains("__ALL"))) {
			...
			if ((sb.toString() != null) && (sb.length() > 1)) {
				buff.append(" or (pr.udrId in (" + sb.toString() + ") and pr.flag=3)");
			}
			buff.append(")");
		}
		...
		buff.append(" group by doc order by doc desc");
		return findByHql(buff.toString(), list.toArray(), pb);
	}
}
