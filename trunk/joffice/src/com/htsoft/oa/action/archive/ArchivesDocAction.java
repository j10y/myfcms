...
public class ArchivesDocAction extends BaseAction {
	...
	public String save() {
		...
		if (this.archivesDoc.getDocId() == null) {
			...
			this.archivesDoc.setFileAttach(this.fileAttachService.getByPath(this.archivesDoc
					.getDocPath()));
			this.archivesDocService.save(this.archivesDoc);
		} else {
			ArchivesDoc oldVersion = (ArchivesDoc) this.archivesDocService.get(this.archivesDoc
					.getDocId());
			...
			this.archivesDoc.setCurVersion(Integer
					.valueOf(oldVersion.getCurVersion().intValue() + 1));
		...
		buff.append(json.exclude(new String[] { "class", "docHistorys" }).serialize(
				this.archivesDoc));
