...
public class ArchDispatchAction extends BaseAction {
	...
	public String disList() {
		...
		List list = this.archDispatchService.findByUser(ContextUtil.getCurrentUser(), pb);
		StringBuffer buff = new StringBuffer("{success:true,'totalCounts':").append(
				...
				if (StringUtils.isNotEmpty(signUserIds)) {
					...
					if (size < 2) {
						if ((this.archUserType.compareTo(ArchDispatch.IS_UNDERTAKE) == 0)
							...
							sdf.format(new Date());
							sb.append("--").append(ContextUtil.getCurrentUser().getFullname())
									.append("--").append(sdf.format(new Date())).append("</div>");
						...
						int recordSize = this.archDispatchService.countArchDispatch(archives
								.getArchivesId());
						if ((this.archUserType.compareTo(ArchDispatch.IS_UNDERTAKE) == 0)
								&& (StringUtils.isNotEmpty(cruArchDepId))
								&& (cruArchDepId.indexOf("$") == -1)) {
