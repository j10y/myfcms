...
public class AppUserAction extends BaseAction {
	...
	public String getCurrent() {
		...
		List<IndexDisplay> list = this.indexDisplayService.findByUser(currentUser.getUserId());
		...
		sb.append("{success:true,user:{userId:'").append(currentUser.getUserId()).append(
				"',fullname:'").append(currentUser.getFullname()).append("',depId:'").append(
				curDep.getDepId()).append("',depName:'").append(curDep.getDepName()).append(
				"',rights:'");
	...
	public String online() {
		...
		if (this.depId != null) {
			for (Object o : onlineUsers.keySet()) {
		...
		if (this.roleId != null) {
			for (Object o : onlineUsers.keySet()) {
				...
				if (Pattern.compile("," + this.roleId + ",").matcher(onlineUser.getRoleIds())
						.find()) {
		...
		StringBuffer buff = new StringBuffer("{success:true,'totalCounts':").append(
				onlineUsers.size()).append(",result:");
	...
	public String find() {
		...
		if (StringUtils.isNotEmpty(strRoleId)) {
			List userList = this.appUserService.findByRole(Long.valueOf(Long.parseLong(strRoleId)),
					pb);
			Type type = new TypeToken() {
			}.getType();
			StringBuffer buff = new StringBuffer("{success:true,'totalCounts':").append(
	...
	public String get() {
		...
		JSONSerializer json = JsonUtil.getJSONSerializer(new String[] { "accessionTime" });
		if (this.userId != null) {
			appUser = (AppUser) this.appUserService.get(this.userId);
		} else {
			json.exclude(new String[] { "accessionTime", "department", "password", "status",
					"position" });
	...
	public String save() {
		...
		if (this.appUser.getUserId() != null) {
			...
			setJsonString("{success:false,msg:'用户登录账号:" + this.appUser.getUsername()
					+ "已存在,请重新输入账号.'}");
		}
	...
	public String subArole() {
		...
		if (StringUtils.isNotEmpty(strRoleId)) {
			...
			List userList = this.appUserService.findSubAppUserByRole(new Long(strRoleId), userIds,
					pb);
	...
	public String upUser() {
		List<Long> ids = this.userSubService.upUser(ContextUtil.getCurrentUserId());
		...
		for (AppUser user : list) {
			buff.append("['" + user.getUserId().toString() + "','" + user.getFullname() + "'],");
		}
		if (list.size() > 0) {
 ...
 * Qualified Name: com.htsoft.oa.action.system.AppUserAction JD-Core Version:
 * 0.5.4
 */