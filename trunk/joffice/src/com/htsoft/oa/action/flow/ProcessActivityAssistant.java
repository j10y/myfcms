...
public class ProcessActivityAssistant {
	...
	public static Map<String, ParamField> constructFieldMap(String processName, String activityName) {
		...
		try {
			...
			logger.warn("error when read the file from " + activityName
					+ "-fields.xml, the reason is not upload the ");
		}
		...
		if (is == null) {
			try {
				...
				logger
						.warn("error when read the file from 通用、表单-fields.xml, the reason is not upload the ");
			}
		}
		...
		for (Element el : els) {
			...
			Integer length = Integer.valueOf((attLength == null) ? 0 : new Integer(attLength
					.getValue()).intValue());
			Short isShowed = Short.valueOf((short) (((attIsShowed == null) || ("true"
					.equals(attIsShowed.getValue()))) ? 1 : 0));
