...
public class DocFolderAction extends BaseAction {
	...
	public String list() {
		...
		List<DocFolder> docList = this.docFolderService.getUserFolderByParentId(curUserId, Long
				.valueOf(0L));
	...
	public String select() {
		...
		for (DocFolder docFolder : docList) {
			List<Integer> rights = this.docPrivilegeService.getRightsByFolder(appUser, docFolder
					.getFolderId());
			...
			if (right == NOT_RIGHT) {
				buff.append("{id:'" + docFolder.getFolderId()).append(
						"',disabled:true,text:'" + docFolder.getFolderName()).append("',");
				buff.append(findChildsFolderByRight(docFolder.getFolderId(), right, false));
			} else {
				buff.append("{id:'" + docFolder.getFolderId()).append(
						"',text:'" + docFolder.getFolderName()).append("',");
				if (right == ALL_RIGHT)
	...
	public String findChildsFolderByRight(Long parentId, Integer right, boolean isAllRight) {
		...
		for (DocFolder folder : list) {
			...
			if (isAllRight) {
				...
				in = NOT_RIGHT;
				AppUser appUser = ContextUtil.getCurrentUser();
				List<Integer> rights = this.docPrivilegeService.getRightsByFolder(appUser, folder
						.getFolderId());
			...
			if (in == NOT_RIGHT) {
				sb.append("{id:'" + folder.getFolderId() + "',disabled:true,text:'"
	...
	public String remove() {
		...
		if (StringUtils.isNotEmpty(folderId)) {
			...
			List<DocFolder> docFolderList = this.docFolderService.getFolderLikePath(tmpFolder
					.getPath());
	...
	public String save() {
		if (this.docFolder.getFolderId() == null) {
			...
			if ((this.docFolder.getParentId() == null)
				...
				DocFolder pFolder = (DocFolder) this.docFolderService.get(this.docFolder
						.getParentId());
				if (pFolder != null) {
	...
	public String move() {
		...
		if ((StringUtils.isNotEmpty(strFolderIdOld)) && (StringUtils.isNotEmpty(strFolderIdNew))) {
			...
			if (folderIdNew.longValue() > 0L) {
				...
				boolean flag = Pattern.compile(folderOld.getPath()).matcher(folderNew.getPath())
						.find();
				if (flag) {
