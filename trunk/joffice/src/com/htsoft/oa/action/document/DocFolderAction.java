 ...
 public class DocFolderAction extends BaseAction
 {
   ...
   public String list()
   {
     ...
     List docList = this.docFolderService.getUserFolderByParentId(curUserId, Long.valueOf(0L));
   ...
   public String tree()
   {
     ...
     List docList = this.docFolderService.getPublicFolderByParentId(Long.valueOf(0L));
     for (DocFolder folder : docList) {
       buff.append("{id:'" + folder.getFolderId()).append("',text:'" + folder.getFolderName()).append("',");
   ...
   public String select()
   {
     ...
     for (DocFolder docFolder : docList) {
       List rights = this.docPrivilegeService.getRightsByFolder(appUser, docFolder.getFolderId());
   ...
   public String findChildsFolder(Long userId, Long parentId)
   {
     ...
     List list = this.docFolderService.getUserFolderByParentId(userId, parentId);
     if (list.size() == 0) {
   ...
   {
     StringBuffer sb = new StringBuffer();
     List list = this.docFolderService.getPublicFolderByParentId(parentId);
     if (list.size() == 0) {
       sb.append("leaf:true,expanded:true},");
       return sb.toString();
     }
   ...
   public String findChildsFolderByRight(Long parentId, Integer right, boolean isAllRight)
   {
     ...
     for (DocFolder folder : list) {
       ...
       if (isAllRight) {
         ...
         in = NOT_RIGHT;
         AppUser appUser = ContextUtil.getCurrentUser();
         List rights = this.docPrivilegeService.getRightsByFolder(appUser, folder.getFolderId());
   ...
   public String remove()
   {
     ...
     if (StringUtils.isNotEmpty(folderId)) {
       ...
       List docFolderList = this.docFolderService.getFolderLikePath(tmpFolder.getPath());
       ...
       for (DocFolder folder : docFolderList) {
         ...
         List priList = this.docPrivilegeService.getAll(filter);
   ...
   public String move()
   {
     ...
     if ((StringUtils.isNotEmpty(strFolderIdOld)) && (StringUtils.isNotEmpty(strFolderIdNew))) {
       ...
       List list = this.docFolderService.getFolderLikePath(oldPath);
       for (DocFolder folder : list) {
