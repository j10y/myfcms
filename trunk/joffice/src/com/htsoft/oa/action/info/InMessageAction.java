...
public class InMessageAction extends BaseAction {
	...
	public String list() {
		...
		List list = this.inMessageService.searchInMessage(appUser.getUserId(), this.inMessage,
		...
		Gson gson = new Gson();
		Type type = new TypeToken<InMessage>() {
		}.getType();
	...
	public String multiRemove() {
		...
		if (ids != null) {
			for (String id : ids) {
				this.inMessage = ((InMessage) this.inMessageService.get(Long.valueOf(Long
						.parseLong(id))));
	...
	public String reply() {
		...
		if (StringUtils.isNotEmpty(strReplyId)) {
			...
			buff.append(
					"{'messageId':" + this.inMessage.getShortMessage().getMessageId()
							+ ",'senderId':'" + this.inMessage.getShortMessage().getSenderId()
							+ "','sender':'" + this.inMessage.getShortMessage().getSender() + "'}")
					.append("]}");
			setJsonString(buff.toString());
		} else {
	...
	public String read() {
		...
		if (userId != null) {
			...
			if (this.inMessage != null) {
				...
				buff.append("{'receiveId':" + this.inMessage.getReceiveId() + ",'messageId':"
						...
						+ date + "','msgType':" + this.shortMessage.getMsgType());
				if (!flag) {
	...
	public String display() {
		...
		AppUser appUser = ContextUtil.getCurrentUser();
		List list = this.shortMessageService.searchShortMessage(appUser.getUserId(), null, null,
				null, pb);
		List inList = new ArrayList();
		StringBuffer buff = new StringBuffer("{success:true,'totalCounts':" + pb.getTotalItems()
				+ ",result:");
