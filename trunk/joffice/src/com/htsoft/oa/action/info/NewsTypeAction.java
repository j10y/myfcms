...
public class NewsTypeAction extends BaseAction {
	...
	public String tree() {
		...
		for (NewsType newsType : list) {
			sb.append("{id:'").append(newsType.getTypeId()).append("',text:'").append(
					newsType.getTypeName()).append("',leaf:true,iconCls:'menu-news'},");
		}
		if (!list.isEmpty()) {
		...
		if (this.newsType != null) {
			...
			if ((top != null) && (top.shortValue() > sn.shortValue())) {
				for (int i = 0; i < top.shortValue() - sn.shortValue(); ++i) {
					setNewsType(this.newsTypeService.findBySn(Short.valueOf((short) (sn
							.shortValue()
							+ i + 1))));
 ...
 * Qualified Name: com.htsoft.oa.action.info.NewsTypeAction JD-Core Version:
 * 0.5.4
 */