 ...
 public class DutyRegisterAction extends BaseAction
 {
   ...
   public String list()
   {
     ...
     List list = this.dutyRegisterService.getAll(filter);
   ...
   public String today()
   {
     ...
     if (dutySystem == null) {
       ...
       for (int i = 0; i < curDaySections.length; ++i) {
         ...
         DutySection dutySection = (DutySection)this.dutySectionService.get(new Long(curDaySections[i]));
 
         DutyRegister signInReg = this.dutyRegisterService.getTodayUserRegister(curUser.getUserId(), DutyRegister.SIGN_IN, dutySection.getSectionId());
         DutyRegister signOffReg = this.dutyRegisterService.getTodayUserRegister(curUser.getUserId(), DutyRegister.SIGN_OFF, dutySection.getSectionId());
         if (i > 0) sb.append(",");
   ...
   public String signIn()
   {
     ...
     this.dutyRegisterService.signInOff(new Long(sectionId), DutyRegister.SIGN_IN, ContextUtil.getCurrentUser(), new Date());
   ...
   {
     String sectionId = getRequest().getParameter("sectionId");
     this.dutyRegisterService.signInOff(new Long(sectionId), DutyRegister.SIGN_OFF, ContextUtil.getCurrentUser(), new Date());
     this.jsonString = "{success:true}";
     return "success";
   }
