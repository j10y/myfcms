 ...
 public class PhoneGroupAction extends BaseAction
 {
   ...
   public String list()
   {
     List list = this.phoneGroupService.getAll(ContextUtil.getCurrentUserId());
   ...
   public String multiDel()
   {
     ...
     if (ids != null) {
       for (String id : ids) {
         ...
         List list = this.phoneGroupService.findBySnDown(phoneGroup.getSn(), phoneGroup.getAppUser().getUserId());
         for (PhoneGroup pg : list) {
   ...
   public String save()
   {
     ...
     if (sn == null) sn = Integer.valueOf(0);
   ...
   public String move() {
     ...
     if (StringUtils.isNotEmpty(strGroupId)) {
       ...
       if ((opt.intValue() == 1) && 
         (sn.intValue() > 1)) {
         PhoneGroup pg = this.phoneGroupService.findBySn(Integer.valueOf(sn.intValue() - 1), userId);
         pg.setSn(sn);
         this.phoneGroupService.save(pg);
         this.phoneGroup.setSn(Integer.valueOf(sn.intValue() - 1));
         this.phoneGroupService.save(this.phoneGroup);
       ...
       if ((opt.intValue() == 3) && 
         (sn.intValue() > 1)) {
         List list = this.phoneGroupService.findBySnUp(sn, userId);
         for (PhoneGroup pg : list) {
           pg.setSn(Integer.valueOf(pg.getSn().intValue() + 1));
           this.phoneGroupService.save(pg);
         }
       ...
       if ((opt.intValue() == 4) && 
         (sn.intValue() < this.phoneGroupService.findLastSn(userId).intValue())) {
         List list = this.phoneGroupService.findBySnDown(sn, userId);
         for (PhoneGroup pg : list) {
           pg.setSn(Integer.valueOf(pg.getSn().intValue() - 1));
           this.phoneGroupService.save(pg);
         }
         this.phoneGroup.setSn(Integer.valueOf(this.phoneGroupService.findLastSn(userId).intValue() + 1));
         this.phoneGroupService.save(this.phoneGroup);
       }
