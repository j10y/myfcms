 ...
 public class MailFolderAction extends BaseAction
 {
   ...
   public String list()
   {
     ...
     List mailFolderList = this.mailFolderService.getAllUserFolderByParentId(curUserId, Long.valueOf(0L));
   ...
   public String findChildsFolder(Long userId, Long parentId) {
     ...
     List folders = this.mailFolderService.getUserFolderByParentId(userId, parentId);
     if (folders.size() == 0) {
   ...
   public String count()
   {
     MailFolder tmpFolder = (MailFolder)this.mailFolderService.get(new Long(this.folderId.longValue()));
     List mailFolderList = this.mailFolderService.getFolderLikePath(tmpFolder.getPath());
   ...
   public String remove()
   {
     ...
     if (this.folderId != null) {
       ...
       if ((count != null) && (new Long(count).longValue() > 0L)) {
         ...
         for (Iterator localIterator1 = mailFolderList.iterator(); localIterator1.hasNext(); ) { folder = (MailFolder)localIterator1.next();
           List mailBoxList = this.mailBoxService.findByFolderId(folder.getFolderId());
