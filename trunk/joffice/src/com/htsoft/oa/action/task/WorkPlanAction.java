...
public class WorkPlanAction extends BaseAction {
	...
	public String list() {
		...
		JSONSerializer serializer = JsonUtil.getJSONSerializer(new String[] { "startTime",
				"endTime" });
		buff.append(serializer.exclude(new String[] { "class", "appUser.password" })
				.serialize(list));
		buff.append("}");
	...
	public String personal() {
		...
		JSONSerializer serializer = JsonUtil.getJSONSerializer(new String[] { "startTime",
				"endTime" });
		buff.append(serializer.exclude(new String[] { "class", "appUser.password", "department" })
				.serialize(list));
		buff.append("}");
		this.jsonString = buff.toString();
		return "success";
	...
	public String department() {
		...
		List list = this.workPlanService.findByDepartment(this.workPlan, user, pb);
		StringBuffer buff = new StringBuffer("{success:true,'totalCounts':").append(
	...
	public String get() {
		WorkPlan workPlan = (WorkPlan) this.workPlanService.get(this.planId);
	...
	public String save() {
		...
		if (isPersonal != 1) {
			if (StringUtils.isNotEmpty(issueScopeIds)) {
				boolean b = this.planAttendService.deletePlanAttend(this.workPlan.getPlanId(),
						Short.valueOf(ISDEPARTMENT), Short.valueOf(NOTPRIMARY));
				if (b) {
					...
					for (int i = 0; i < strIssueScopeId.length; ++i) {
						if (StringUtils.isNotEmpty(strIssueScopeId[i])) {
							...
							pa.setDepartment((Department) this.departmentService.get(depId));
			...
			if (StringUtils.isNotEmpty(participantsIds)) {
				boolean b = this.planAttendService.deletePlanAttend(this.workPlan.getPlanId(),
						Short.valueOf(NOTDEPARTMENT), Short.valueOf(NOTPRIMARY));
				if (b) {
					...
					for (int i = 0; i < strParticipantsId.length; ++i) {
						if (StringUtils.isNotEmpty(strParticipantsId[i])) {
							...
							PlanAttend pa = new PlanAttend();
							pa.setAppUser((AppUser) this.appUserService.get(userId));
	...
	public String show() {
		...
		if (StringUtils.isNotEmpty(strPlanId)) {
			...
			this.workPlan = ((WorkPlan) this.workPlanService.get(planId));
		}
		return "show";
	}
