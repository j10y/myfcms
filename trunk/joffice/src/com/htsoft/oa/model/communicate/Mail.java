...
public class Mail extends BaseModel {
	...
	public boolean equals(Object object) {
		...
		return new EqualsBuilder().append(this.mailId, rhs.mailId).append(this.sender, rhs.sender)
				...
				.append(this.content, rhs.content).append(this.subject, rhs.subject).append(
						this.copyToNames, rhs.copyToNames).append(this.copyToIDs, rhs.copyToIDs)
				.append(this.recipientNames, rhs.recipientNames).append(this.recipientIDs,
						rhs.recipientIDs).append(this.mailStatus, rhs.mailStatus).append(
						this.fileIds, rhs.fileIds).append(this.filenames, rhs.filenames).isEquals();
	}

	public int hashCode() {
		return new HashCodeBuilder(-82280557, -700257973).append(this.mailId).append(this.sender)
				.append(this.importantFlag).append(this.sendTime).append(this.content).append(
						this.subject).append(this.copyToNames).append(this.copyToIDs).append(
						this.recipientNames).append(this.recipientIDs).append(this.mailStatus)
				.append(this.fileIds).append(this.filenames).toHashCode();
	}

	public String toString() {
		return new ToStringBuilder(this).append("mailId", this.mailId)
				.append("sender", this.sender).append("importantFlag", this.importantFlag).append(
						"sendTime", this.sendTime).append("content", this.content).append(
						"subject", this.subject).append("copyToNames", this.copyToNames).append(
						"copyToIDs", this.copyToIDs).append("recipientNames", this.recipientNames)
				.append("recipientIDs", this.recipientIDs).append("mailStatus", this.mailStatus)
				.append("fileIds", this.fileIds).append("filenames", this.filenames).toString();
	}
