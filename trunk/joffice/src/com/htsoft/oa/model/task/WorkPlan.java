...
public class WorkPlan extends BaseModel {
	...
	public boolean equals(Object object) {
		...
		return new EqualsBuilder().append(this.planId, rhs.planId).append(this.planName,
				rhs.planName).append(this.planContent, rhs.planContent).append(this.startTime,
				rhs.startTime).append(this.endTime, rhs.endTime).append(this.issueScope,
				rhs.issueScope).append(this.participants, rhs.participants).append(this.principal,
				rhs.principal).append(this.note, rhs.note).append(this.status, rhs.status).append(
				this.isPersonal, rhs.isPersonal).append(this.icon, rhs.icon).isEquals();
	}

	public int hashCode() {
		return new HashCodeBuilder(-82280557, -700257973).append(this.planId).append(this.planName)
				.append(this.planContent).append(this.startTime).append(this.endTime).append(
						this.issueScope).append(this.participants).append(this.principal).append(
						this.note).append(this.status).append(this.isPersonal).append(this.icon)
				.toHashCode();
	}

	public String toString() {
		return new ToStringBuilder(this).append("planId", this.planId).append("planName",
				this.planName).append("planContent", this.planContent).append("startTime",
				this.startTime).append("endTime", this.endTime).append("issueScope",
				this.issueScope).append("participants", this.participants).append("principal",
				this.principal).append("note", this.note).append("status", this.status).append(
				"isPersonal", this.isPersonal).append("icon", this.icon).toString();
	}
}
