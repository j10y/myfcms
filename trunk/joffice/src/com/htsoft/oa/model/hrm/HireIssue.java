...
public class HireIssue extends BaseModel {
	...
	public boolean equals(Object object) {
		...
		return new EqualsBuilder().append(this.hireId, rhs.hireId).append(this.title, rhs.title)
				.append(this.startDate, rhs.startDate).append(this.endDate, rhs.endDate).append(
						this.hireCount, rhs.hireCount).append(this.jobName, rhs.jobName).append(
						this.jobCondition, rhs.jobCondition).append(this.regFullname,
						rhs.regFullname).append(this.regDate, rhs.regDate).append(
						this.modifyFullname, rhs.modifyFullname).append(this.modifyDate,
						rhs.modifyDate).append(this.checkFullname, rhs.checkFullname).append(
						this.checkOpinion, rhs.checkOpinion).append(this.checkDate, rhs.checkDate)
				.append(this.status, rhs.status).append(this.memo, rhs.memo).isEquals();
	}

	public int hashCode() {
		return new HashCodeBuilder(-82280557, -700257973).append(this.hireId).append(this.title)
				.append(this.startDate).append(this.endDate).append(this.hireCount).append(
						this.jobName).append(this.jobCondition).append(this.regFullname).append(
						this.regDate).append(this.modifyFullname).append(this.modifyDate).append(
						this.checkFullname).append(this.checkOpinion).append(this.checkDate)
				.append(this.status).append(this.memo).toHashCode();
	}

	public String toString() {
		return new ToStringBuilder(this).append("hireId", this.hireId).append("title", this.title)
				.append("startDate", this.startDate).append("endDate", this.endDate).append(
						"hireCount", this.hireCount).append("jobName", this.jobName).append(
						"jobCondition", this.jobCondition).append("regFullname", this.regFullname)
