...
public class SalaryPayoff extends BaseModel {
	...
	public boolean equals(Object object) {
		...
		return new EqualsBuilder().append(this.recordId, rhs.recordId).append(this.fullname,
				rhs.fullname).append(this.userId, rhs.userId).append(this.profileNo, rhs.profileNo)
				.append(this.idNo, rhs.idNo).append(this.standAmount, rhs.standAmount).append(
						this.encourageAmount, rhs.encourageAmount).append(this.deductAmount,
						rhs.deductAmount).append(this.achieveAmount, rhs.achieveAmount).append(
						this.encourageDesc, rhs.encourageDesc).append(this.deductDesc,
						rhs.deductDesc).append(this.memo, rhs.memo).append(this.acutalAmount,
						rhs.acutalAmount).append(this.regTime, rhs.regTime).append(this.register,
						rhs.register).append(this.checkName, rhs.checkName).append(this.checkTime,
						rhs.checkTime).append(this.checkStatus, rhs.checkStatus).append(
						this.startTime, rhs.startTime).append(this.endTime, rhs.endTime).append(
						this.checkOpinion, rhs.checkOpinion)
				.append(this.standardId, rhs.standardId).isEquals();
	}

	public int hashCode() {
		return new HashCodeBuilder(-82280557, -700257973).append(this.recordId).append(
				this.fullname).append(this.userId).append(this.profileNo).append(this.idNo).append(
				this.standAmount).append(this.encourageAmount).append(this.deductAmount).append(
				this.achieveAmount).append(this.encourageDesc).append(this.deductDesc).append(
				this.memo).append(this.acutalAmount).append(this.regTime).append(this.register)
				.append(this.checkName).append(this.checkTime).append(this.checkStatus).append(
						this.startTime).append(this.endTime).append(this.checkOpinion).append(
						this.standardId).toHashCode();
	}

	public String toString() {
		return new ToStringBuilder(this).append("recordId", this.recordId).append("fullname",
				this.fullname).append("userId", this.userId).append("profileNo", this.profileNo)
				.append("idNo", this.idNo).append("standAmount", this.standAmount).append(
						"encourageAmount", this.encourageAmount).append("deductAmount",
						this.deductAmount).append("achieveAmount", this.achieveAmount).append(
						"encourageDesc", this.encourageDesc).append("deductDesc", this.deductDesc)
				.append("memo", this.memo).append("acutalAmount", this.acutalAmount).append(
						"regTime", this.regTime).append("register", this.register).append(
						"checkName", this.checkName).append("checkTime", this.checkTime).append(
						"checkStatus", this.checkStatus).append("startTime", this.startTime)
				.append("endTime", this.endTime).append("checkOpinion", this.checkOpinion).append(
						"standardId", this.standardId).toString();
	}
}
