...
public class InStock extends BaseModel {
	...
	public boolean equals(Object object) {
		...
		return new EqualsBuilder().append(this.buyId, rhs.buyId).append(this.providerName,
				rhs.providerName).append(this.stockNo, rhs.stockNo).append(this.price, rhs.price)
				.append(this.inCounts, rhs.inCounts).append(this.amount, rhs.amount).append(
						this.inDate, rhs.inDate).append(this.buyer, rhs.buyer).isEquals();
	}

	public int hashCode() {
		return new HashCodeBuilder(-82280557, -700257973).append(this.buyId).append(
				this.providerName).append(this.stockNo).append(this.price).append(this.inCounts)
				.append(this.amount).append(this.inDate).append(this.buyer).toHashCode();
	}

	public String toString() {
		return new ToStringBuilder(this).append("buyId", this.buyId).append("providerName",
				this.providerName).append("stockNo", this.stockNo).append("price", this.price)
				.append("inCounts", this.inCounts).append("amount", this.amount).append("inDate",
						this.inDate).append("buyer", this.buyer).toString();
	}
}
