...
public class CarApply extends BaseModel {
	...
	public boolean equals(Object object) {
		...
		return new EqualsBuilder().append(this.applyId, rhs.applyId).append(this.department,
				rhs.department).append(this.userFullname, rhs.userFullname).append(this.applyDate,
				rhs.applyDate).append(this.reason, rhs.reason)
				.append(this.startTime, rhs.startTime).append(this.endTime, rhs.endTime).append(
						this.proposer, rhs.proposer).append(this.userId, rhs.userId).append(
						this.mileage, rhs.mileage).append(this.oilUse, rhs.oilUse).append(
						this.notes, rhs.notes).append(this.approvalStatus, rhs.approvalStatus)
				.isEquals();
	}

	public int hashCode() {
		return new HashCodeBuilder(-82280557, -700257973).append(this.applyId).append(
				this.department).append(this.userFullname).append(this.applyDate).append(
				this.reason).append(this.startTime).append(this.endTime).append(this.proposer)
				.append(this.userId).append(this.mileage).append(this.oilUse).append(this.notes)
				.append(this.approvalStatus).toHashCode();
	}

	public String toString() {
		return new ToStringBuilder(this).append("applyId", this.applyId).append("department",
				this.department).append("userFullname", this.userFullname).append("applyDate",
				this.applyDate).append("reason", this.reason).append("startTime", this.startTime)
				.append("endTime", this.endTime).append("proposer", this.proposer).append("userId",
						this.userId).append("mileage", this.mileage).append("oilUse", this.oilUse)
				.append("notes", this.notes).append("approvalStatus", this.approvalStatus)
				.toString();
	}
}
