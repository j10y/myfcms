...
public class OfficeGoods extends BaseModel {
	...
	public boolean equals(Object object) {
		...
		return new EqualsBuilder().append(this.goodsId, rhs.goodsId).append(this.goodsName,
				rhs.goodsName).append(this.goodsNo, rhs.goodsNo).append(this.specifications,
				rhs.specifications).append(this.unit, rhs.unit).append(this.isWarning,
				rhs.isWarning).append(this.notes, rhs.notes).append(this.stockCounts,
				rhs.stockCounts).isEquals();
	}

	public int hashCode() {
		return new HashCodeBuilder(-82280557, -700257973).append(this.goodsId).append(
				this.goodsName).append(this.goodsNo).append(this.specifications).append(this.unit)
				.append(this.isWarning).append(this.notes).append(this.stockCounts).toHashCode();
	}

	public String toString() {
		return new ToStringBuilder(this).append("goodsId", this.goodsId).append("goodsName",
				this.goodsName).append("goodsNo", this.goodsNo).append("specifications",
				this.specifications).append("unit", this.unit).append("isWarning", this.isWarning)
				.append("notes", this.notes).append("stockCounts", this.stockCounts).toString();
	}
}
