...
public class Contract extends BaseModel {
	...
	public boolean equals(Object object) {
		...
		return new EqualsBuilder().append(this.contractId, rhs.contractId).append(this.contractNo,
				rhs.contractNo).append(this.subject, rhs.subject).append(this.contractAmount,
				rhs.contractAmount).append(this.mainItem, rhs.mainItem).append(this.salesAfterItem,
				rhs.salesAfterItem).append(this.validDate, rhs.validDate).append(this.expireDate,
				rhs.expireDate).append(this.serviceDep, rhs.serviceDep).append(this.serviceMan,
				rhs.serviceMan).append(this.signupUser, rhs.signupUser).append(this.signupTime,
				rhs.signupTime).append(this.creator, rhs.creator).append(this.createtime,
				rhs.createtime).append(this.consignAddress, rhs.consignAddress).append(
				this.consignee, rhs.consignee).isEquals();
	}

	public int hashCode() {
		return new HashCodeBuilder(-82280557, -700257973).append(this.contractId).append(
				this.contractNo).append(this.subject).append(this.contractAmount).append(
				this.mainItem).append(this.salesAfterItem).append(this.validDate).append(
				this.expireDate).append(this.serviceDep).append(this.serviceMan).append(
				this.signupUser).append(this.signupTime).append(this.creator).append(
				this.createtime).append(this.consignAddress).append(this.consignee).toHashCode();
	}

	public String toString() {
		return new ToStringBuilder(this).append("contactId", this.contractId).append("contractNo",
				this.contractNo).append("subject", this.subject).append("contractAmount",
				this.contractAmount).append("mainItem", this.mainItem).append("salesAfterItem",
				this.salesAfterItem).append("validDate", this.validDate).append("expireDate",
				this.expireDate).append("serviceDep", this.serviceDep).append("serviceMan",
				this.serviceMan).append("signupUser", this.signupUser).append("signupTime",
				this.signupTime).append("creator", this.creator).append("createtime",
				this.createtime).append("consignAddress", this.consignAddress).append("consignee",
				this.consignee).toString();
	}
}
