...
public class Product extends BaseModel {
	...
	public boolean equals(Object object) {
		...
		return new EqualsBuilder().append(this.productId, rhs.productId).append(this.productName,
				rhs.productName).append(this.productModel, rhs.productModel).append(this.unit,
				rhs.unit).append(this.costPrice, rhs.costPrice).append(this.salesPrice,
				rhs.salesPrice).append(this.productDesc, rhs.productDesc).append(this.createtime,
				rhs.createtime).append(this.updatetime, rhs.updatetime).isEquals();
	}

	public int hashCode() {
		return new HashCodeBuilder(-82280557, -700257973).append(this.productId).append(
				this.productName).append(this.productModel).append(this.unit)
				.append(this.costPrice).append(this.salesPrice).append(this.productDesc).append(
						this.createtime).append(this.updatetime).toHashCode();
	}

	public String toString() {
		return new ToStringBuilder(this).append("productId", this.productId).append("productName",
				this.productName).append("productModel", this.productModel).append("unit",
				this.unit).append("costPrice", this.costPrice)
				.append("salesPrice", this.salesPrice).append("productDesc", this.productDesc)
