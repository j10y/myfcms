...
public class Archives extends BaseModel {
	...
	public boolean equals(Object object) {
		...
		return new EqualsBuilder().append(this.archivesId, rhs.archivesId).append(this.typeName,
				rhs.typeName).append(this.archivesNo, rhs.archivesNo).append(this.issueDep,
				rhs.issueDep).append(this.subject, rhs.subject).append(this.issueDate,
				rhs.issueDate).append(this.status, rhs.status).append(this.shortContent,
				rhs.shortContent).append(this.fileCounts, rhs.fileCounts).append(this.privacyLevel,
				rhs.privacyLevel).append(this.urgentLevel, rhs.urgentLevel).append(this.issuer,
				rhs.issuer).append(this.issuerId, rhs.issuerId).append(this.keywords, rhs.keywords)
				.append(this.sources, rhs.sources).append(this.archType, rhs.archType).append(
						this.recDepIds, rhs.recDepIds).append(this.recDepNames, rhs.recDepNames)
				.append(this.orgArchivesId, rhs.orgArchivesId)
				.append(this.depSignNo, rhs.depSignNo).isEquals();
	}

	public int hashCode() {
		return new HashCodeBuilder(-82280557, -700257973).append(this.archivesId).append(
				this.typeName).append(this.archivesNo).append(this.issueDep).append(this.subject)
				.append(this.issueDate).append(this.status).append(this.shortContent).append(
						this.fileCounts).append(this.privacyLevel).append(this.urgentLevel).append(
						this.issuer).append(this.issuerId).append(this.keywords).append(
						this.sources).append(this.archType).append(this.recDepIds).append(
						this.recDepNames).append(this.orgArchivesId).append(this.depSignNo)
				.toHashCode();
	}

	public String toString() {
		return new ToStringBuilder(this).append("archivesId", this.archivesId).append("typeName",
				this.typeName).append("archivesNo", this.archivesNo).append("issueDep",
				this.issueDep).append("subject", this.subject).append("issueDate", this.issueDate)
				.append("status", this.status).append("shortContent", this.shortContent).append(
						"fileCounts", this.fileCounts).append("privacyLevel", this.privacyLevel)
				.append("urgentLevel", this.urgentLevel).append("issuer", this.issuer).append(
						"issuerId", this.issuerId).append("keywords", this.keywords).append(
						"sources", this.sources).append("archType", this.archType).append(
						"recDepIds", this.recDepIds).append("recDepNames", this.recDepNames)
				.append("orgArchivesId", this.orgArchivesId).append("depSignNo", this.depSignNo)
				.toString();
	}
