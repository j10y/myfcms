...
public class ArchDispatch extends BaseModel {
	...
	public boolean equals(Object object) {
		...
		return new EqualsBuilder().append(this.dispatchId, rhs.dispatchId).append(
				this.dispatchTime, rhs.dispatchTime).append(this.userId, rhs.userId).append(
				this.fullname, rhs.fullname).append(this.isRead, rhs.isRead).append(this.subject,
				rhs.subject).append(this.readFeedback, rhs.readFeedback).append(this.archUserType,
				rhs.archUserType).append(this.disRoleId, rhs.disRoleId).append(this.disRoleName,
				rhs.disRoleName).isEquals();
	}

	public int hashCode() {
		return new HashCodeBuilder(-82280557, -700257973).append(this.dispatchId).append(
				this.dispatchTime).append(this.userId).append(this.fullname).append(this.isRead)
				.append(this.subject).append(this.readFeedback).append(this.archUserType).append(
						this.disRoleId).append(this.disRoleName).toHashCode();
	}

	public String toString() {
		return new ToStringBuilder(this).append("dispatchId", this.dispatchId).append(
				"dispatchTime", this.dispatchTime).append("userId", this.userId).append("fullname",
				this.fullname).append("isRead", this.isRead).append("subject", this.subject)
				.append("readFeedback", this.readFeedback).append("isUndertake", this.archUserType)
				.append("disRoleId", this.disRoleId).append("disRoleName", this.disRoleName)
				.toString();
	}
}
