...
public class OrderUtil {
	...
	public static boolean updateOrder(String table, String type, String targetOrder, String orders,
			String wherePart) {
		return updateOrder(table, "OrderFlag", type, targetOrder, orders, wherePart);
	}

	public static boolean updateOrder(String table, String column, String type, String targetOrder,
			String orders, String wherePart) {
		return updateOrder(table, column, type, targetOrder, orders, wherePart, null);
	}

	public static boolean updateOrder(String table, String column, String type, String targetOrder,
			String orders, String wherePart, Transaction tran) {
		if ((StringUtil.isEmpty(targetOrder)) || (targetOrder.length() < 13)) {
			targetOrder = String.valueOf(getDefaultOrder());
		}
			...
			if (arr[i] == target) {
				...
				if (flag) {
					...
					for (j = 0; j < arr.length; ++j) {
						if (j != i) {
							qb = new QueryBuilder("update " + table + " set " + column
									+ "=? where " + column + "=?", (target - d) * 10L);
							--d;
						} else {
							qb = new QueryBuilder("update " + table + " set " + column
									+ "=? where " + column + "=?", target * 10L);
						}
						qb.add(arr[j]);
						tran.add(qb);
					}
					...
					for (j = 0; j < i; ++j) {
						...
						qb = new QueryBuilder("update " + table + " set " + column + "=" + column
								+ "-? where " + column + " between ? and ? and " + wherePart);
						qb.add(j + 1);
						qb.add(arr[j]);
						qb.add(arr[(j + 1)]);
						tran.add(qb);
				...
				qb = new QueryBuilder("update " + table + " set " + column + "=" + column
						+ "/10 where " + column + ">? and " + wherePart, target * 9L);
				tran.add(qb);
				if (bFlag) {
					return true;
				}
 ...
 * com.zving.platform.pub.OrderUtil JD-Core Version: 0.5.3
 */