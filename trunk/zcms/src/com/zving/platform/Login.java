 ...
 public class Login extends Page
 {
   ...
   public void submit() {
     ...
     if ((!(Config.isAllowLogin)) && (!(user.getUserName().equalsIgnoreCase("admin")))) {
       UserLog.log("Log", "Login", "临时禁止登录.用户名" + $V("UserName"), this.Request.getClientIP(), $V("UserName"));
     ...
     if ((userSet == null) || (userSet.size() < 1)) {
       UserLog.log("Log", "Login", "登陆失败.用户名：" + $V("UserName"), this.Request.getClientIP(), $V("UserName"));
       ...
       if ((!("admin".equalsIgnoreCase(user.getUserName()))) && 
         ...
         UserLog.log("Log", "Login", "登陆失败.用户名：" + $V("UserName") + "已停用", this.Request.getClientIP(), $V("UserName"));
   ...
   public static DataCollection getAllPriv(DataCollection Response) {
     if ("admin".equalsIgnoreCase(User.getUserName())) {
       ...
       Response.put("roleCodes", (roleCodeList == null) ? "" : StringUtil.join(roleCodeList.toArray()));
       ...
       for (int i = 0; i < Priv.PRIV_MAP.size(); ++i) {
         if ("menu".equals(ks[i])) continue; if ("site".equals(ks[i])) {
           continue;
         }
         qb = new QueryBuilder(
       ...
       for (i = 0; i < roleCodeList.size(); ++i) {
         ...
         for (int j = 0; j < Priv.PRIV_MAP.size(); ++j) {
           if ("menu".equals(ks[j])) continue; if ("site".equals(ks[j])) {
             continue;
           }
           qb = new QueryBuilder(
             "select ID,Code,Value from ZDPrivilege where OwnerType =? and Owner=? and PrivType=? and exists (select '' from ZCCatalog where SiteID =? and ZCCatalog.InnerCode = ZDPrivilege.ID ) ");
           ...
           qb.add(ks[j]);
           qb.add(SiteID);
           Response.put(roleCodeList.get(i) + ks[j].toString() + "DT", qb.executeDataTable());
         }
       }
     }
     return Response;
