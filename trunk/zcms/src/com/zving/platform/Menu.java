 ...
 public class Menu extends Page
 {
   ...
   public void del() {
     ...
     for (int i = 0; i < set.size(); ++i) {
       ...
       if (menu.getParentID() == 0L) {
         ...
         if (count > 0L) {
           this.Response.setStatus(0);
           UserLog.log("System", "DelMenu", "删除菜单" + menu.getName() + "失败", this.Request.getClientIP());
   ...
   public void sortMenu() {
     ...
     for (int i = 0; i < parentDT.getRowCount(); ++i) {
       ...
       DataTable childDT = new QueryBuilder("select * from zdMenu where parentID = ? order by orderflag,id", parentDT.get(i, "ID")).executeDataTable();
     ...
     if ("before".equalsIgnoreCase(ordertype)) {
       i = 0; break label342:
       do { if (DT.getString(i, "ID").equals(nextMenu)) {
           m = 0; break label275:
           do { branchList.add(orderDT.getDataRow(m));
 
             ++m; label275: if (orderDT == null) break label324;  }
         ...
         label324: branchList.add(DT.getDataRow(i));
 
         label339: ++i; label342: if (DT == null) break label549;  }
     ...
     for (int i = 0; i < branchList.size(); ++i) {
       label549: DataRow dr = (DataRow)branchList.get(i);
       trans.add(new QueryBuilder("update zdmenu set orderflag = ? where ID = ?", i, dr.getString("ID")));
     }
     if (trans.commit())
 ...
 * Qualified Name:     com.zving.platform.Menu
