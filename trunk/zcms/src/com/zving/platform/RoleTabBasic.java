 ...
 public class RoleTabBasic extends Page
 {
   public static Mapx init(Mapx params)
   {
     ...
     map.put("BranchName", new QueryBuilder("select name from zdbranch where BranchInnerCode=? Order by OrderFlag", role.getBranchInnerCode()).executeString());
     return map;
   }
   ...
   public void del()
   {
     ...
     trans.add(new ZDPrivilegeSchema().query(new QueryBuilder("where OwnerType=? and Owner=?", "R", RoleCode)), 5);
   ...
   public static void dg1DataBind(DataGridAction dga)
   {
     ...
     QueryBuilder qb1 = new QueryBuilder("select * from ZDUser where UserName in (select UserName from ZDUserRole b where b.RoleCode=?)", RoleCode);
     QueryBuilder qb2 = new QueryBuilder("select count(*) from ZDUser where UserName in (select UserName from ZDUserRole b where b.RoleCode =?)", RoleCode);
     if (dga.getTotal() == 0) {
       dga.setTotal(qb2);
     }
   ...
   public static void bindUserList(DataGridAction dga)
   {
     ...
     conditions.append("%'");
     conditions.append(" and not exists (select '' from zduserrole where zduserrole.roleCode='" + roleCode + "' and zduserrole.userName=zduser.userName)");
   ...
   public void delUserFromRole()
   {
     ...
     for (int i = 0; i < UserNames.length; ++i) {
       DataTable dt = new QueryBuilder("select RoleCode from ZDUserRole where UserName=? and RoleCode!=?", UserNames[i], RoleCode).executeDataTable();
