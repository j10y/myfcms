				...
				if ((this.doc.getLevel() == this.level - 1) && (this.doc.isTextContent())) {
					...
					for (int k = 0; k < urls.length; ++k) {
						...
						if ((!(this.extracter.isMatchedUrl(url2, this.doc.getUrl())))
								|| (this.list.containsKey(url2)))
							continue;
						if ((this.list.getCrawler().getConfig().getMaxPageCount() > 0)
								&& (this.list.size() >= this.list.getCrawler().getConfig()
										.getMaxPageCount())) {
							break;
						}
					...
					long percent = Math.round(this.extracter.extractedCount * 10000.0D
							/ this.extracter.size);
					long p = percent % 100L;
					...
					LogUtil.getLogger().info(
							"Extracting,Thread " + this.threadIndex + "\tL" + this.doc.getLevel()
									+ "\t" + (percent / 100L) + "." + pStr + "%");
				}
				this.doc = this.list.next();
			} else {
 ...
 * com.zving.search.crawl.UrlExtracterThread JD-Core Version: 0.5.3
 */