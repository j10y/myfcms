 ...
 public class LastVisitReport extends Page
 {
   ...
   public static DataTable getLastVisit(long siteID, int pageSize, int pageIndex)
   {
     ...
     if ((q == null) || (q.size() == 0)) {
       dt = 
         new QueryBuilder("select * from ZCVisitLog where SiteID=? order by AddTime desc", siteID).executePagedDataTable(pageSize, pageIndex);
     } else {
       Visit[] arr = (Visit[])null;
       synchronized (q) {
         arr = new Visit[(q.size() > pageSize) ? pageSize : q.size()];
         for (i = q.size() - 1; i >= q.size() - arr.length; --i) {
       ...
       for (int i = arr.length - 1; i >= 0; --i) {
         ...
         dt.insertRow(new Object[] { v.URL, v.IP, v.OS, v.Browser, v.Screen, district, new Date(v.VisitTime) });
       }
       dt.sort("AddTime");
     }
