 ...
 public class VoteReport extends Ajax
 {
   public void getVoteData()
   {
     ...
     DataTable dt = new QueryBuilder("select item,score from zcvoteitem where subjectid = ?", subjectID).executeDataTable();
   ...
   public static DataTable getList1(Mapx params, DataRow parentDR)
   {
     ...
     for (int i = 0; i < dt.getRowCount(); ++i) {
       DataTable dt1 = new QueryBuilder("select * from zcvoteitem where voteID =? and SubjectID=? order by ID", voteID, dt.get(i, "ID")).executeDataTable();
   ...
   public static DataTable getList2(Mapx params, DataRow parentDR) {
     String voteID = params.getString("ID");
     DataTable dt = new QueryBuilder("select * from zcvoteitem where voteID =? and SubjectID=? order by ID", voteID, parentDR.getString("ID")).executeDataTable();
 ...
 * Qualified Name:     com.zving.cms.stat.report.VoteReport
