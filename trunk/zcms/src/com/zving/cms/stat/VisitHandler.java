 ...
 public class VisitHandler
 {
             ...
             public void run() { LogUtil.getLogger().info("正在更新统计信息......");
               ...
               try {
                 for (int i = 0; i < VisitHandler.items.length; ++i)
                   VisitHandler.items[i].update(VisitHandler.this, current, this.val$mArr[i], this.val$isNewPeriod);
               }
               catch (Throwable t) {
                 t.printStackTrace();
               } finally {
   ...
   public static void deal(HttpServletRequest request, HttpServletResponse response)
   {
     ...
     try
     {
       ...
       try {
         response.getWriter().println("<br><br><div style='font-size:9pt'>&nbsp;&nbsp;错误的SiteID。</div>");
