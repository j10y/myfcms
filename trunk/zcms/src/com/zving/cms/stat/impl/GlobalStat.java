	...
	public void deal(Visit v) {
		if ("Unload".equals(v.Event)) {
			...
			if (v.UVFlag) {
				...
				if (v.Frequency > 0) {
					String item = String.valueOf(v.Frequency);
					if (v.Frequency > 30) {
						item = "&gt;30";
					}
			...
			if (obj == null) {
				...
				if (time - lastIDClearTime >= 900000L) {
					...
					for (int i = 0; i < ks.length; ++i) {
						...
						long avgStickTime = VisitCount.getInstance().get(time, "Global",
								"StickTime", "0");
						String item = String.valueOf(vh.Depth);
						if (vh.Depth > 30) {
							item = "&gt;30";
						}
						VisitCount.getInstance().add(v.SiteID, "Global", "VisitDepth", item);
						VisitCount.getInstance().addAverage(v.SiteID, "Global", "StickTotalTime",
								"0", avgStickTime + (vh.LastTime - vh.StartTime) / 1000L);
				...
				if (time - vh.LastTime < 900000L) {
					...
					long avgStickTime = VisitCount.getInstance().get(time, "Global", "StickTime",
							"0");
					String item = String.valueOf(vh.Depth);
					if (vh.Depth > 30) {
						item = "&gt;30";
 ...
 * com.zving.cms.stat.impl.GlobalStat JD-Core Version: 0.5.3
 */