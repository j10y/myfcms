 ...
 public class PubFun
 {
   ...
   {
     DataTable dt = new QueryBuilder("select Name,BranchInnerCode,TreeLevel from ZDBranch where BranchInnerCode like ? order by OrderFlag", User.getBranchInnerCode() + "%").executeDataTable();
     indentDataTable(dt);
   ...
   public static void initUserRoleMap(String userName)
   {
     ...
     DataTable dt = new QueryBuilder("select RoleCode from zduserrole where UserName = ?", userName).executeDataTable();
   ...
   public static String getCurrentPage(long catalogID, int level, String separator, String target)
   {
     ...
     if (!("0".equals(parentID))) {
       ...
       url = url + " " + separator + " <a href='" + levelString + catalogPath + "' target='" + target + "'>" + linkText + "</a>";
     } else {
       url = url + " " + separator + " <a href='" + levelString + catalogPath + "' target='" + target + "'>" + linkText + "</a>";
       linkText = "首页";
       url = "<a href='" + levelString + "' target='" + target + "'>" + linkText + "</a>" + url;
     }
   ...
   {
     return getArticleURL(articleID);
   }
   ...
   public static String getArticleURL(ZCSiteSchema site, ZCCatalogSchema catalog, ZCArticleSchema article) {
     HtmlNameParser nameParser = new HtmlNameParser(site.toDataRow(), catalog.toDataRow(), article.toDataRow(), catalog.getDetailNameRule());
   ...
   public static String getGoodsURL(String ID) {
     DataTable dt = new QueryBuilder("select catalogID from zcgoods where ID=?", ID).executeDataTable();
   ...
   public static String getImagePlayer(ZCImagePlayerSchema imagePlayer) {
     QueryBuilder qb = new QueryBuilder("select b.* from ZCImageRela a,zcimage b where a.id = b.id  and a.RelaID=? and a.RelaType=? order by a.orderflag desc, a.addtime desc", imagePlayer.getID(), "ImagePlayerImage");
     ...
     StringBuffer sb = new StringBuffer();
     sb.append("<script type=\"text/javascript\" src=\"" + Config.getContextPath() + "Tools/Swfobject.js\"></script>\n");
     sb.append("<div id='" + id + "'>\n");
     sb.append("  This text is replaced by the Flash movie.\n");
     sb.append("</div>\n");
     sb.append("<script type='text/javascript'>\n");
     sb.append("var so=new SWFObject('" + Config.getContextPath() + "Tools/ImagePlayer.swf','ImagePlayer1'," + imagePlayer.getWidth() + "," + (imagePlayer.getHeight() + 22L) + ",'7','#FFFFFF','high');\n");
     ...
     if (StringUtil.isNotEmpty(Province)) {
       ...
       return District.filter(new Filter(Province) {
         public boolean filter(Object o) {
           ...
           return (("2".equals(dr.getString("TreeLevel"))) && (dr.getString("Code").substring(0, 2).equals(((String)this.Param).substring(0, 2))));
         }
     ...
     if (StringUtil.isNotEmpty(City)) {
       ...
       if ("0".equalsIgnoreCase(district.getType())) {
         return District.filter(new Filter(City) {
           public boolean filter(Object o) {
             ...
             return (("3".equals(dr.get("TreeLevel").toString())) && (dr.get("Code").toString().substring(0, 3).equals(((String)this.Param).substring(0, 3))));
           }
       ...
       return District.filter(new Filter(City) {
         public boolean filter(Object o) {
           ...
           return (("3".equals(dr.getString("TreeLevel"))) && (dr.get("Code").toString().substring(0, 4).equals(((String)this.Param).substring(0, 4))));
         }
