 ...
 public class Link extends Page
 {
   ...
   public void edit()
   {
     ...
     if (StringUtil.isNotEmpty(ImageID)) {
       DataTable dt = new QueryBuilder("select path,srcfilename from zcimage where id=?", ImageID).executeDataTable();
   ...
   public void getPicSrc()
   {
     ...
     if (dt.getRowCount() > 0)
       this.Response.put("picSrc", Config.getContextPath() + Config.getValue("UploadDir") + "/" + 
         SiteUtil.getAlias(Application.getCurrentSiteID()) + "/" + dt.get(0, "path").toString() + 
         dt.get(0, "srcfilename").replaceAll("//", "/").toString());
   }
