...
public class Magazine extends Page {
	...
	public static Mapx initIssue(Mapx params) {
		String sql = "select concat(year,'年',periodNum,'期') as Name,ID from zcmagazineissue where MagazineID=(select min(id) from zcmagazine where siteid="
				+ Application.getCurrentSiteID() + ") order by id desc";
	...
	public void del() {
		...
		trans.add(new ZCMagazineIssueSchema().query(new QueryBuilder(" where magazineID =?", ID)),
				5);

		trans.add(new ZCMagazineCatalogRelaSchema().query(new QueryBuilder(" where magazineID =?",
				ID)), 5);
 ...
 * com.zving.cms.site.Magazine JD-Core Version: 0.5.3
 */