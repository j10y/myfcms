...
public class Site extends Page {
	...
	public static Mapx initDialog(Mapx params) {
		...
		if (o1 != null) {
			...
			params.put("radioAutoIndexFlag", HtmlUtil.codeToRadios("AutoIndexFlag", "YesOrNo",
					indexFlag));
			...
			params.put("radioAutoStatFlag", HtmlUtil.codeToRadios("AutoStatFlag", "YesOrNo",
					statFlag));

			String auditFlag = site.getCommentAuditFlag();
			params.put("radioCommentAuditFlag", HtmlUtil.codeToRadios("CommentAuditFlag",
					"YesOrNo", auditFlag));
		...
		params.put("radioCommentAuditFlag", HtmlUtil.codeToRadios("CommentAuditFlag", "YesOrNo",
				"Y"));
		return params;
	}
	...
	public static void updatePrivAndFile(String alias) {
		...
		String path = Config.getContextRealPath() + Config.getValue("UploadDir") + "/" + alias
				+ "/upload/Image".replaceAll("//", "/");
		...
		String templatePath = Config.getContextRealPath() + Config.getValue("Statical.TemplateDir")
				+ "/" + alias + "/template";
		...
		String imagePath = Config.getContextRealPath() + Config.getValue("Statical.TemplateDir")
				+ "/" + alias + "/images";
		...
		FileUtil.copy(Config.getContextRealPath() + "/Tools/ImagePlayer.swf", imagePath
				+ "/ImagePlayer.swf");
		FileUtil.copy(Config.getContextRealPath() + "/Tools/player.swf", imagePath + "/player.swf");
		FileUtil.copy(Config.getContextRealPath() + "/Tools/Swfobject.js", imagePath
				+ "/Swfobject.js");
	...
	public static void addDefaultCatalogLib(long siteID, Transaction trans, String catalogType,
			String[] catalogNames) {
	...
	public void del() {
		...
		if (trans.commit()) {
			...
			if (ids.indexOf(String.valueOf(Application.getCurrentSiteID())) >= 0) {
	...
	public void delSiteRela(String siteIDs, Transaction trans) {
		...
		for (int i = 0; i < trs.length; ++i) {
			...
			if (tr.isExportData)
				deleteRelaTableData(tr.TableCode, tr.KeyField, tr.RelaTable, tr.RelaField, siteIDs,
						trans);
		}
	}
	...
	public void deleteTableData(String tableName, String siteIDs, Transaction trans) {
		...
		String backup = "insert into b" + tableName + " select " + tableName + ".*,'" + backupNO
				+ "','" + backupOperator + "','" + backupTime + "',null from " + tableName
	...
	public void deleteRelaTableData(String tableName, String foreinKey, String relaTableName,
			String relaField, String siteIDs, Transaction trans) {
		String wherePart = " where exists(select '' from " + relaTableName + " where " + relaField
		...
		try {
			...
			while (iter.hasNext()) {
				...
				if (!(item.isFormField())) {
					...
					String FileName = "SiteUpload_" + DateUtil.getCurrentDate("yyyyMMddHHmmss")
							+ ".dat";
	...
	public void importStep2DataBind(DataGridAction dga) {
		...
		if (dga.getPageIndex() == 0) {
			...
			dt.set(0, "Name", "<input class='inputText' style='width:150px' id='Name' value='"
					+ map.getString("Name") + "'>");
			dt.set(0, "Alias", "<input class='inputText' style='width:100px' id='Alias' value='"
					+ map.getString("Alias") + "'>");
			dt.set(0, "URL", "<input class='inputText' style='width:250px' id='URL' value='"
					+ map.getString("URL") + "'>");
		}
		dga.bindData(dt);
	}
	...
	public void checkImportSite() {
		...
		QueryBuilder qb = new QueryBuilder("select count(1) from ZCSite where Alias=? and ID<>?",
				Alias, ID);
		if (qb.executeInt() != 0) {
 ...
 * com.zving.cms.site.Site JD-Core Version: 0.5.3
 */