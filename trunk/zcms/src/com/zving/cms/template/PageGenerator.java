 ...
 public class PageGenerator
 {
   ...
   public PageGenerator(LongTimeTask ltt)
   {
     ...
     this.templateDir = contextRealPath + Config.getValue("Statical.TemplateDir").replace('\\', '/');
   ...
   public boolean staticCatalog(ZCCatalogSchema catalog, boolean isGenerateDetail, int publishPages)
   {
     ...
     if ((StringUtil.isNotEmpty(indexTemplate)) && (!("Y".equals(catalog.getSingleFlag())))) {
       indexTemplate = this.templateDir + "/" + siteAlias + "/" + indexTemplate;
       ...
       if (tp == null) {
         this.task.addError("首页模板" + indexTemplate + "不存在");
         return false;
       }
       ...
       if ("Y".equals(site.getAutoStatFlag()))
       {
         ...
         String statUrl = "SiteID=" + site.getID() + "&Type=Article&CatalogInnerCode=" + catalog.getInnerCode() + "&Dest=" + serviceUrl + "/Stat.jsp";
         statScript = getStatScript(statUrl);
       }
 
       statScript = statScript + "\n<!-- Published by " + this.product.getAppCode() + "(" + this.product.getAppName() + ") " + this.product.getMainVersion() + " PublishDate " + DateUtil.getCurrentDateTime() + "-->";
     ...
     if (StringUtil.isNotEmpty(rssTemplate)) {
       rssTemplate = this.templateDir + "/" + siteAlias + "/" + rssTemplate;
       ...
       if (tp != null) {
         ...
         tp.define("TemplateData", catalogTemplateData);
         tp.define("catalog", catalogTemplateData.getCatalog());
         this.task.setCurrentInfo("正在处理栏目\"" + catalog.getName() + "\"");
         if (!(writeHTML(tp, path, "rss.xml", "")))
           return false;
       }
       else {
         this.task.addError("栏目“" + catalog.getName() + "”Rss模板" + rssTemplate + "不存在");
         return false;
       }
     }
     ...
     if (!("Y".equals(catalog.getSingleFlag()))) {
       ...
       if (StringUtil.isEmpty(listTemplate)) {
         this.task.addError("栏目“" + catalog.getName() + "”列表页模板不能为空");
       }
 
       listTemplate = this.templateDir + "/" + siteAlias + "/" + listTemplate;
       listTemplate = listTemplate.replaceAll("//", "/");
       listParser = getParser(site.getID(), listTemplate, level);
       if (listParser == null) {
         this.task.addError("栏目“" + catalog.getName() + "”列表页模板" + listTemplate + "不存在");
         return true;
       }
     ...
     if ((StringUtil.isEmpty(catalog.getDetailTemplate())) && 
       (!("Y".equals(catalog.getSingleFlag()))) && (StringUtil.isEmpty(catalog.getIndexTemplate()))) {
       this.task.addError("栏目“" + catalog.getName() + "”详细页模板不能为空");
     }
 
     String detailTemplate = this.templateDir + "/" + siteAlias + "/" + catalog.getDetailTemplate();
     ...
     if (!("Y".equals(catalog.getSingleFlag()))) {
       ...
       pageSize = (obj == null) ? (int)catalog.getListPageSize() : Integer.parseInt(obj.toString());
     ...
     if ((type == 1) || (type == 3) || (type == 2)) {
       ...
       {
         wherePart = " where " + catalogWherePart + " and status in(" + 20 + "," + 30 + ") and publishdate<='" + DateUtil.getCurrentDateTime() + "'  order by topflag desc,orderflag desc,publishdate desc,id desc";
       } else {
         String allowStatus = CatalogUtil.getAllowStatus(catalog.getID());
         if (StringUtil.isEmpty(allowStatus)) {
           allowStatus = "20,30";
         }
         wherePart = " where " + catalogWherePart + " and status in(" + allowStatus + ") and publishdate<='" + DateUtil.getCurrentDateTime() + "'  order by topflag desc,orderflag desc,publishdate desc,id desc";
       }
       ...
       if ("Y".equals(catalog.getSingleFlag())) {
         ...
         if (dtArticle.getRowCount() > 0) {
           ...
           if ("1".equals(article.getString("TemplateFlag"))) {
             ...
             detailTemplateName = this.templateDir + "/" + siteAlias + detailTemplateName;
           ...
           article.set("Content", article.getString("Content").replaceAll(imagePath, CatalogUtil.getLevelStr(detailLevel)));
 
           HtmlNameParser nameParser = new HtmlNameParser(site.toDataRow(), catalog.toDataRow(), article, catalog.getDetailNameRule());
           HtmlNameRule h = nameParser.getNameRule();
           String dirPath = rootPath + h.getDirPath();
           staticDoc("Article", detailParser, detailTemplateData, article, dirPath, h.getFileName());
         }
       } else {
         total = new QueryBuilder("select count(1) from zcarticle " + wherePart.substring(0, wherePart.indexOf("order by")), param).executeInt();
         if (total > 0) {
           ...
           if (publishPages != 0) {
             fetchCount = (publishPages % batchPages == 0) ? publishPages / batchPages : publishPages / batchPages + 1;
             batchPages = (publishPages > batchPages) ? batchPages : publishPages;
           }
           ...
           {
             qb = new QueryBuilder("select " + cols + " from zcarticle " + wherePart, param);
             ...
             for (int j = 0; j < dtArticleSize; ++j) {
               ...
               if ("1".equals(article.getString("TemplateFlag"))) {
                 ...
                 detailParser = getParser(site.getID(), detailTemplateName, level);
                 if (detailParser == null) {
                   this.task.addError("文章详细页指定模板：" + detailTemplateName + " 不存在");
                   detailParser = catalogDetailParser;
                 }
               } else {
                 detailParser = catalogDetailParser;
               ...
               if ("4".equals(article.getString("Type"))) {
                 dtArticle.getDataRow(j).set("Link", article.getString("RedirectURL"));
               } else {
                 DataRow catalogRow = null;
                 boolean currentCatalog = catalog.getID() == article.getLong("CatalogID");
                 if (currentCatalog) {
                   catalogRow = catalog.toDataRow();
                 }
 
                 HtmlNameParser nameParser = new HtmlNameParser(site.toDataRow(), catalogRow, article, catalog.getDetailNameRule());
                 HtmlNameRule h = nameParser.getNameRule();
                 level = h.getLevel();
                 ...
                 if ((isGenerateDetail) && (currentCatalog)) {
                   ...
                   article.set("Content", article.getString("Content").replaceAll(imagePath, 
                     detailTemplateData.getLevelStr()));
 
                   article.set("HitCount", getClickScript(article.getString("ID")));
                   ...
                   if (!(staticDoc("Article", detailParser, detailTemplateData, article, dirPath, h.getFileName()))) {
                     return false;
                   }
                 }
             ...
             for (int k = 0; k < batchPages; ++k) {
               ...
               catalogTemplateData.setListTable(new DataTable(dtArticle.getDataColumns(), values));
               ...
               if (!(writeListHTML(site.getID(), listParser, catalogTemplateData, dirPath, listNameRule, pageIndex, statScript))) {
                 return false;
               }
               ++pageIndex;
             }
           ...
           if ("Y".equals(site.getAutoStatFlag()))
           {
             serviceUrl = Config.getValue("ServicesContext");
             statUrl = "SiteID=" + site.getID() + "&Type=Article&CatalogInnerCode=" + catalog.getInnerCode() + 
               "&Dest=" + serviceUrl + "/Stat.jsp";
             statScript = getStatScript(statUrl);
           }
 
           String dirPath = rootPath + CatalogUtil.getPath(catalog.getID());
           if (!(writeListHTML(site.getID(), listParser, catalogTemplateData, dirPath, listNameRule, 0, statScript)))
             return false;
         }
       }
     }
     else
     {
       ...
       if (type == 4) {
         ...
         ZCImageSet set = image.query(new QueryBuilder(" where " + catalogWherePart + " order by id desc", param));
         ...
         if ((dt != null) && (dt.getRowCount() > 0)) {
             ...
             for (int j = 0; j < dt.getRowCount(); ++j) {
               ...
               if ((imageRelaTable != null) && (imageRelaTable.getRowCount() > 0)) {
                 ...
                 if (StringUtil.isNotEmpty(nameRule)) {
                   HtmlNameParser nameParser = new HtmlNameParser(site.toDataRow(), null, imageRelaTable.get(0), nameRule);
                   HtmlNameRule h = nameParser.getNameRule();
                   row.set("articlelink", catalogTemplateData.getLevelStr() + h.getFullPath());
                 } else {
                   String path = catalogTemplateData.getLevelStr() + CatalogUtil.getPath(articleCatalogID) + imageRelaTable.getString(0, "id") + ".shtml";
               ...
               {
                 String serviceUrl = Config.getValue("ServicesContext");
                 String statUrl = "SiteID=" + site.getID() + "&Type=Image&CatalogInnerCode=" + catalog.getInnerCode() + 
                   "&LeafID=" + row.getString("ID") + "&Dest=" + serviceUrl + "/Stat.jsp";
                 statScript = getStatScript(statUrl);
               }
           ...
           dtSize = dt.getRowCount();
           total = dtSize;
 
           statScript = (total % pageSize == 0) ? total / pageSize : total / pageSize + 1;
 
           catalogTemplateData.setTotal(total);
           catalogTemplateData.setPageCount(statScript);
           catalogTemplateData.setPageSize(pageSize);
 
           pageIndex = 0;
           for (k = 0; k < statScript; ++k) {
             catalogTemplateData.setPageIndex(pageIndex);
             ...
             values = new Object[count][dt.getColCount()];
             for (j = 0; j < count; ++j) {
             ...
             if ("Y".equals(site.getAutoStatFlag())) {
               serviceUrl = Config.getValue("ServicesContext");
               statUrl = "SiteID=" + site.getID() + "&Type=Image&CatalogInnerCode=" + catalog.getInnerCode() + 
                 "&Dest=" + serviceUrl + "/Stat.jsp";
               statScript = getStatScript(statUrl);
             }
             if (!(writeListHTML(site.getID(), listParser, catalogTemplateData, rootPath, listNameRule, pageIndex, statScript))) {
               return false;
             }
             ++pageIndex;
           }
           ...
           if (!(writeListHTML(site.getID(), listParser, catalogTemplateData, rootPath, listNameRule, 0, statScript)))
             return false;
         }
       }
       else
       {
         ...
         if (type == 5) {
           ...
           ZCVideoSet set = video.query(new QueryBuilder(" where " + catalogWherePart + " order by id desc", param));
           dt = set.toDataTable();
           if ((dt != null) && (dt.getRowCount() > 0)) {
             if (catalogDetailParser != null) {
               dt.insertColumn("Link");
               dt.insertColumn("filePath");
               for (j = 0; j < dt.getRowCount(); ++j) {
                 ...
                 for (int i = 0; i < count; ++i) {
                   ...
                   path = path + dt.getString(j, "path") + i + "_" + dt.getString(j, "FileName");
                 }
                 ...
                 {
                   String serviceUrl = Config.getValue("ServicesContext");
                   String statUrl = "SiteID=" + site.getID() + "&Type=Video&CatalogInnerCode=" + catalog.getInnerCode() + 
                     "&LeafID=" + dt.getDataRow(j).getString("ID") + "&Dest=" + serviceUrl + "/Stat.jsp";
                   statScript = getStatScript(statUrl);
                 }
             ...
             statUrl = (serviceUrl % pageSize == 0) ? serviceUrl / pageSize : serviceUrl / pageSize + 1;
 
             catalogTemplateData.setTotal(serviceUrl);
             catalogTemplateData.setPageCount(statUrl);
             catalogTemplateData.setPageSize(pageSize);
             ...
             for (k = 0; k < statUrl; ++k) {
               ...
               values = new Object[count][dt.getColCount()];
               for (statScript = 0; statScript < count; ++statScript) {
                 values[statScript] = dt.getDataRow(k * pageSize + statScript).getDataValues();
               }
               ...
               if ("Y".equals(site.getAutoStatFlag())) {
                 serviceUrl = Config.getValue("ServicesContext");
                 statUrl = "SiteID=" + site.getID() + "&Type=Video&CatalogInnerCode=" + catalog.getInnerCode() + 
                   "&Dest=" + serviceUrl + "/Stat.jsp";
                 statScript = getStatScript(statUrl);
               }
               if (!(writeListHTML(site.getID(), listParser, catalogTemplateData, rootPath, listNameRule, pageIndex, statScript))) {
             ...
             if ("Y".equals(site.getAutoStatFlag()))
             {
               ...
               statUrl = "SiteID=" + site.getID() + "&Type=Article&CatalogInnerCode=" + catalog.getInnerCode() + 
                 "&Dest=" + serviceUrl + "/Stat.jsp";
               j = getStatScript(statUrl);
             }
 
             if (!(writeListHTML(site.getID(), listParser, catalogTemplateData, rootPath, listNameRule, 0, j)))
               return false;
           }
         }
         else
         {
           ...
           if (type == 6) {
             ...
             ZCAudioSet set = audio.query(new QueryBuilder(" where " + catalogWherePart + " order by id desc", param));
             dt = set.toDataTable();
             ...
             if ((dt != null) && (dt.getRowCount() > 0)) {
               if (catalogDetailParser != null) {
                 for (j = 0; j < set.size(); ++j) {
                   ...
                   nameRule = dt.getDataRow(j).getString("ID") + ".shtml";
 
                   statUrl = "";
                   if ("Y".equals(site.getAutoStatFlag()))
                   {
                     pageIndex = Config.getValue("ServicesContext");
                     k = "SiteID=" + site.getID() + "&Type=Audio&CatalogInnerCode=" + catalog.getInnerCode() + 
                       "&LeafID=" + dt.getDataRow(j).getString("ID") + "&Dest=" + pageIndex + "/Stat.jsp";
                     statUrl = getStatScript(k);
                   }
 
                   writeHTML(catalogDetailParser, detailPath, nameRule, statUrl);
                 }
               }
               ...
               statUrl = (nameRule % pageSize == 0) ? nameRule / pageSize : nameRule / pageSize + 1;
 
               catalogTemplateData.setTotal(nameRule);
               catalogTemplateData.setPageCount(statUrl);
               catalogTemplateData.setPageSize(pageSize);
               ...
               for (k = 0; k < statUrl; ++k) {
                 ...
                 if ("Y".equals(site.getAutoStatFlag())) {
                   serviceUrl = Config.getValue("ServicesContext");
                   statUrl = "SiteID=" + site.getID() + "&Type=Audio&CatalogInnerCode=" + catalog.getInnerCode() + 
                     "&Dest=" + serviceUrl + "/Stat.jsp";
                   statScript = getStatScript(statUrl);
                 }
                 if (!(writeListHTML(site.getID(), listParser, catalogTemplateData, rootPath, listNameRule, pageIndex, statScript))) {
               ...
               if ("Y".equals(site.getAutoStatFlag()))
               {
                 nameRule = Config.getValue("ServicesContext");
                 statUrl = "SiteID=" + site.getID() + "&Type=Article&CatalogInnerCode=" + catalog.getInnerCode() + 
                   "&Dest=" + nameRule + "/Stat.jsp";
                 j = getStatScript(statUrl);
               }
             ...
             ZCAttachmentSet set = attach.query(new QueryBuilder(" where " + catalogWherePart + " order by id desc", param));
             dt = set.toDataTable();
             ...
             if ((dt != null) && (dt.getRowCount() > 0)) {
               if (catalogDetailParser != null) {
                 for (j = 0; j < set.size(); ++j) {
                   ...
                   {
                     pageIndex = Config.getValue("ServicesContext");
                     k = "SiteID=" + site.getID() + "&Type=Attachment&CatalogInnerCode=" + 
                       catalog.getInnerCode() + "&LeafID=" + dt.getDataRow(j).getString("ID") + 
                       "&Dest=" + pageIndex + "/Stat.jsp";
                     statUrl = getStatScript(k);
                   }
               ...
               statScript = dt.getRowCount();
               serviceUrl = statScript;
               statUrl = (serviceUrl % pageSize == 0) ? serviceUrl / pageSize : serviceUrl / pageSize + 1;
               catalogTemplateData.setTotal(serviceUrl);
               catalogTemplateData.setPageCount(statUrl);
               catalogTemplateData.setPageSize(pageSize);
               ...
               for (k = 0; k < statUrl; ++k) {
                 ...
                 count = pageSize;
                 if ((k + 1) * pageSize > statScript) {
                   count = statScript - (k * pageSize);
                 }
                 ...
                 if ("Y".equals(site.getAutoStatFlag())) {
                   serviceUrl = Config.getValue("ServicesContext");
                   statUrl = "SiteID=" + site.getID() + "&Type=Attachment&CatalogInnerCode=" + 
     ...
     label7194: staticInnerPageBlock(site, catalog);
   ...
   public boolean staticSiteIndex(ZCSiteSchema site) {
     ...
     statScript = statScript + "\n<!-- Published by " + this.product.getAppCode() + "(" + this.product.getAppName() + ") " + 
       this.product.getMainVersion() + " PublishDate " + DateUtil.getCurrentDateTime() + "-->";
     ...
     int level = 0;
     ZCPageBlockSet blockSet = new ZCPageBlockSchema().query(
       new QueryBuilder("where id in (select blockid from ZCTemplateBlockRela where filename='" + template + "_" + level + "')"));
   ...
   public boolean staticDoc(String docType, Schema doc)
   {
     ...
     if (StringUtil.isNotEmpty(docDataRow.getString("Content"))) {
       docDataRow.set("Content", docDataRow.getString("Content").replaceAll(imagePath, CatalogUtil.getLevelStr(level)));
     }
   ...
   private boolean staticDoc(String docType, TemplateParser parser, CmsTemplateData templateData, DataRow docDataRow, String rootPath, String fileName) {
     ...
     if ("article".equalsIgnoreCase(docType)) {
       ...
       while ((!("Y".equals(keyWordFlag))) && (!("N".equals(keyWordFlag))) && (!("0".equalsIgnoreCase(cid)))) {
       ...
       ZCKeywordSet keywordSet = (ZCKeywordSet)Keyword.KeyWordMap.get(docDataRow.getString("SiteID"));
       ...
       if (pages.length > 0) {
         ...
         for (int k = 0; k < pages.length; ++k) {
           ...
           {
             String serviceUrl = Config.getValue("ServicesContext");
             String statUrl = "SiteID=" + docDataRow.getString("SiteID") + "&Type=" + docType + "&CatalogInnerCode=" + docDataRow.getString("CatalogInnerCode") + "&LeafID=" + docDataRow.getString("ID") + "&Dest=" + serviceUrl + "/Stat.jsp";
             statScript = getStatScript(statUrl);
           }
 
           statScript = statScript + "\n<!-- Published by " + this.product.getAppCode() + " " + this.product.getMainVersion() + " PublishDate " + DateUtil.getCurrentDateTime() + "-->";
   ...
   private boolean staticInnerPageBlock(ZCSiteSchema site, ZCCatalogSchema catalog)
   {
     ...
     if (StringUtil.isNotEmpty(detailTemplate)) {
       detailTemplate = "/" + alias + detailTemplate + "_" + CatalogUtil.getPathLevel(catalog.getDetailNameRule(), 0);
   ...
   private boolean staticInnerPageBlock(ZCSiteSchema site, ZCCatalogSchema catalog, String templateFile) {
     ...
     if (set.size() > 0) {
       for (int i = 0; i < set.size(); ++i) {
         ZCPageBlockSchema block = set.get(i);
         String targetFile = this.templateDir + "/" + site.getAlias() + "/" + block.getFileName();
     ...
     if (files.exists()) {
       ...
       for (int i = 0; i < fileList.length; ++i) {
         ...
         if ((!(tmpFileName.startsWith(fileName + "_"))) || 
           (tmpFileName.lastIndexOf("_") == -1)) continue;
         String levelStr = "";
   ...
   public boolean writeListHTML(long siteID, TemplateParser tp, CmsTemplateData templateData, String path, String listNameRule, int pageIndex, String statScript)
   {
 ...
 * Qualified Name:     com.zving.cms.template.PageGenerator
