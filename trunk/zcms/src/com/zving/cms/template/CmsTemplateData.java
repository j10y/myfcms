...
public class CmsTemplateData extends TemplateData {
	...
	public DataTable getCatalogList(String item, String catalogType, String name,
			String displayLevel, String count, String condition) {
		DataTable dt = null;

		String key = item + "_" + catalogType + "_" + displayLevel + "_" + name + "_" + count + "_"
				+ condition;
		...
		if ("catalog".equalsIgnoreCase(item)) {
			...
			if ("root".equalsIgnoreCase(displayLevel)) {
				...
				if ((StringUtil.isEmpty(parentID)) && (this.catalog != null)) {
			...
			sql = "select * from zccatalog where siteID=? " + parentSql + typeSql
			...
			for (int i = 0; i < dt.getRowCount(); ++i) {
				...
				if ((dt.getString(i, "url").startsWith("http"))
						|| (dt.getString(i, "url").startsWith("/"))) {
	...
	public DataTable getDocList(String item, String catalogName, String displayLevel, String type,
			String count) {
	...
	public DataTable getDocList(String item, String catalogName, String displayLevel,
			String orderType, String count, String condition) {
		return getDocList(item, catalogName, displayLevel, "", orderType, count, condition);
	}

	public DataTable getDocList(String item, String catalogName, String displayLevel,
			String hasAttribute, String orderType, String count, String condition) {
		DataTable dt = null;
		String key = item + "_" + catalogName + "_" + displayLevel + "_" + hasAttribute + "_"
				+ orderType + "_" + count + "_" + condition;
		...
		if ("article".equals(item)) {
			...
			if (catalogName.indexOf(",") == -1) {
				if ((StringUtil.isNotEmpty(catalogID))
						&& (StringUtil.isNotEmpty(CatalogUtil.getWorkflow(catalogID))))
					allowStatus = "20,30";
					...
					allowStatus = CatalogUtil.getAllowStatus(catalogID);
						...
						&& (StringUtil.isNotEmpty(SiteUtil
								.getAllowStatus(this.site.getString("ID"))))) {
					allowStatus = SiteUtil.getAllowStatus(this.site.getString("ID"));
				}
			}

			statusStr = "status in(" + allowStatus + ") and publishdate<='"
					+ DateUtil.getCurrentDateTime() + "' and";
		}
		if ((StringUtil.isEmpty(condition)) || ("null".equals(condition))) {
			condition = "1=1";
		}
		String sql = "select * from zc" + item + " where " + condition + " and " + statusStr
				+ catalogStr + typeStr;
			...
			if ("article".equalsIgnoreCase(item)) {
				if ("4".equals((String) dt.get(i, "Type"))) {
					...
					if (StringUtil.isNotEmpty(nameRule)) {
						HtmlNameParser nameParser = new HtmlNameParser(this.site, null, dt.get(i),
								nameRule);
						HtmlNameRule h = nameParser.getNameRule();
						columnValue[i] = levelString + h.getFullPath();
					} else {
						columnValue[i] = levelString + CatalogUtil.getPath(docCatalogID) + docID
								+ ".shtml";
					}
				...
				if ((imageRelaTable != null) && (imageRelaTable.getRowCount() > 0)) {
					...
					if (StringUtil.isNotEmpty(nameRule)) {
						HtmlNameParser nameParser = new HtmlNameParser(this.site, null,
								imageRelaTable.get(0), nameRule);
						HtmlNameRule h = nameParser.getNameRule();
						dt.set(i, "articlelink", levelString + h.getFullPath());
					} else {
						String path = levelString + CatalogUtil.getPath(articleCatalogID)
								+ articleID + ".shtml";
				...
				columnValue[i] = Config.getValue("ServicesContext") + "/AttachDownLoad.jsp?id="
						+ docID;
			} else {
				String nameRule = CatalogUtil.getTemplateRule(docCatalogID);
				if (StringUtil.isNotEmpty(nameRule)) {
	...
	public DataTable getMagazineList(String item, String type, String name, String count,
			String condition) {
		DataTable dt = null;
	...
	public DataTable getMagazineIssueList(String item, String name, String type, String count,
			String condition) {
		DataTable dt = null;
		...
		if ("magazineissue".equalsIgnoreCase(item)) {
			...
			if (magazineDT.getRowCount() > 0) {
				...
				if (this.catalog != null) {
					...
					if ((catalogType == 3) && (catalogName.indexOf("年第") != -1)) {
						String currentYear = catalogName.substring(0, catalogName.indexOf("年第"));
						String currentPeriodNum = catalogName.substring(
								catalogName.indexOf("年第") + 2, catalogName.indexOf("期"));
						sql = "select * from ZCMagazineIssue where MagazineID=? and Year='"
								+ currentYear + "' and PeriodNum='" + currentPeriodNum + "'";
					}
				}
				...
				if (dt.getRowCount() > 0) {
					for (int i = 0; i < dt.getRowCount() - 1; ++i) {
						String prevCatalogID = CatalogUtil
								.getIDByName(this.site.getString("ID"), dt.getString(i, "Year")
					...
					DataTable magazineIssueDT = new QueryBuilder(
							"select * from ZCMagazineIssue where MagazineID=? and ID>? order by id",
							magazineDT.getString(0, "ID"), dt.getString(0, "ID"))
							.executeDataTable();
					if (magazineIssueDT.getRowCount() > 0) {
						nextCatalogID = CatalogUtil.getIDByName(this.site.getString("ID"),
					...
					magazineIssueDT = new QueryBuilder(
							"select * from ZCMagazineIssue where MagazineID=? and ID<? order by id desc",
							magazineDT.getString(0, "ID"), dt.getString(0, "ID"))
							.executeDataTable();
					if (magazineIssueDT.getRowCount() > 0) {
						nextCatalogID = CatalogUtil.getIDByName(this.site.getString("ID"),
								magazineIssueDT.getString(0, "Year") + "年第"
										+ magazineIssueDT.getString(0, "PeriodNum") + "期");
						dt.set(dt.getRowCount() - 1, "PrevLink", staticPath
								+ CatalogUtil.getPath(nextCatalogID));
					} else {
						dt.set(dt.getRowCount() - 1, "PrevLink", "#");
					}

					for (int m = 0; m < dt.getRowCount(); ++m) {
						String ID = CatalogUtil.getIDByName(this.site.getString("ID"), dt
								.getString(m, "Year")
	...
	public DataTable getFriendLinkList(String item, String group, String count, String condition) {
		...
		String sql = "select * from zclink where siteID = " + this.site.getString("ID")
				+ " and LinkGroupID = " + "(select id from zclinkgroup where siteID = "
				+ this.site.getString("ID") + " and name=?) order by OrderFlag,id desc";
	...
	public DataTable getBlockList(String item, String count, String condition) {
		...
		for (int i = 0; i < size; ++i) {
			if ("1".equalsIgnoreCase(blockType)) {
				...
				columnValue[i] = CatalogUtil.getPath(catalog)
						+ publishDate.substring(0, publishDate.lastIndexOf("-")) + "/"
						+ publishDate.substring(publishDate.lastIndexOf("-") + 1) + "/" + articleID
						+ ".shtml";
			} else if ("2".equalsIgnoreCase(blockType)) {
	...
	public String getAD(String name) {
		...
		if ((StringUtil.isEmpty(name)) || (name == null)) {
			...
			DataTable positionDT = new QueryBuilder(
					"select * from zcadposition where PositionName = ? and siteid=?", name,
					this.site.getString("ID")).executeDataTable();
			if (positionDT.getRowCount() <= 0) {
				sb.append("广告版位代码：" + name + "没有找到");
			} else {
				String jsName = positionDT.getString(0, "jsname");
				sb.append("<script language='javascript' src='" + getLevelStr() + jsName
						+ "'></script>");

				if (!(new File(new StringBuffer(String.valueOf(Config.getContextRealPath()))
						.append(Config.getValue("Statical.TargetDir")).append("/").append(
								positionDT.getString(0, "SiteID")).append("/").toString()
						.replaceAll("//", "/")
	...
	public String getForm(String code) {
		...
		if (StringUtil.isEmpty(code)) {
			...
			ZCCustomTableSet set = new ZCCustomTableSchema().query(new QueryBuilder("where Code=?",
					code));
	...
	public String getVote(String name, String id) {
		...
		sb.append("<script language='javascript' src='"
				+ new StringBuffer(String.valueOf(getLevelStr())).append("js/vote_").append(
						vote.getID()).toString().replaceAll("//", "/") + ".js'></script>");
	...
	public DataTable getVoteSubjects(String voteID, int count) {
		QueryBuilder qb = new QueryBuilder(
				"select * from zcvoteSubject where VoteID = ? order by ID", voteID);
		return qb.executePagedDataTable(count, 0);
	}

	public DataTable getVoteItems(String subjectID, String inputType, int count) {
		QueryBuilder qb = new QueryBuilder(
				"select * from zcvoteitem where subjectID = ? order by ID", subjectID);
		...
		for (int i = 0; i < dt.getRowCount(); ++i) {
			...
			if ("0".equals(dt.getString(i, "ItemType")))
				html = "<label><input name='Subject_" + subjectID + "' type='" + inputType
						+ "' value='" + dt.getString(i, "id") + "' />" + dt.getString(i, "item")
						+ "</label>\n";
			else {
				html = "<label><input name='Subject_" + subjectID + "' type='" + inputType
						+ "' value='" + dt.getString(i, "id") + "' id='Subject_" + subjectID
						+ "_Item_" + dt.getString(i, "id") + "_Button'/>" + dt.getString(i, "item")
						+ "</label><input id='Subject_" + subjectID + "_Item_"
						+ dt.getString(i, "id") + "' name='Subject_" + subjectID + "_Item_"
						+ dt.getString(i, "id")
						+ "' type='text' value='' onClick=\\\"clickInput('Subject_" + subjectID
						+ "_Item_" + dt.getString(i, "id") + "');\\\"/>\n";
			}
			dt.set(i, "html", html);
		}
		return dt;
	...
	public String getComment(String count) {
		return "<script src=\"" + Config.getValue("ServicesContext")
				+ Config.getValue("CommentListJS") + "?RelaID=" + this.doc.getString("ID")
				+ "&CatalogID=" + this.doc.getString("CatalogID") + "&CatalogType="
				+ this.doc.getString("Type") + "&SiteID=" + this.site.getString("id") + "&Count="
				+ count + "\"></script>";
	}
	...
	public String getImagePlayer(String name, String width, String height, String count) {
		...
		sb.append("<param name=\"FlashVars\" value=\"pics=" + pics.toString() + "&links="
				+ links.toString() + "&texts=" + texts.toString() + "&borderwidth=" + pWidth
		...
		sb
				.append("<embed src=\""
						...
						+ "FlashVars=\"wmode=transparent&pics=" + pics.toString() + "&links="
						+ links.toString() + "&texts=" + texts.toString() + "&borderwidth="
						+ pWidth + "&borderheight=" + imgHeight + showText);

		sb.append("&button_pos=4&overtxtcolor=FFFF00&txtcolor=FFFF00\" width=\"" + pWidth
				+ "\" height=\"" + pHeight + "\">");
		sb.append("</embed>");
		sb.append("</object>");
		...
		if ("article".equalsIgnoreCase(type)) {
			...
			if ((name != null) && (!("".equals(name))) && (!("null".equals(name)))) {
				...
				if ((set != null) && (set.size() > 0)) {
					...
					docRow.insertColumn("link", levelString
							+ CatalogUtil.getPath(article.getCatalogID()) + article.getID()
							+ ".shtml");
				}
			}
		}
 ...
 * com.zving.cms.template.CmsTemplateData JD-Core Version: 0.5.3
 */