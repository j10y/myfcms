 ...
 public class ExpressionTokenizer
 {
   ...
   public int nextToken()
   {
     ...
     this.tokenVal = null;
     if (this.index == this.length) return 12;
     int start = this.index;
     char currentChar = this.expr[this.index];
     char nextChar = '\0';
     this.index += 1;
     if (this.index < this.length) nextChar = this.expr[this.index];
     ...
     if ((currentChar == '"') || (currentChar == '\'')) {
       ...
       do {
         if (isMetaChar(this.expr[this.index]))
           break;
         this.index += 1; } while (this.index < this.length);
 ...
 * Qualified Name:     com.zving.framework.ssi.ExpressionTokenizer
