 ...
 public final class RequestUtil
 {
     ...
     throws UnsupportedEncodingException
   {
     ...
     try {
       if (encoding == null) {
         bytes = data.getBytes(); break label39:
       }
     ...
     label39: parseParameters(map, bytes, encoding);
   }
   ...
   public static String URLDecode(String str, String enc, boolean isQuery)
   {
     ...
     try {
       if (enc == null) {
         bytes = str.getBytes(); break label34:
       }
       bytes = str.getBytes(enc);
     }
     catch (UnsupportedEncodingException localUnsupportedEncodingException) {
     }
     label34: return URLDecode(bytes, enc, isQuery);
   }
     ...
     throws UnsupportedEncodingException
   {
     if ((data != null) && (data.length > 0)) {
       ...
       while (ix < data.length) {
         ...
         case '%':
           data[(ox++)] = 
             (byte)((convertHexDigit(data[(ix++)]) << 4) + 
 ...
 * Qualified Name:     com.zving.framework.ssi.RequestUtil
