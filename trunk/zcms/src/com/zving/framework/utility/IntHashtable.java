   ...
   implements Cloneable
 {
   ...
   public synchronized Object remove(int key)
   {
     ...
     int hash = key;
     int index = (hash & 0x7FFFFFFF) % tab.length;
     IntHashtableEntry e = tab[index]; for (IntHashtableEntry prev = null; e != null; e = e.next) {
       if ((e.hash == hash) && (e.key == key)) {
         if (prev != null)
           prev.next = e.next;
         else {
   ...
   public synchronized Object clone()
   {
     try
     {
       ...
       for (int i = this.table.length; i-- > 0; ) {
         t.table[i] = ((this.table[i] != null) ? (IntHashtableEntry)this.table[i].clone() : null);
       }
       return t;
     }
 ...
 * Qualified Name:     com.zving.framework.utility.IntHashtable
