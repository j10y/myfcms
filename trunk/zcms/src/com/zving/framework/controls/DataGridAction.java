   ...
   implements IControlAction
 {
   ...
   private void bindData() throws Exception
   {
     ...
     int i;
     if ((this.TreeFlag) && 
       (this.DataSource.getDataColumn("ParentID") != null) && (this.DataSource.getDataColumn("ID") != null)) {
       Mapx map = new Mapx();
       for (i = 0; i < this.DataSource.getRowCount(); ++i) {
         map.put(this.DataSource.getString(i, "ID"), this.DataSource.getString(i, "ParentID"));
       }
     ...
     for (i = 0; i < this.headTR.Children.size(); ++i)
     {
       ...
       if ("Selector".equalsIgnoreCase(this.headTR.getTD(i).getAttribute("ztype"))) {
         field = this.headTR.getTD(i).getAttribute("field");
         ...
         for (j = 1; j < this.Table.Children.size(); ++j) {
           this.Table.getTR(j).getTD(i).setInnerHTML(
             "<input type='" + type + "' name='" + this.ID + "_RowCheck' id='" + this.ID + "_RowCheck" + j + 
             "' value='" + this.DataSource.getString(j - 1, field) + "'>");
           this.Table.getTR(j).getTD(i).setAttribute("onclick", "DataGrid.onSelectorClick(this,event);" + onSelect);
       ...
       if ("Checkbox".equalsIgnoreCase(this.headTR.getTD(i).getAttribute("ztype"))) {
         ...
         for (j = 1; j < this.Table.Children.size(); ++j) {
           ...
           this.Table.getTR(j).getTD(i).setInnerHTML(
             "<input type='checkbox' " + disabled + " name='" + this.ID + "_" + field + "_Checkbox' id='" + 
             this.ID + "_" + field + "_Checkbox" + j + "' value='" + checkedvalue + "' " + checked + 
             ">");
         }
       }
       ...
       if ("DropDownList".equalsIgnoreCase(this.headTR.getTD(i).getAttribute("ztype"))) {
         ...
         for (int j = 1; j < this.Table.Children.size(); ++j) {
           StringBuffer sb = new StringBuffer();
           sb.append("<div ztype='select' disabled='true' style='display:none;" + zstyle + ";' name='" + this.ID + 
             "_" + field + "_DropDownList" + j + "' id='" + this.ID + "_" + field + "_DropDownList" + j + 
             "' >");
       ...
       if ("Tree".equalsIgnoreCase(this.headTR.getTD(i).getAttribute("ztype")))
       {
         ...
         for (int j = 1; j < this.Table.Children.size(); ++j) {
           ...
           if (field != null)
           {
             ...
             sb.append("<input type='checkbox'  name='" + this.ID + "_TreeRowCheck' id='" + this.ID + "_TreeRowCheck_" + 
       ...
       if ("true".equalsIgnoreCase(this.headTR.getTD(i).getAttribute("drag"))) {
         for (j = 1; j < this.Table.Children.size(); ++j) {
           ...
           this.Table.getTR(j).setAttribute("onMouseUp", "DragManager.onMouseUp(event,this)");
           ...
           if (userAgent.indexOf("msie") >= 0)
             this.Table.getTR(j).setAttribute("onMouseEnter", "DragManager.onMouseOver(event,this)");
           else {
             this.Table.getTR(j).setAttribute("onMouseOver", "DragManager.onMouseOver(event,this)");
           }
           this.Table.getTR(j).setAttribute("dragOver", "DataGrid.dragOver");
 
           if (userAgent.indexOf("msie") >= 0)
             this.Table.getTR(j).setAttribute("onMouseLeave", "DragManager.onMouseOut(event,this)");
           else {
             this.Table.getTR(j).setAttribute("onMouseOut", "DragManager.onMouseOut(event,this)");
           }
     ...
     for (i = 0; i < this.Table.getChildren().size(); ++i) {
       HtmlTR tr = this.Table.getTR(i);
       for (j = 0; j < tr.getChildren().size(); ++j) {
         td = tr.getTD(j);
   ...
   public String scrollWrap()
   {
     ...
     sb.append("<div id='" + this.ID + "_Wrap' class='dataTable dt_scrollable dt_nobr' ztype='_DataGridWrapper'");
     ...
     for (i = 0; i < tmpTable.getTR(0).getChildren().size(); ++i) {
   ...
   private void dealPageBar() {
     try {
       ...
       String html = getPageBarHtml(this.ID, this.Params, this.Total, this.PageIndex, this.PageSize, this.isSimplePageBar);
   ...
   public String getScript() {
     ...
     if (this.editTR != null) {
       ...
       for (i = 0; i < this.editTR.getChildren().size(); ++i) {
         sb.append("_Zving_Arr.push(\"" + StringUtil.javaEncode(this.editTR.getTD(i).getInnerHTML()) + "\");");
       }
     ...
     if (this.PageFlag) {
       sb.append("DataGrid.setParam('" + this.ID + "','" + "_ZVING_PAGEINDEX" + "'," + this.PageIndex + ");");
       sb.append("DataGrid.setParam('" + this.ID + "','" + "_ZVING_PAGETOTAL" + "'," + this.Total + ");");
       sb.append("DataGrid.setParam('" + this.ID + "','" + "_ZVING_SIZE" + "'," + this.PageSize + ");");
     }
     if (this.SortFlag) {
       sb.append("DataGrid.setParam('" + this.ID + "','" + "_ZVING_SORTSTRING" + "','" + this.SortString + 
         "');");
     }
     sb.append("DataGrid.setParam('" + this.ID + "','" + "_ZVING_MULTISELECT" + "','" + this.MultiSelect + "');");
     sb.append("DataGrid.setParam('" + this.ID + "','" + "_ZVING_AUTOFILL" + "','" + this.AutoFill + "');");
     sb.append("DataGrid.setParam('" + this.ID + "','" + "_ZVING_SCROLL" + "','" + this.Scroll + "');");
     sb.append("DataGrid.setParam('" + this.ID + "','" + "_ZVING_LAZY" + "','" + this.Lazy + "');");
     if (this.cacheSize > 0) {
       sb.append("DataGrid.setParam('" + this.ID + "','" + "_ZVING_CACHESIZE" + "','" + this.cacheSize + "');");
     }
   ...
   public static String getPageBarHtml(String id, Mapx params, int total, int pageIndex, int pageSize, boolean simpleFlag)
   {
     StringBuffer sb = new StringBuffer();
 ...
 * Qualified Name:     com.zving.framework.controls.DataGridAction
