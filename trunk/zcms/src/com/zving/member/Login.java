 ...
 public class Login extends Ajax
 {
   public static Mapx initSiteLinks(Mapx params)
   {
     ...
     DataTable dt = new QueryBuilder("select * from zccatalog where Type = 1 and ParentID = 0 and SiteID = ?", SiteID).executePagedDataTable(10, 0);
     SiteLinks = SiteLinks + "<a href='" + Path + "'>首页</a>&nbsp;&nbsp;&nbsp;";
     if ((dt != null) && (dt.getRowCount() > 0)) {
       for (int i = 0; i < dt.getRowCount(); ++i) {
         SiteLinks = SiteLinks + "<a href='" + Path + dt.getString(i, "URL") + "'>" + dt.getString(i, "Name") + "</a>&nbsp;&nbsp;&nbsp;";
       }
     }
   ...
   public void doLogin() {
     ...
     if (member.IsExists()) {
       ...
       if (member.checkPassWord(passWord)) {
         String VerifyCookie = StringUtil.md5Hex(member.getPassword().substring(0, 10) + System.currentTimeMillis());
         ...
         member.setMemberLevel(new QueryBuilder(new StringBuffer("select ID from ZDMemberLevel where Score <= ").append(member.getScore()).append(" order by Score desc").toString()).executeOneValue());
   ...
   public static void checkAndLogin(HttpServletRequest request) {
     ...
     if ((!(User.isManager())) && (!(User.isLogin())) && (StringUtil.isNotEmpty(UserName))) {
       Member member = new Member(UserName);
       if ((!(member.fill())) || 
         (!(StringUtil.isNotEmpty(member.getProp1()))) || (!(member.getProp1().equalsIgnoreCase(VerifyCookie)))) return;
       User.setManager(false);
       User.setLogin(true);
