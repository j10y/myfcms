...
public class Article extends Ajax {
			...
			if (StringUtil.isNotEmpty(ID)) {
				...
				if (article.fill()) {
					...
					DataTable dt = new QueryBuilder(
							"select ID from ZCAttachmentRela where RelaID = " + article.getID()
									+ " and RelaType = 'MemberArticle'").executeDataTable();
					...
					params.put("CustomColumn", ColumnUtil.getHtml("1", String.valueOf(CatalogID),
							"2", String.valueOf(article.getID()), "hidden"));
				}
			} else {
	...
	public static Mapx initSiteID(Mapx params) {
		...
		if ((StringUtil.isEmpty(siteID)) || (siteID.equals("0"))) {
			siteID = (String) new QueryBuilder("select ID from ZCSite order by AddTime desc")
					.executeOneValue();
		}
	...
	public void doSave() {
		...
		new QueryBuilder("delete from ZCAttachmentrela where RelaID = " + article.getID()
				+ " and RelaType = 'MemberArticle'").executeNoQuery();
		if (StringUtil.isNotEmpty(AttachIDs)) {
			...
			for (int i = 0; i < AttachArr.length; ++i) {
				...
				if (attach.fill()) {
					Content = Content + "<a href='#;' onClick=\"javascript:window.open('"
							+ Config.getContextPath() + "Services/AttachDownLoad.jsp?id="
							+ attach.getID() + "&SiteID=" + SiteID + "', '_blank')" + "\">下载附件："
							+ attach.getOldName() + "</a><br/>";
		...
		if (flag) {
			trans.add(ColumnUtil.getValueFromRequest(Long.parseLong(CatalogID), article.getID(),
					this.Request), 1);
			trans.add(article, 1);
		} else {
			...
			trans.add(ColumnUtil.getValueFromRequest(Long.parseLong(CatalogID), Long
					.parseLong(ArticleID), this.Request), 1);
			trans.add(article, 2);
		}
		if (trans.commit())
	...
	public static void dg1DataList(DataListAction dla) {
		...
		for (int i = 0; i < dt.getRowCount(); ++i) {
			...
			if (dt.getString(i, "StatusName").equals("已发布"))
				dt.set(i, "DO", "<cite><a href='#;' onclick='preview(" + dt.getString(i, "ID")
						+ ");'>浏览</a></cite>");
			else {
				dt.set(i, "DO", "<cite><a href='#;' onclick='preview(" + dt.getString(i, "ID")
 ...
 * com.zving.member.Article JD-Core Version: 0.5.3
 */