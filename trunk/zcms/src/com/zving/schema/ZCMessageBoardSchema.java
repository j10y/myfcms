 ...
 public class ZCMessageBoardSchema extends Schema
 {
   ...
   protected static final String _UpdateAllSQL = "update ZCMessageBoard set ID=?,SiteID=?,Type=?,Title=?,Content=?,PublishFlag=?,ReplyFlag=?,ReplyContent=?,Email=?,Tel=?,Mobile=?,Address=?,AttachPath=?,Prop1=?,Prop2=?,UserName=?,AddUser=?,AddUserIP=?,AddTime=?,ModifyUser=?,ModifyTime=? where ID=?";
