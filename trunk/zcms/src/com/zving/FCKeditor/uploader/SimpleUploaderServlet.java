 ...
 public class SimpleUploaderServlet extends HttpServlet
 {
     ...
     throws ServletException
   {
     ...
     for (int i = 0; i < arr.length; ++i) {
       allowedExtensions.put(arr[i], stringToArrayList(getInitParameter("AllowedExtensions" + arr[i])));
       deniedExtensions.put(arr[i], stringToArrayList(getInitParameter("DeniedExtensions" + arr[i])));
     }
   }
   ...
   public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
     ...
     if ("activex".equalsIgnoreCase(fromObjec)) {
       ...
       out.println("window.parent.onUploadCompleted(" + returnValue + ",'" + returnIDs.toString() + "','" + 
         errorMessage + "');");
       out.println("</script>");
       out.flush();
       out.close();
   ...
   private String[] repeatUpload(Mapx files, Mapx fields) throws Exception
   {
     ...
     if ("Image".equalsIgnoreCase((String)fields.get("FileType"))) {
       ...
       while (it.hasNext()) {
         ...
         AbsolutePath = getServletContext()
           .getRealPath(new StringBuffer(String.valueOf(baseDir)).append("/").append(SiteAlias).append("/").append(image.getPath()).toString()) + 
           "/";
       ...
       while (it.hasNext()) {
         uplFile = (FileItem)files.get(it.next());
         AbsolutePath = getServletContext().getRealPath(new StringBuffer(String.valueOf(baseDir)).append("/").append(SiteAlias).append("/").append(attach.getPath()).toString()) + "/";
       ...
       while (it.hasNext()) {
         uplFile = (FileItem)files.get(it.next());
         AbsolutePath = getServletContext().getRealPath(new StringBuffer(String.valueOf(baseDir)).append("/").append(SiteAlias).append("/").append(audio.getPath()).toString()) + "/";
   ...
   private String[] upload(Mapx files, Mapx fields) throws Exception {
     ...
     if (CatalogID == null) {
       if ("Image".equals(FileType))
         ...
         CatalogID = new QueryBuilder("select ID from ZCCatalog  where type='5' and Name ='默认视频' and siteid=?", 
           Application.getCurrentSiteID()).executeString();
       else if ("Attach".equals(FileType))
         CatalogID = new QueryBuilder("select ID from ZCCatalog  where type='7' and Name ='默认附件' and siteid=?", 
           Application.getCurrentSiteID()).executeString();
       else if ("Audio".equals(FileType)) {
         CatalogID = new QueryBuilder("select ID from ZCCatalog  where type='6' and Name ='默认音频' and siteid=?", 
           Application.getCurrentSiteID()).executeString();
       }
     }
     ...
     AbsolutePath = getServletContext().getRealPath(new StringBuffer(String.valueOf(baseDir)).append("/").append(siteAlias).append("/").append(path).toString()) + "/";
       ...
       for (int m = files.size() - 1; m >= 0; --m) {
         ...
         if (extIsAllowed(FileType, ext)) {
           if ("zip".equalsIgnoreCase(ext)) {
             String tempPath = getServletContext().getRealPath(new StringBuffer(String.valueOf(baseDir)).append(siteAlias).toString()) + "/Temp/";
             ...
             try {
               ...
               System.out.println("-------------" + fileName + " 该图片已被损坏！-------------");
             }
 
             if (StringUtil.isNotEmpty((String)fields.get("OtherIDs"))) {
               OtherIDs = StringUtil.splitEx((String)fields.get("OtherIDs"), ",");
       ...
       if ("Video".equalsIgnoreCase(FileType)) {
         ...
         while (it.hasNext()) {
           ...
           if (extIsAllowed(FileType, ext)) {
             ...
             video.setSuffix(ext);
             video.setInfo(fields.get("Info"));
             video.setTag(fields.get("Tag"));
             video.setIsOriginal(fields.get("IsOriginal"));
             ...
             if (StringUtil.isNotEmpty((String)fields.get("OtherIDs"))) {
               ...
               for (int j = 0; j < OtherIDs.length; ++j) {
                 ...
                 videoClone.setCatalogInnerCode(CatalogUtil.getInnerCode(OtherIDs[j]));
         ...
         while (it.hasNext()) {
           ...
           if ("zip".equalsIgnoreCase(ext)) {
             ...
             for (int i = 0; i < audioFiles.length; ++i) {
               ...
               oldName = oldName.substring(0, oldName.indexOf("."));
               if ((!(extIsAllowed(FileType, ext))) || 
                 (!("Audio".equalsIgnoreCase(FileType)))) continue;
               ...
               audio.setSuffix(ext);
               audio.setFileSize(FileUtils.byteCountToDisplaySize(audioFiles[i].length()));
               audio.setSystem("CMS");
               ...
               if (StringUtil.isNotEmpty((String)fields.get("OtherIDs"))) {
                 ...
                 for (j = 0; j < OtherIDs.length; ++j) {
                   ...
                   audioClone.setCatalogInnerCode(CatalogUtil.getInnerCode(OtherIDs[j]));
             ...
             if (fields.get(uplFile.getFieldName() + "IsOriginal") != null) {
               audio.setIsOriginal((String)fields.get(uplFile.getFieldName() + "IsOriginal"));
             }
         ...
         while (it.hasNext()) {
           ...
           if (extIsAllowed(FileType, ext)) {
             ...
             attachment.setName(fields.getString(new StringBuffer(String.valueOf(uplFile.getFieldName())).append("Name").toString()));
             ...
             if ("Y".equals(attachment.getIsLocked())) {
               attachment
                 .setPassword(StringUtil.md5Hex(fields.getString(uplFile.getFieldName() + "Password")));
             }
     ...
     if ((StringUtil.isNotEmpty(catalog.getIndexTemplate())) || (StringUtil.isNotEmpty(catalog.getListTemplate())) || (StringUtil.isNotEmpty(catalog.getDetailTemplate()))) {
 ...
 * Qualified Name:     com.zving.FCKeditor.uploader.SimpleUploaderServlet
