...
package specialdevice.fredck.FCKeditor.tags;
...
import specialdevice.fredck.FCKeditor.FCKeditor;
...
public class FCKeditorTag extends BodyTagSupport  {
	...
	private String fontNames = "宋体;黑体;隶书;楷体_GB2312;Arial;Comic Sans MS;Courier New;Tahoma;Times New Roman;Verdana";
   ...
   /**
	...
	public int doStartTag() throws JspException {
		...
		String rootPath = fcked.getBasePath();
		if(toolbarSet!=null)
			fcked.setToolbarSet(toolbarSet);		
		if (basePath!=null)
		  fcked.setBasePath(rootPath+basePath);		  
		if(width!=null)
		...
		if (skinPath != null)
			fcked.getConfig().put("SkinPath",fcked.getBasePath()+skinPath);
		if (pluginsPath != null)
			fcked.getConfig().put("PluginsPath",fcked.getBasePath()+pluginsPath);	
		if (fullPage != null)
		...
		if (linkBrowserURL != null)
			fcked.getConfig().put("LinkBrowserURL",fcked.getBasePath()+linkBrowserURL);
		if (imageBrowserURL != null)
			fcked.getConfig().put("ImageBrowserURL",fcked.getBasePath()+imageBrowserURL);
		if (flashBrowserURL != null)
			fcked.getConfig().put("FlashBrowserURL",fcked.getBasePath()+flashBrowserURL);
		if (linkUploadURL != null)
			fcked.getConfig().put("LinkUploadURL",fcked.getBasePath()+linkUploadURL);
		if (imageUploadURL != null)
			fcked.getConfig().put("ImageUploadURL",fcked.getBasePath()+imageUploadURL);
		if (flashUploadURL != null)
			fcked.getConfig().put("FlashUploadURL",fcked.getBasePath()+flashUploadURL);
