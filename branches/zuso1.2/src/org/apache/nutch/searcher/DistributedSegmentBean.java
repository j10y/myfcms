...
public class DistributedSegmentBean implements SegmentBean {
	...
	private class SegmentWorker implements Runnable {
		...
		public void run() {
			try {
				...
				Iterator<Map.Entry<String, Integer>> i = segmentMap.entrySet().iterator();
				while (i.hasNext()) {
	...
	public DistributedSegmentBean(Configuration conf, Path serversConfig) throws IOException {
		this.timeout = conf.getLong("ipc.client.timeout", 60000);

		List<SegmentBean> beanList = new ArrayList<SegmentBean>();

		List<InetSocketAddress> segmentServers = NutchBean.readAddresses(serversConfig, conf);

		for (InetSocketAddress addr : segmentServers) {
			SegmentBean bean = (RPCSegmentBean) RPC.getProxy(RPCSegmentBean.class, FetchedSegments.VERSION, addr, conf);
			beanList.add(bean);
		}
	...
	public Summary[] getSummary(HitDetails[] detailsArr, Query query) throws IOException {
		...
		for (int i = 0; i < summaryTasks.size(); i++) {
			...
			if (detailsList[i].size() > 0) {
				HitDetails[] taskDetails = detailsList[i].toArray(new HitDetails[detailsList[i].size()]);
		...
		try {
			summaries = executor.invokeAll(summaryTasks, timeout, TimeUnit.MILLISECONDS);
