...
/**
 * Java version of the arclab readability javascript program. This uses jsoup to
 * handle the DOM tree and provide us with the sorts of operations that the
 * javascript code loves. Make one of these objects for each page. Provide it
 * with an object to fetch more next pages to support that stuff.
 */
public class Readability {
	private static final Logger LOG = LoggerFactory
			.getLogger(Readability.class);
	...
	/**
	 * Process the content of a page. This takes a String, since JSoup does not
	 * handle byte input. Caller has to worry about charset detection and
	 * conversion.
	 * 
	 * @param url
	 *            the initial url
	...
	private void incrementContentScore(Element node, double score) {
		node.attr("data-readability.contentScore",
				Double.toString(getContentScore(node) + score));
	}
	...
	private void initializeNode(Element node) {
		...
		} else if ("pre".equals(tagName) || "td".equals(tagName)
				|| "blockquote".equals(tagName)) {
			incrementContentScore(node, 3);
		} else if ("address".equals(tagName) || "ol".equals(tagName)
				|| "ul".equals(tagName) || "dl".equals(tagName)
				|| "dd".equals(tagName) || "dt".equals(tagName)
	...
	private Element grabArticle(Element pageElement) {
		...
		/*
		 * Note: in Javascript, this list would be *live*. If you deleted a node
		 * from the tree, it and its children would remove themselves. To get
		 * the same effect, we make a linked list and we remove things from it.
		 * This won't win prizes for speed, but, then again, the code in
		 * Javascript has to be doing something nearly as awful.
		 */
		 ...
		 * block level elements.) Note: Assignment from index for performance.
		...
		while (elIterator.hasNext()) {
			...
			if (stripUnlikelyCandidates) {
				...
				if (Patterns.exists(Patterns.UNLIKELY_CANDIDATES,
						unlikelyMatchString)
						&& !Patterns.exists(Patterns.OK_MAYBE_ITS_A_CANDIDATE,
								unlikelyMatchString)
						&& !"body".equals(node.tagName())) {
					LOG.debug("Removing unlikely candidate - "
							+ unlikelyMatchString);
					List<Element> toRemoveAndBelow = node.getAllElements();
					elIterator.remove();
					/*
					 * adding 'node' to that set is harmless and reduces the
			...
			if ("p".equals(node.tagName()) || "td".equals(node.tagName())
					|| "pre".equals(node.tagName())) {
			...
			if ("div".equals(node.tagName())) {
				...
				if (!hasBlock) {
					...
					/* EXPERIMENTAL *//*
									 * grab just child text and wrap each chunk
									 * in a p
									 */
		 ...
		 * content-y they look. Then add their score to their parent node. A
		 * score is determined by things like number of commas, class names,
		...
		for (Element nodeToScore : nodesToScore) {
			...
			/*
			 * If the grandparent has no parent, we don't want it as a
			 * candidate. It's probably a symptom that we're operating in an
			 * orphan.
			 */
			if (grandParentNode.parent() != null
					&& "".equals(grandParentNode.attr("readability"))) {
			 ...
			 * to 3 points.
			 */
			contentScore += Math.min(Math.floor(innerText.length() / 100.0),
					3.0);
		...
		for (Element candidate : candidates) {
			...
			LOG.debug("Candidate [" + candidate.getClass() + "] ("
					+ candidate.className() + ":" + candidate.id()
					+ ") with score " + newScore);

			if (null == topCandidate
					|| newScore > getContentScore(topCandidate)) {
		...
		double siblingScoreThreshold = Math.max(10,
				getContentScore(topCandidate) * 0.2);
		...
		for (Element siblingNode : siblingNodes) {
			...
			LOG.debug("Looking at sibling node: [" + siblingNode.getClass()
					+ "] (" + siblingNode.className() + ":" + siblingNode.id()
					+ ")");
			if (scored) {
			...
			if (scored
					&& (getContentScore(siblingNode) + contentBonus >= siblingScoreThreshold)) {
				append = true;
			}
			...
			if (append) {
				...
				if (!"div".equals(siblingNode.tagName())
						&& !"p".equals(siblingNode.tagName())) {
					/*
					...
					LOG.debug("Altering siblingNode of "
							+ siblingNode.tagName() + " to div.");
	...
	private void cleanConditionally(Element e, String tag) {
		 ...
		 * the traversal. TODO: Consider taking into account original
		 * contentScore here.
		 **/
		for (int i = curTagsLength - 1; i >= 0; i--) {
			...
			LOG.debug("Cleaning Conditionally [" + ee.getClass() + "] ("
					+ ee.className() + ":" + ee.id() + ")" + contentScore);
	...
	private void cleanHeaders(Element e) {
		for (int headerIndex = 1; headerIndex < 3; headerIndex++) {
			...
			for (int i = headers.size() - 1; i >= 0; i--) {
				if (getClassWeight(headers.get(i)) < 0
						|| getLinkDensity(headers.get(i)) > 0.33) {
	...
	private void prepArticle(Element articleContent) {
		...
		Elements parasWithPreceedingBreaks = articleContent
				.getElementsByTag("br + p");
	...
	private URI findBaseUrl0(String stringUrl) throws URISyntaxException {
		...
		URI baseUrl = new URI(url.getScheme(), url.getAuthority(),
		...
		for (int i = 0; i < urlSlashes.size(); i++) {
			...
			if (segment.indexOf(".") != -1) {
				...
				/*
				 * If the type isn't alpha-only, it's probably not actually a
				 * file extension.
				 */
			...
			/*
			 * If our first or second segment is smaller than 3 characters, and
			 * the first segment was purely alphas, remove it.
			 */
			/* /i again */
			if (i < 2 && segment.length() < 3
					&& !urlSlashes.get(0).matches("[a-z]"))
				del = true;
		...
		URI cleaned = new URI(url.getScheme(), url.getAuthority(), "/"
				+ cleanedPath.substring(0, cleanedPath.length() - 1), null,
				null);
		return cleaned;
	}
	...
	private String findNextPageLink(Element body) {
		 ...
		 * id, or being a child of a node with a page-y className or id. Also
		 * possible: levenshtein distance? longest common subsequence? After we
		 * do that, assign each page a score, and
		 **/
		for (Element link : allLinks) {
			String linkHref = link.attr("abs:href").replaceAll("#.*$", "")
					.replaceAll("/$", "");

			/* If we've already seen this page, ignore it */
			if ("".equals(linkHref) || linkHref.equals(articleBaseUrl)
					|| linkHref.equals(givenUrl)
			...
			if (Patterns.EXTRANEOUS.matcher(linkText).matches()
					|| linkText.length() > 25) {
				continue;
			}

			/*
			 * If the leftovers of the URL after removing the base URL don't
			 * contain any digits, it's certainly not a next page link.
			 */
			...
			/**
			 * If the articleBaseUrl isn't part of this URL, penalize this link.
			 * It could still be the link, but the odds are lower. Example:
			...
			String linkData = linkText + " " + link.className() + " "
					+ link.id();
			...
			if (Patterns.exists(Patterns.FIRST_OR_LAST, linkData)) {
				...
				// the text,
				/*
				 * If we already matched on "next", last is probably fine. If we
			...
			while (parentNode != null) {
				String parentNodeClassAndId = parentNode.className() + " "
						+ parentNode.id();
				if (!positiveNodeMatch
						&& Patterns.match(Patterns.PAGINATION,
								parentNodeClassAndId)) {
					positiveNodeMatch = true;
					linkObj.incrementScore(25);
				}
				if (!negativeNodeMatch
						&& Patterns.match(Patterns.NEGATIVE,
								parentNodeClassAndId)) {
					/*
					 * If this is just something like "footer", give it a
					 * negative. If it's something like "body-and-footer", leave
					 * it be.
					 */
					if (!Patterns.exists(Patterns.POSITIVE,
							parentNodeClassAndId)) {
						linkObj.incrementScore(-25);
			 ...
			 * NOTE: I'm finding this to cause more harm than good where
			 * something is exactly 50 points. Dan, can you show me a
			 * counterexample where this is necessary? if
			 * (linkHref.indexOf(window.location.href) !== 0) { linkObj.score -=
			 * 1; }
			 **/
		 ...
		 * candidate for the next page URL. Require at least a score of 50,
		 * which is a relatively high confidence that this page is the next
		 * link.
		 **/
		PageLinkInfo topPage = null;
		for (Map.Entry<String, PageLinkInfo> pageEntry : possiblePages
				.entrySet()) {
			if (pageEntry.getValue().getScore() >= 50
					&& (topPage == null || topPage.getScore() < pageEntry
							.getValue().getScore())) {
