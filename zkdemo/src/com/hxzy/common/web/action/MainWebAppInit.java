 ...
 * 
 */
public class MainWebAppInit implements WebAppInit {

	private static final Log log = Log.lookup(MainWebAppInit.class);

	final static String PATH = "/WEB-INF/";

	final static String CONFIG = "main.properties";
	...
	private void loadProperites(ServletContext context) {
		try {
			InputStreamReader isr = new InputStreamReader(context.getResourceAsStream(PATH + CONFIG), "UTF-8"); 
			...
			while ((prop = bufReader.readLine()) != null) {
				...
				if (CATEGORY_TYPE.equals(arg0)) {
					...
					Category cate = new Category(key, vals[1].trim(), vals[2].trim(), vals[3]
							.trim());
					_cateMap.put(key, cate);
				} else {
					...
					if (vals.length < 4) {
						log
								.error("This demo item has no enough argument: size less than 4, for example, categoryId,FileURL,IconURL,Label");
						continue;
					}
					// [ItemId=CategoryId, Demo File URL, Icon URL, Label]
					MainItem item = new MainItem(key, arg0, vals[1].trim(), vals[2].trim(), vals[3]
							.trim());
					cate.addItem(item);
				}
			}
